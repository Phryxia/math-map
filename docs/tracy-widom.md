...
고윳값들은 독립이 아니다. 결합밀도에 $\prod_{i<j}\lvert\lambda_i - \lambda_j\rvert^{\beta}$ 라는 인자가 있어 서로 강하게 밀어낸다. 최대 고윳값이 위로 올라가려면 자기 혼자 움직이는 것이 아니라 아래 고윳값들을 전부 밀어야 하므로 오른쪽 꼬리가 짧고, 반대로 아래로 내려가려면 스펙트럼 전체가 수축해야 하므로 왼쪽 꼬리도 짧다. 그 결과 좌우 비대칭이면서 양쪽 다 정규분포보다 가벼운 꼬리를 가진 분포가 된다. 왼쪽이 $e^{-\lvert s\rvert^3/12}$ 이고 오른쪽이 $e^{-\frac43 s^{3/2}}$ 로, 양쪽의 지수마저 다르다.
