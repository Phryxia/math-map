...
세 의미론이 모두 직관주의 논리에 대해 완전하며 서로 번역된다. Kripke frame 의 위로 닫힌 집합들이 Heyting algebra 를 이루고, 위상공간의 열린집합 격자도 그렇다. 격자에서 $A \to B$ 가 "$A \wedge x \le B$ 인 최대의 $x$" 로 정의되는데, 이 상대 여원의 개념이 Kripke 의 "모든 미래에 대한 양화" 와 같은 일을 한다.
