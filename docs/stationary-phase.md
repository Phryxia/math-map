...
피적분함수의 절댓값이 어디서나 $|g|$ 로 같아 감쇠가 크기를 정하지 못하는데도 $I(\lambda)$ 는 $\lambda\to\infty$ 에서 작아진다. 원인은 이웃한 점들의 위상이 서로 달라 생기는 상쇄다. 상쇄되지 않는 곳은 위상이 국소적으로 멈추는 **정상점** $\varphi'(x_0)=0$ 이며, 그 주위에서만 기여가 살아남는다.
...
$t<0$ 이면 정상점이 둘이라 진동하고 $t>0$ 이면 실수 정상점이 없어 지수적으로 작아진다. 경계 $t=0$ 에서 두 정상점이 충돌해 퇴화한다. 무지개의 밝은 띠, 파동의 화선(caustic), WKB 근사가 전향점에서 깨지는 현상이 이 그림이다.
...
정상점이 $x_1,x_2$ 두 개이고 $\varphi''(x_1)>0>\varphi''(x_2)$ 이면
