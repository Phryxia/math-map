...
정상점이 없을 때의 소멸, 위상 인자, 그리고 두 정상점의 간섭을 직접 적분해서 본다.
...
# 1. 정상점이 없으면 초다항적으로 작다 (g 가 해석적이라 지수적으로)
print("정상점 없음:  int e^{-x^2} e^{i lam x} dx = sqrt(pi) e^{-lam^2/4}")
for lam in [2, 5, 10, 20]:
    num = simpson(lambda x: exp(-x*x)*cexp(1j*lam*x), -8, 8, 200000)
    print(f"  lam={lam:3d}: 수치 {abs(num):.3e}   정확 {sqrt(pi)*exp(-lam*lam/4):.3e}")

# 2. 한 정상점: 위상 인자 e^{i pi sgn(phi'')/4}
print("\n한 정상점:  int e^{-x^2} e^{i lam x^2} dx,  선행항 sqrt(pi/lam) e^{i pi/4}")
for lam in [10, 40, 160]:
    num = simpson(lambda x: exp(-x*x)*cexp(1j*lam*x*x), -12, 12, 400000)
    print(f"  lam={lam:4d}: 수치 {num:+.6f}   선행항 {sqrt(pi/lam)*cexp(1j*pi/4):+.6f}"
          f"   정확 {(pi/(1-1j*lam))**0.5:+.6f}")

# 3. 정상점 둘의 간섭: phi = x^3/3 - x, 정상점 x=±1, phi'' = ±2
#    phi(1) = -2/3, phi(-1) = +2/3 이므로 두 기여가 켤레가 되어 답이 실수가 된다
print("\n두 정상점:  int e^{-x^2} e^{i lam (x^3/3 - x)} dx")
print("  예측 = 2 e^{-1} sqrt(pi/lam) cos(2 lam/3 - pi/4)")
for lam in [20, 50, 100, 200, 400]:
    num = simpson(lambda x: exp(-x*x)*cexp(1j*lam*(x**3/3 - x)), -6, 6,
                  max(200000, int(lam*4000)))
    pred = 2*exp(-1)*sqrt(pi/lam)*cos(2*lam/3 - pi/4)
    print(f"  lam={lam:4d}: 수치 {num.real:+.6f}   예측 {pred:+.6f}   |차이| {abs(num-pred):.1e}")

# 정상점 없음:  int e^{-x^2} e^{i lam x} dx = sqrt(pi) e^{-lam^2/4}
#   lam=  2: 수치 6.520e-01   정확 6.520e-01
#   lam=  5: 수치 3.422e-03   정확 3.422e-03
#   lam= 10: 수치 2.462e-11   정확 2.462e-11
#   lam= 20: 수치 3.755e-16   정확 6.594e-44
#
# 한 정상점:  int e^{-x^2} e^{i lam x^2} dx,  선행항 sqrt(pi/lam) e^{i pi/4}
#   lam=  10: 수치 +0.414551+0.375163j   선행항 +0.396333+0.396333j   정확 +0.414551+0.375163j
#   lam=  40: 수치 +0.200596+0.195644j   선행항 +0.198166+0.198166j   정확 +0.200596+0.195644j
#   lam= 160: 수치 +0.099391+0.098772j   선행항 +0.099083+0.099083j   정확 +0.099391+0.098772j
#
# 두 정상점:  int e^{-x^2} e^{i lam (x^3/3 - x)} dx
#   예측 = 2 e^{-1} sqrt(pi/lam) cos(2 lam/3 - pi/4)
#   lam=  20: 수치 +0.291426   예측 +0.291556   |차이| 1.3e-04
#   lam=  50: 수치 +0.082047   예측 +0.078353   |차이| 3.7e-03
#   lam= 100: 수치 -0.129727   예측 -0.129856   |차이| 1.3e-04
#   lam= 200: 수치 +0.076341   예측 +0.076053   |차이| 2.9e-04
#   lam= 400: 수치 -0.026226   예측 -0.026391   |차이| 1.7e-04
```

**첫 묶음에서 $\lambda=20$ 의 두 값이 어긋난다.** 정확값 $6.6\times10^{-44}$ 는 배정밀도로 표현조차 되지 않는 크기이고, 수치가 준 $3.8\times10^{-16}$ 은 Simpson 격자의 반올림 바닥이다. 이 어긋남이 오히려 요점을 보여 준다. **$10^{44}$ 배의 상쇄**가 일어나고 있는 것이고, 그것을 만들어 낸 것은 어디서나 크기가 $e^{-x^2}$ 로 같은 피적분함수의 위상뿐이다. Laplace 방법에서는 이런 규모의 상쇄가 원리적으로 일어나지 않는다.

**둘째 묶음에서 선행항과 정확값의 차이가 $1/\lambda$ 로 준다.** 이 적분은 $\sqrt{\pi/(1-i\lambda)}$ 로 정확히 계산되므로 선행항 $\sqrt{\pi/\lambda}e^{i\pi/4}$ 와의 비가 $(1-i/\lambda)^{-1/2}$ 다. 실수부와 허수부가 선행항에서는 정확히 같은데($e^{i\pi/4}$ 이므로) 참값에서는 조금 어긋나 있고, 그 어긋남이 $\lambda=10\to160$ 에서 $0.039\to0.006$ 으로 줄어든다. 위상 인자 $\pi/4$ 는 극한에서만 정확하다.

**셋째 묶음이 간섭이다.** $\lambda$ 를 키울 때 값이 $+0.29,\,+0.08,\,-0.13,\,+0.08,\,-0.03$ 으로 부호를 바꾸며 진동한다. 크기는 $\lambda^{-1/2}$ 로 줄지만 단조롭지 않다. 두 정상점 $x=\pm1$ 의 위상차 $2\lambda/3$ 이 만드는 $\cos$ 때문이고, 공식이 그 진동까지 맞힌다. 답이 실수로 나오는 것도 우연이 아니다. $\varphi$ 가 홀함수이고 $g$ 가 짝함수라 두 기여가 켤레이기 때문인데, 이때 $e^{\pm i\pi/4}$ 두 위상 인자가 켤레 쌍을 이루는 것이 필수적이다. **위상 인자의 부호를 하나라도 틀리면 $\cos$ 이 $\sin$ 으로 바뀌어 예측이 완전히 어긋난다.** $\lambda=50$ 에서 상대오차가 유독 큰 것은 그 근처에서 $\cos$ 이 영에 가까워 선행항이 작아지고 $O(\lambda^{-1})$ 보정이 상대적으로 두드러지기 때문이다.
...
[^1]: 표준적인 서술은 L. Hörmander, *The Analysis of Linear Partial Differential Operators I* (1983), 7.7 절. 고차원 정상위상 공식과 Morse 지표의 취급이 여기 있다. 퇴화 정상점의 분류는 특이점 이론(Arnold) 쪽 이야기다.
[^2]: 반고전 극한과 Maslov 지표는 V. Guillemin–S. Sternberg, *Geometric Asymptotics* (1977). 본문의 수치 계산은 직접 한 것이다.
