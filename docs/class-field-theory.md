...
CM 방법은 원하는 위수를 갖는 타원곡선을 유한체 위에 만들 때 쓴다. 적당한 허수 이차체의 힐베르트 유체 다항식을 계산해 그 근으로 $j$ 불변량을 얻으며, 곡선을 무작위로 뽑아 점 개수를 세는 것보다 빠르다. 특정 매장 차수를 갖는 곡선이 필요한 [쌍 기반 암호](pairing-based-cryptography.md)에서는 사실상 유일한 수단이다.
