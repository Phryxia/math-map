...
차수가 곱셈적이라는 사실이 불가능성 증명의 엔진이다. 작도로 얻을 수 있는 수는 2차 확대를 유한 번 쌓아서만 도달하므로 차수가 $2$ 의 거듭제곱이어야 하고, 세제곱근 $2$ 는 차수가 3이므로 도달할 수 없다.
...
확대체의 원소 연산은 최소다항식으로 나눈 나머지 연산으로 구현된다.

```python
# Q(√2) = Q[x]/(x^2 - 2) 에서 (a + b√2)(c + d√2) 를 x^2 = 2 로 환원
def mul(p, q):
    a, b = p
    c, d = q
    return (a * c + 2 * b * d, a * d + b * c)

print(mul((1, 1), (1, -1)))  # (1+√2)(1-√2) = -1  ->  (-1, 0)
```
