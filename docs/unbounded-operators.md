...
유한차원에 없던 구별도 생긴다. 대칭은 내적 안에서 옮길 수 있다는 성질이고 자기수반은 정의역까지 일치한다는 조건이다. [스펙트럼 정리](spectral-theorem.md)와 유니터리 시간 발전은 자기수반일 때만 작동하며, 양자역학에서 경계조건이 물리적으로 중요한 근거가 여기 있다.
...
수반의 정의역은 훨씬 크다. 경계조건 없이 미분이 $L^2$ 에 있는 모든 함수이며, $T^\ast f=-if'$ 다. 결손공간을 계산하면 $-if'=\pm if$ 즉 $f(x)=e^{\mp x}$ 이고 둘 다 $L^2[0,1]$ 에 있으므로 $n_+=n_-=1$ 이다.
