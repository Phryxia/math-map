...
- $\mathcal D_0$: 양 끝에서 0 이 되는 매끄러운 함수들.
- $\mathcal D_\theta$: $f(1)=e^{i\theta}f(0)$ 인 함수들.
- $\mathcal D_{\max}$: 미분이 $L^2$ 에 있는 모든 함수.
...
같은 작용소를 반직선 $[0,\infty)$ 에서 보면 사정이 바뀐다. $e^{-x}$ 만 제곱적분가능하므로 $n_+=1$, $n_-=0$ 이고, 자기수반 확장이 하나도 없다. 반직선 위의 운동량 관측량이 존재하지 않는다는 것이 이 계산의 물리적 의미다.
...
$T$ 가 자기수반이고 $V$ 가 대칭이며 상대적으로 유계, 즉 어떤 $a<1$, $b$ 에 대해
...
Schrödinger 방정식뿐 아니라 열방정식과 파동방정식도 같은 틀로 다룬다. 자기수반 작용소에서 $e^{-tT}$, $\cos(t\sqrt T)$ 같은 함수를 만들어 해를 쓰며, 이것이 반군 이론으로 일반화된다. 자기수반이 아닌 생성원에 대해서는 Hille–Yosida 정리가 대응하는 조건을 준다.
