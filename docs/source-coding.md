...
KL divergence 가 항상 0 이상이고 Kraft 부등식에 의해 $S \le 1$ 이므로 $-\log_2 S \ge 0$ 이다. 등호는 $S = 1$ 이고 $p = q$ , 즉 모든 확률이 2 의 거듭제곱의 역수이고 $\ell(a) = -\log_2 p(a)$ 일 때만 성립한다. 부호 설계는 결국 분포를 추측하는 일이며, 틀린 분포 $q$ 를 믿고 부호를 짜면 정확히 $D(p\|q)$ 비트만큼 손해를 본다.
