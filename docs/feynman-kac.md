...
방정식에 $-c(x)u$ 항이 있으면 기댓값 안에 $\exp(-\int c\thinspace ds)$ 가 붙는다. 이 인자를 확률로 읽을 수 있다. 입자가 비율 $c(x)$ 로 소멸한다고 하면, 만기까지 살아남을 확률이 정확히 그 지수다. 즉 소멸항이 있는 방정식의 해는 "살아남은 입자들만 센 기댓값" 이다.
...
일반적인 $c$ 와 $g$ 에 대해서는 $Y_s = u(s,X_s)e^{-\int_t^sc\thinspace dr} + \int_t^s g\thinspace e^{-\int_t^rc}\thinspace dr$ 로 잡으면 같은 계산이 된다. 지수 인자를 미분할 때 나오는 $-cu\thinspace ds$ 와 적분 항에서 나오는 $g\thinspace ds$ 가 방정식의 남은 두 항을 정확히 상쇄한다.
