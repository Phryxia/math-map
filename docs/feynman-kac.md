...
무작위 걷기와 조화함수 사이에 같은 구조가 있다. 격자에서 $u(x)$ 가 이웃 평균과 같다는 이산 Laplace 방정식은 $u(X_n)$ 이 martingale 이라는 것과 동치이고, 경계값 문제의 해는 경계에 처음 닿는 지점에서의 경계값의 기댓값이다. Feynman–Kac 은 이 관찰의 연속시간 판이고 소멸항이 있는 경우까지 다룬다.
