...
두 이동에 불변인 양을 만드는 절차는 Reidemeister 이동을 다룰 때와 같다. 라벨 붙은 링크에 값을 매기는 규칙을 세우고 이동마다 값이 변하지 않음을 확인한다. Kirby I 은 값을 $\Delta\_\pm$ 로 나누어 맞추고, Kirby II 는 아래의 소멸 성질이 맞춘다. 라벨 하나를 고정하면 미끄러뜨릴 때 라벨이 섞이므로 모든 라벨을 한꺼번에 쓴다.
...
$(S^2)\_{0j}=\delta_{j0}$ 을 쓴 곳이 모듈러 조건이 들어가는 자리다. $S$ 가 가역이 아니면 $S^2$ 가 전하켤레 치환이 되지 못하고, 소멸이 일어나지 않아 Kirby II 가 깨진다. 모듈러 조건 없이도 매듭 불변량은 얻지만 3 차원 다양체 불변량으로는 올라가지 못하는 이유가 이 한 자리에 있다.
...
첫 식은 $\Delta_+/\mathcal D$ 가 절댓값 1 임을 뜻하고, 둘째 식이 그 위상각의 정체를 밝힌다. $c$ 는 순수하게 [범주](category.md)에서 정의되는 유리수인데, 그 범주가 [정점작용소대수](vertex-operator-algebras.md)(vertex operator algebra, VOA)에서 왔다면 $c$ 가 그 VOA 의 중심 전하와 일치한다. $\mathrm{SU}(2)\_k$ 라면
...
라벨을 $n$ 으로 올리면 색 Jones 다항식이 나오고, $\mathfrak{sl}\_N$ 을 쓰면 HOMFLY(Hoste–Ocneanu–Millett–Freyd–Lickorish–Yetter), 직교·심플렉틱 계열에서는 Kauffman 다항식이 나온다. Jones 가 작용소대수에서 우연히 발견한 다항식이 왜 하필 그런 형태인지에 대한 답이 이 사전이다. Jones 다항식은 $\mathrm{SU}(2)\_1$ 에서 뽑은 하나의 열이고, 3 차원 다양체 불변량은 같은 구성을 수술까지 밀어 올린 결과다.
...
- **범주화**: [Khovanov 호몰로지](khovanov-homology.md)가 Jones 다항식을 사슬복합체의 [Euler 지표](euler-characteristic.md)로 올려놓듯, $\tau$ 를 범주화하는 시도가 이어지고 있다.
