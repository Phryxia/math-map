...
이 부등식은 L2 수렴까지 준다. 독립성은 완전히 쓰이지 않고 쌍마다 상관이 없다는 조건이면 충분하다. 분산이 무한한 경우의 약한 법칙(Khinchin)은 절단 논법을 쓴다. 절댓값이 상수 c 를 넘는 부분을 잘라낸 변수에 위 논법을 적용하고, 잘라낸 꼬리의 기여가 적분 가능성에 의해 c 를 키우면 0 으로 간다.

## 강한 법칙의 증명

4 차 모멘트가 유한하다고 가정하면 증명이 짧다. 중심화한 합의 4 차 모멘트를 전개하면 독립성 때문에 살아남는 항의 개수가 n 의 제곱 규모이므로 다음을 얻는다.
