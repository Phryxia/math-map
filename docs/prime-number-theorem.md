...
따라서 0 점의 실수부가 오차의 크기를 직접 결정한다. 실수부가 $1$ 에 가까운 0 점이 있으면 오차가 크고, 모두 $1/2$ 에 몰려 있으면 오차가 $\sqrt x$ 수준으로 작다. 소수 정리 자체는 "실수부가 1 인 0 점이 없다" 만으로 충분하고, Riemann 가설은 "모두 실수부가 정확히 $1/2$ 이다" 라는 훨씬 강한 주장이다.
