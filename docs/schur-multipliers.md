...
두 번째가 본론이다. $G$ 가 완전군($G=[G,G]$)이면 **보편 중심확대** $\tilde G$ 가 존재한다.
...
이것이 2-코사이클 조건이다. 한편 대표원을 다시 고르는 것($\rho'(g)=\lambda(g)\rho(g)$)은 $c$ 를 다음만큼 바꾼다.
...
생성된 위수 8 군은 $\{\pm I,\pm X,\pm Z,\pm XZ\}$ 이고 $X^2=Z^2=I$, $(XZ)^2=-I$ 이므로 이면체군 $D_4$ 다. 이 확대를 통해 $(\mathbb Z/2)^2$ 이 2 차원 기약 사영표현을 갖게 되는데, 아벨군의 보통 기약표현은 전부 1 차원이므로 이것은 사영표현에서만 가능한 현상이다. 양자정보에서 Pauli 군이라 부르는 대상이 바로 이것이고, 안정자 부호의 대수적 뼈대다.
...
| $A_n$, $n\ge5$, $n\neq6,7$ | $\mathbb Z/2$ |
| $A_6,A_7$ | $\mathbb Z/6$ |
| $\mathrm{PSL}_2(q)$, 대부분 | $\mathbb Z/2$ |
| $M_{11},M_{23},M_{24}$ | 자명 |
| $M_{12},M_{22}$ | $\mathbb Z/2$, $\mathbb Z/{12}$ |
| 괴물군 $\mathbb M$ | 자명 |
...
산재군의 곱셈자 표에서 $6.\mathrm{Suz}$, $12.M_{22}$, $3.J_3$ 같은 예외적으로 큰 덮개들이 눈에 띄고, 이들은 격자나 부호 위의 작용을 구성할 때 실제로 필요한 형태다. Leech 격자 위의 작용에서 $6.\mathrm{Suz}$ 가 자연스럽게 나타나는 것이 대표적이다.
