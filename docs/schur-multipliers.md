...
생성된 위수 8 군은 $\{\pm I,\pm X,\pm Z,\pm XZ\}$ 이고 $X^2=Z^2=I$ 이고 $(XZ)^2=-I$ 이므로 이면체군 $D_4$ 다. 이 확대를 통해 $(\mathbb Z/2)^2$ 이 2 차원 기약 사영표현을 갖게 되는데, 아벨군의 보통 기약표현은 전부 1 차원이므로 이것은 사영표현에서만 가능한 현상이다. 양자정보에서 Pauli 군이라 부르는 대상이 바로 이것이고, 안정자 부호의 대수적 뼈대다.
