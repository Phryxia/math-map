...
증명은 Farkas 보조정리를 쓴다. 선형 부등식계에 해가 없으면 그 사실을 증명하는 음이 아닌 선형결합이 존재한다는 정리다. 두 최적값이 다르다고 가정하면 해가 없는 부등식계가 나오고, Farkas 보조정리가 주는 선형결합의 계수가 차이를 없애는 쌍대 허용해가 된다. 볼록성의 분리초평면 정리로도 같은 결론을 얻는다.
...
- 변수에 정수 제약을 더한 정수계획법은 NP(nondeterministic polynomial time)-hard 다. [P 대 NP 문제](p-np.md)와 이어진다.
...
- 최대유량과 최소절단의 동치, 이분 [그래프](graphs.md)의 최대 매칭과 최소 정점 덮개의 동치(König 정리)는 선형계획 쌍대성에 정수성이 더해진 결과다. [네트워크 흐름](network-flow.md)의 최대유량 최소절단 정리가 그 예다.
