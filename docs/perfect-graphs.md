...
그래프에서 최대 클릭의 크기 `\omega(G)` 는 채색수 `\chi(G)` 의 자명한 하계다. 클릭 안의 정점은 서로 인접하므로 색이 모두 달라야 하기 때문이다. 두 값의 간격은 얼마든지 벌어진다. 삼각형이 없으면서(`\omega=2`) 채색수가 임의로 큰 그래프가 존재한다.
...
꼭짓점이 4 개 이하인 그래프는 모두 완전하다. 홀수 구멍 중 가장 작은 것이 `C_5` 이기 때문이다. 삼각형 `C_3` 은 완전하고(`\omega=\chi=3`), 그다음 홀수 순환이 `C_5` 다.
...
`G` 가 완전할 필요충분조건은 `G` 가 Berge 그래프인 것이다. Chudnovsky, Robertson, Seymour, Thomas 가 2002 년에 증명했으며 논문이 150 쪽이 넘는다.
...
Mirsky 정리(최장 사슬의 길이 = 반사슬 분할의 최소 개수)가 나머지 절반이며, 두 정리가 약한 완전그래프 정리로 서로의 보그래프 진술이 된다. `\alpha`, `\omega`, `\chi` 사이의 최소최대 등식들이 완전성이라는 한 틀에 모인다.
