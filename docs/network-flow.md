...
가 된다. 즉 "모든 $s\text{-}t$ 경로를 길이 1 이상으로 만드는 최소 비용의 간선 길이 배정"이다. 이것이 **분수 절단** 문제이며, 최대유량 최소절단 정리는 이 LP의 최적해가 항상 $0/1$ 값으로 잡힐 수 있다는 정수성 주장으로 읽힌다. 실제로 흐름 문제의 제약 행렬(접합 행렬)은 완전 유니모듈러이므로 정수 꼭짓점이 보장된다. LP 쌍대성 일반론은 [선형계획법](linear-programming.md)과 [Lagrange 쌍대성](lagrange-duality.md)에 있고, 흐름 문제에서는 강 쌍대성이 조합론적으로 구성된다.
