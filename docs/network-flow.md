...
**네트워크**는 유향 그래프 $G = (V, E)$ 와 용량 함수 $c : E \to \mathbb{R}_{\ge 0}$, 그리고 서로 다른 두 정점 source $s$ 와 sink $t$ 의 조합이다.
...
$s \in S$ 이고 $t \in T = V \setminus S$ 인 분할 $(S, T)$ 를 **$s$-$t$ 절단**이라 하고, 그 용량을
...
(2) ⇒ (3): $S$ 를 $G_f$ 에서 $s$ 로부터 도달 가능한 정점 집합으로, $T = V \setminus S$ 로 둔다. 가정에 의해 $t \notin S$ 이므로 이는 $s$-$t$ 절단이다. $u \in S$ 이고 $v \in T$ 이고 $(u,v) \in E$ 라면 잔여 용량이 0이어야 하므로 $f(u,v) = c(u,v)$ 다. $u \in T$ 이고 $v \in S$ 이고 $(u,v) \in E$ 라면 역방향 잔여 용량이 0이어야 하므로 $f(u,v) = 0$ 이다. 약 쌍대성 보조정리의 등식에 대입하면 $\lvert f \rvert = \operatorname{cap}(S, T)$ 를 얻는다.
...
이후 개선 계보는 Dinic( $O(V^2 E)$, 단위 용량에서는 $O(E\sqrt{E})$ ), push–relabel( $O(V^3)$ ), 그리고 최근의 거의 선형 시간 알고리즘으로 이어진다.
...
경로 기반으로 쓰면 더 투명하다. $P$ 를 모든 $s$-$t$ 경로의 집합이라 하고 $x_p \ge 0$ 을 경로 $p$ 에 흘리는 양이라 하면
...
단위 용량 네트워크에서 Dinic은 $O(E\sqrt{V})$ 로 돌고, 이는 Hopcroft–Karp와 같은 복잡도다. 우연이 아니라 같은 알고리즘의 두 얼굴이다.
