...
끝난 상태가 최적인 이유는 절단이 증명서가 되기 때문이다. $s$ 에서 잔여 그래프로 도달 가능한 정점 집합을 $S$ 라 하면, $S$ 에서 나가는 원래 간선은 전부 포화이고 $S$ 로 들어오는 간선은 전부 0 이다. 둘 중 하나라도 어긋나면 잔여 간선이 남아 $S$ 가 더 커진다. 그러므로 흐르는 양이 $S$ 를 가르는 절단의 용량과 같고, 모든 흐름이 모든 절단 이하이므로 이 흐름은 최대이고 이 절단은 최소다.

개선 방향이 없는 상태에서 최적성 증명서가 따라 나오는 이 구조는 [매칭과 Hall 정리](matchings.md)의 Berge 정리, [선형계획법](linear-programming.md)의 상보 여유 조건과 같다.
