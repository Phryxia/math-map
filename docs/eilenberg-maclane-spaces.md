...
코호몰로지는 사슬 복합체로 정의되지만 이 대응을 거치면 공간 사이의 사상 집합이 된다. 코호몰로지 연산과 장애 이론의 계산이 이 대응 위에서 이루어진다.
...
$H^n(-;G)$ 에서 $H^m(-;G')$ 로 가는 자연변환 전체는 $H^m(K(G,n);G')$ 과 일대일 대응한다. 표현가능성을 Yoneda 보조정리에 넣은 결과다.
...
연결 공간 $X$ 마다 사상들의 탑 $\dots\to X_n\to X_{n-1}\to\dots\to X_1$ 이 있어 $X_n$ 의 호모토피군이 $n$ 차까지는 $X$ 와 같고 그 위로는 $0$ 이다. 각 단계 $X_n\to X_{n-1}$ 은 올이 $K(\pi_n(X),n)$ 인 올다발이고, 그 올다발을 정하는 자료가 $H^{n+1}(X_{n-1};\pi_n(X))$ 의 원소다.
