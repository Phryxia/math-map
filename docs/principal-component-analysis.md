...
평균을 원점으로 옮긴 데이터 구름은 타원체 모양으로 퍼진다. 이 타원체의 가장 긴 축이 첫 주성분이고 그에 직교하는 가장 긴 축이 두 번째 주성분이다. 공분산행렬이 이 타원체의 대수적 표현이고, [스펙트럼 정리](spectral-theorem.md)에 따라 대칭행렬은 직교하는 주축을 가진다. [고윳값](eigenvalues.md)이 그 축 방향의 분산이다.
...
**재구성 오차 최소화.** $k$ 차원 부분공간 $W$ 로의 정사영을 $P_W$ 라 하면 다음 최소화 문제의 해가 상위 $k$ 개 주성분이 생성하는 공간이다.
...
내적을 커널로 바꾸면 커널 PCA 가 되고, 사영과 재구성을 신경망으로 바꾸면 오토인코더가 된다. 선형 오토인코더의 최적해는 주성분이 생성하는 공간이다. 관측을 저차원 잠재변수의 선형변환에 등방성 잡음을 더한 것으로 보는 확률적 PCA 의 최대가능도 해도 고전적 PCA 와 일치한다.
