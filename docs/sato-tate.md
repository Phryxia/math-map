...
$n=1$ 의 해석적 접속은 모듈러성 정리(Wiles 등)가 준다. $n=2$ 도 Gelbart–Jacquet 이 일찍 해결했고 $n=3,4$ 까지는 Kim–Shahidi 가 갔다. 문제는 **모든 $n$ 에 대한 결과**가 필요하다는 것이다.
...
- 소수의 정확히 절반에서 $a_p=0$ 이다. 실측 비율은 $0.5027$ 이다. 그리고 그 소수가 **전부** $p\equiv2\pmod3$ 다. $\mathbb Q(\zeta_3)$ 에서 불활성인 소수이고, 거기서 곡선이 초특이다.
...
측도가 곡선의 대칭군에 달려 있다는 진술이 표 두 개로 드러난다. $\mathrm{SU}(2)$ 면 $\sin^2$ 이고 $\mathrm{U}(1)$ 이면 균등이다.
