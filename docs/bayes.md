...
사건 `H₁, …, H_m` 이 전체를 서로 겹치지 않게 분할하고 각 확률이 양수이면
...
`P(E|H)` 와 `P(H|E)` 는 다르다. 결함률 `1%`, 결함 부품이 검사에 걸릴 확률 `90%`, 정상 부품이 잘못 걸릴 확률 `5%` 라 하면
...
사전확률이 `0.001` 이면 한 번 양성이어도 사후는 `1.8%` 에 그친다. 반면 사전 `0.01` 에서 양성이 세 번 반복되면 `0.15`, `0.77`, `0.98` 로 올라간다. 한 번의 검사 성능보다 사전확률과 증거의 누적이 결론을 지배한다는 점이 수치로 드러난다. 다만 아래에서 보듯 반복 갱신은 조건부 독립을 전제한다.
