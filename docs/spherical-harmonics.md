...
원 위의 함수는 $e^{in\theta}$ 로 전개한다. 구면 위의 함수는 무엇으로 전개하는가.

답은 회전군이 정한다. $L^2(S^2)$ 에 $\mathrm{SO}(3)$ 가 작용하고, 이 [Hilbert 공간](hilbert-spaces.md)이 [기약표현](group-representations.md)들의 직교합으로 쪼개진다.

$$
L^2(S^2)=\widehat{\bigoplus_{\ell\ge0}}\ \mathcal H_\ell,\qquad \dim\mathcal H_\ell=2\ell+1
$$

$\mathcal H_\ell$ 의 원소가 차수 $\ell$ 의 **구면조화함수**다. 각 $\mathcal H_\ell$ 은 $\mathrm{SO}(3)$ 의 기약표현이고, $\ell$ 마다 정확히 하나씩 나온다. 곧 이 분해는 회전군의 표현론 그 자체다.

전개가 왜 이렇게 되는지도 표현론이 설명한다. 구면 Laplace 작용소는 회전과 교환하므로 그 고유공간이 표현의 불변 부분공간이 되고, Schur 보조정리가 기약 조각으로의 분해를 강제한다. 원에서 $e^{in\theta}$ 가 나오는 이유가 $\mathrm{SO}(2)$ 의 기약표현이 1 차원 문자들이기 때문인 것과 정확히 같은 논리이며, 군이 비아벨이 되면서 조각의 차원이 1 에서 $2\ell+1$ 로 커졌을 뿐이다.
...
이다. 아래 코드가 이 계산을 $\ell\le8$ 에서 직접 확인한다.
...
$\mathrm{SO}(3)$ 는 $\mathcal H_\ell$ 에 작용한다. 회전은 $\Delta$ 와 교환하고 차수를 보존하기 때문이다. 이 표현이 기약이라는 것이 핵심 사실이다.

기약성을 보면 분해가 유일해진다. $\Delta_{S^2}$ 의 $-\ell(\ell+1)$ 고유공간이 $\mathcal H_\ell$ 이고, 고유값이 서로 다르므로 고유공간이 직교한다. 회전불변 작용소는 각 $\mathcal H_\ell$ 위에서 스칼라로만 작용할 수 있다. Schur 보조정리의 직접적 귀결이며, 이것이 구면 위의 합성곱과 필터링이 "각 $\ell$ 마다 하나의 수" 로 서술되는 이유다.
...
아벨군에서는 기약표현이 모두 1 차원이라 각 주파수가 함수 하나씩이었다. 비아벨이 되면 같은 고유값을 공유하는 함수가 여러 개 묶여 나오고, 그 묶음이 표현의 단위가 된다.
...
좌변은 $\mathcal H_\ell$ 로의 사영을 주는 핵인데, 우변이 두 방향의 내적에만 의존한다. 사영이 회전불변 작용소이므로 그 핵이 불변량 $\hat x\cdot\hat y$ 의 함수여야 한다는 표현론적 이유에서 나온다. $\ell$ 번째 Legendre 다항식이 이 자리에 있는 것은 대상이 아니라 결과다.
...
- **우주론.** 우주 마이크로파 배경의 온도 요동을 $\ell$ 별 검정력 $C_\ell$ 로 요약한다. 회전불변성 때문에 $m$ 에 대한 정보가 통계적으로 무의미해지고 $\ell$ 마다 수 하나만 남는다는 사실이 Schur 보조정리의 직접적 결과다.
