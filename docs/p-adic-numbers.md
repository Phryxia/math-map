...
- 국소해석적 함수, $p$ 진 [측도](measure.md), $p$ 진 $L$ 함수의 이론이 발달해 있다. Kubota–Leopoldt 의 $p$ 진 zeta 함수가 Bernoulli 수의 합동 관계를 보간하며 Iwasawa 이론의 출발점이 된다.
...
컴퓨터 대수 시스템이 $\mathbb Z[x]$ 의 다항식을 인수분해하는 표준 경로가 Hensel 이다. 적당한 $p$ 를 골라 $\mathbb F_p[x]$ 에서 인수분해하고 그 인수들을 법 $p^k$ 로 올린 뒤 계수 상계를 넘을 때까지 $k$ 를 키워 $\mathbb Z[x]$ 의 인수를 복원한다. 인수의 조합을 시험하는 마지막 단계가 지수적이라 격자 환원으로 바꾼 것이 LLL(Lenstra–Lenstra–Lovász) 기반 알고리즘이다.
