...
군의 부분군, 환의 아이디얼, 벡터 공간의 부분공간은 각각 격자를 이룬다. 두 부분대상의 만남은 교집합이고 이음은 둘을 포함하는 최소 부분대상이다. 이 격자가 분배인지 모듈러인지가 구조의 성질을 반영한다. [가군](modules.md)의 부분가군 격자는 모듈러이고, 이것이 Jordan–Hölder 정리의 배경이다.
