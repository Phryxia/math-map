...
*정리.* $k$ 가 대수적으로 닫힌 체이고 $V$ 가 $k$ 위의 유한차원 벡터 공간이면, 각 선형사상 $T\colon V\to V$ 에 대해 $T$ 의 행렬이 Jordan 행렬이 되는 기저가 존재한다. 블록의 다중집합은 블록의 순서를 빼고 유일하다[^1].
...
- 거듭제곱의 점근. $A^m$ 의 각 블록은 $\binom{m}{i}\lambda^{m-i}$ 꼴 성분을 가지므로, $|\lambda|<1$ 인 블록은 크기와 무관하게 0 으로 가고 $|\lambda|=1$ 이면서 크기가 2 이상인 블록이 있으면 $A^m$ 은 발산한다. [Markov 연쇄](markov-chains.md)의 전이행렬이 수렴하는지 판정할 때 쓰인다.
