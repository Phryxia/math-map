...
자연변환은 같은 두 범주 사이를 잇는 두 [functor](functors.md) 를 비교하는 방법이다. $F$ 와 $G$ 가 대상마다 내놓는 결과를 사상 하나로 이어 주되, 그 이음이 원래 범주의 모든 사상과 양립해야 한다는 조건을 단다.

이 조건이 "자연스럽다" 는 일상어에 정확한 뜻을 준다. 유한차원 벡터 공간과 그 쌍대공간은 차원이 같아 동형이지만 그 동형은 기저를 골라야 정해진다. 반면 벡터 공간과 이중쌍대공간 사이의 동형은 아무것도 고르지 않고 만들 수 있다. 수학에서 오래 쓰이던 이 구별이 자연변환 하나로 형식화된다. 임의의 선택 없이, 모든 사상과 동시에 어울리는 대응만이 자연변환이다.

# 직관

## 선택 없이 만들어지는 대응

$V$ 에서 $V^*$ 로 가려면 기저를 하나 정해야 한다. 기저를 바꾸면 대응도 바뀌므로 이 동형은 선형사상들과 일관되게 어울리지 않는다. 반면 $V$ 에서 $V^{**}$ 로 가는 대응은 $v$ 를 "$v$ 에서 값을 재는 함수" $\varphi \mapsto \varphi(v)$ 로 보내면 끝이고, 아무 선택도 필요 없다. 그래서 전자는 자연동형이 아니고 후자는 자연동형이다.

## 사각형이 닫혀야 한다

자연변환의 조건은 그림 하나로 요약된다. $C$ 의 사상 $f$ 를 먼저 따라간 뒤 옮기는 것과, 옮긴 뒤 $D$ 쪽에서 따라가는 것이 같아야 한다.
...
대상마다 아무 사상이나 고르면 이 사각형이 닫히지 않는다. 사각형이 모든 $f$ 에 대해 닫힌다는 것은 대응이 각 대상을 따로 보고 정해진 것이 아니라 구조 전체를 보고 한 번에 정해졌다는 뜻이다.
...
$\alpha_c$ 를 $\alpha$ 의 $c$ 성분이라 한다. 모든 성분이 동형사상이면 $\alpha$ 를 자연동형이라 하고, 이때 성분별 역 $\alpha_c^{-1}$ 도 naturality 를 만족해 $G \Rightarrow F$ 인 역 자연변환이 된다.
...
$C$ 에서 $D$ 로 가는 functor 들을 대상으로, 자연변환들을 사상으로, 수직 합성을 합성으로 삼으면 범주 $[C, D]$ 가 된다. 항등 자연변환은 각 성분이 항등사상인 것이고, 이 범주에서의 동형이 곧 자연동형이다. Functor 를 대상으로 삼는 이 시점부터 범주론은 한 층 위에서 전개된다.

# 성질

## 자연동형과 범주의 동치

Functor $F : C \to D$ 와 $G : D \to C$ 에 대해 $GF \cong \mathrm{id}_C$ 이고 $FG \cong \mathrm{id}_D$ 인 자연동형이 있으면 두 범주는 동치다. 동형(대상이 일대일 대응)보다 약하지만 실제로 쓰이는 것은 거의 항상 동치이며, "같다" 를 대상 수준이 아니라 사상까지 포함해 재는 올바른 방식이다.
...
리스트를 뒤집는 연산은 원소가 무엇인지 보지 않고 위치만 다루므로 `map f (reverse xs) = reverse (map f xs)` 가 항상 성립한다. 값을 들여다보는 연산은 그렇지 않다. 아래에서 그 차이를 확인한다.
...
`picky` 가 깨지는 이유는 분명하다. 원소를 `f` 로 옮기고 나면 조건을 만족하는 원소가 달라지므로, 언제 거르느냐에 따라 답이 바뀐다. 자연변환은 "무엇이 들어 있는지 묻지 않고 구조만 다루는" 연산이고, 프로그래밍에서 타입 변수에 대해 다형인 함수가 대체로 자연변환인 것이 이 때문이다.
...
세 주제 모두 "자연변환을 셀 수 있게 만든다" 는 공통점이 있다. 자연변환이 범주론의 기본 단위인 이유다.

# 활용

## 구성의 일관성을 진술하기

"이 동형은 표준적이다", "이 공식은 좌표에 의존하지 않는다", "이 최적화는 언제나 안전하다" 같은 문장은 모두 어떤 자연변환의 naturality 로 번역된다. 증명해야 할 것이 사각형 하나로 고정되므로, 막연한 자연스러움 대신 검사 가능한 조건이 남는다.

## 이론 사이의 번역

호몰로지 이론의 연결 준동형, 대수적 구성의 표준 사상, 표현론의 지표 대응은 모두 자연변환으로 서술되어야 제 역할을 한다. 어떤 사상이 "모든 경우에 동시에" 정의된다는 점이 정리의 진술을 짧게 만들고 재사용 가능하게 만든다.

## 프로그램 변환

컨테이너의 구조만 바꾸는 함수(뒤집기, 앞뒤 자르기, 리스트를 옵션으로 바꾸기)는 자연변환이고, naturality 등식이 곧 컴파일러가 쓸 수 있는 재작성 규칙이 된다. `map` 을 자연변환 앞으로 옮기거나 뒤로 미루는 변환이 안전한 이유가 여기에 있다.[^1]
