...
$V$ 에서 $V^*$ 로 가려면 기저를 정해야 하고 기저를 바꾸면 대응도 바뀌므로 이 동형은 선형사상들과 일관되게 어울리지 않는다. $V$ 에서 $V^{**}$ 로 가는 대응은 $v$ 를 $\varphi \mapsto \varphi(v)$ 로 보내면 되고 선택이 필요 없다. 전자는 자연동형이 아니고 후자는 자연동형이다.
...
- **이중쌍대.** $V \mapsto V^{**}$ 는 유한차원 벡터 공간 위에서 항등 functor 와 자연동형이다. $V \mapsto V^*$ 는 반변 functor 라 항등 functor 와 비교할 수 없고, 차원이 같다는 사실로 만든 동형은 기저 선택에 의존해 naturality 가 깨진다.
