...
언어에 새 상수 $c$ 를 더하고 무한히 많은 문장 $c \gt 0$, $c \gt S(0)$, $c \gt S(S(0))$ 을 이어 공리로 추가한다. 이 집합의 유한 부분집합은 $c$ 를 충분히 큰 수로 해석하면 만족되므로, [Compactness 정리](lowenheim-skolem.md)에 의해 전체가 모형을 가진다. 그 모형에서 $c$ 는 모든 표준 자연수보다 크다.

2차 형태에서 이 논법이 막히는 이유는 2차 논리가 Compactness 정리를 만족하지 않기 때문이다. 범주성과 Compactness 는 함께 성립할 수 없다.
...
- **자연수의 구성.** [ZFC 공리계](zfc-axioms.md)(Zermelo–Fraenkel 집합론에 선택공리를 더한 공리계)에서 무한공리로 얻은 귀납적 집합의 최소 원소를 $\mathbb N$ 으로 두면 Peano 공리가 정리로 증명된다. 자연수를 원시 개념으로 두지 않고 집합으로 환원하는 경로다.
