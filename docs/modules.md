...
군환 $k[G]$ 위의 가군은 [군의 표현](group-representations.md)의 대상이고, $\mathrm{char}k$ 가 $|G|$ 를 나누지 않을 때 이 환 위의 모든 가군이 단순가군의 직합이 된다는 것이 Maschke 정리다. 이는 "가군 [범주](category.md)가 벡터 공간 범주처럼 행동하는 조건" 을 묻는 일반적 질문의 한 사례다.[^2]
