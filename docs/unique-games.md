...
다. 곧 **한쪽 변수의 값이 정해지면 다른 쪽이 유일하게 결정된다**. 이 "유일성" 때문에 만족 가능한 사례는 쉽다. 연결성분마다 값 하나를 정하면 나머지가 전파되므로 다항시간에 풀린다.

어려움은 거의 만족 가능한 경우에 나타난다. 99% 의 제약을 만족하는 배정이 있을 때 그런 배정을 (혹은 1% 라도 만족하는 배정을) 찾는 일이 어렵다는 것이 추측이다. 전파가 모순에 부딪히면 어디서 틀렸는지 알 수 없고, 오류를 감수하며 전역적으로 최선을 찾아야 한다.

## 왜 하필 이 문제인가

PCP 정리의 표준 형태는 3 항 제약(3SAT 류)에서 강력하지만 2 항 제약에서는 약하다. 그런데 최대 절단, 정점 덮개, 최대 $k$ 절단처럼 실무에서 중요한 문제들이 2 항이다. 유일게임은 **2 항이면서 난해할 수 있는 가장 단순한 후보**다. 유일성 조건을 조금만 더 강하게 하면 문제가 쉬워지고, 조금만 약하게 하면 이미 PCP 로 난해성이 알려진다. 정확히 경계에 놓여 있다.
...
```mermaid
graph TD
  UGC["유일게임 추측"] -->|"Majority is Stablest"| MC["MaxCut 최적 근사비 = α_GW"]
  UGC --> VC["정점 덮개 2-ε 난해"]
  UGC --> ALL["모든 CSP 에서<br/>기본 SDP 가 최적"]
  KMS["2-to-2 정리 (2018)"] -->|"완전성 1/2 판본"| UGC
  KMS --> VC2["정점 덮개 √2-ε 난해 (무조건)"]
  SUBEXP["준지수 시간 알고리즘<br/>(ABS 2010)"] -.->|"NP-난해성에 제약"| UGC
```
...
UGC 는 **소집합 확장 추측**(SSEH)에서 따라온다. 그래프의 작은 집합들이 모두 잘 확장되는지 판정하는 문제의 난해성인데, 이쪽이 더 순수하게 조합적이라 다루기 쉬우리라 기대된다. 두 추측이 동치인지는 알려져 있지 않고, SSEH 가 더 강하다는 것만 알려져 있다.

# 활용

## 알고리즘 설계의 지침

UGC 를 믿는 실무적 태도는 이렇다. 어떤 CSP 를 근사하고 싶으면 먼저 기본 SDP 완화를 세우고 반올림하라. 그보다 잘하려는 시도는 UGC 가 참인 한 헛수고다. 이 지침 덕분에 연구 자원이 SDP 반올림 기법의 개선과, 애초에 CSP 가 아닌 문제(그래프 분할, 클러스터링 등)로 옮겨갔다.

## 해석과 조합의 도구가 들어오다

UGC 를 둘러싼 작업은 이론 전산학에 부울 함수 해석학을 정착시켰다. 초입방체 위 Fourier 전개, 초축약성 부등식, 불변원리 같은 도구가 표준 언어가 되었고, [Expander 그래프](expander-graphs.md)의 스펙트럼 기법과 결합해 별개의 연구 분야를 이루었다. 추측 자체가 미해결이어도 그 주변에서 나온 도구가 이미 큰 성과다.
