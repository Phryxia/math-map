...
- **모든 CSP.** Raghavendra 의 정리에 의해, 임의의 제약 충족 문제(constraint satisfaction problem, CSP)에서 기본 반정부호 계획법(semidefinite programming, SDP) 완화의 적분 간극과 근사 난해성 한계가 일치한다. 문제마다 알고리즘을 새로 설계할 이유가 없어진다.
