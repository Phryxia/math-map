...
로 잡는 것이 최적이다. 증명 스케치: 임의의 다른 기각역 `R'` 와 비교하여 `R \ R'` 에서는 `Λ > c`, `R' \ R` 에서는 `Λ <= c` 이므로, 두 검정력 차이를 두 영역의 적분으로 쪼개면 부호가 고정된다. [가설검정](hypothesis-testing.md)에서 p-값 계산이 우도비에 의존하는 이유다.
