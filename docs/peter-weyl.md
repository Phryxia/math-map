...
$G=S^1$ 이면 이 분해가 [Fourier 급수](fourier-series.md)이고 $G=\mathrm{SO}(3)$ 이면 [구면조화함수](spherical-harmonics.md) 전개다. 둘 다 군 위의 함수를 기약표현으로 분해한 것이다.
...
콤팩트성은 여기에 쓰인다. Haar 측도의 전체 질량이 유한해야 평균이 정의된다. $G=\mathbb R$ 에서는 전체 질량이 무한이어서 이 논법이 무너지고, 유니터리 표현이 직합이 아니라 직적분으로 분해되어 연속 스펙트럼이 나온다.
...
세 번째 항목에 따라 추상적으로 주어진 콤팩트 Lie 군을 언제나 행렬군으로 다룰 수 있다.

지표는 류함수 공간 $L^2(G)^G$ 의 정규직교기저를 이룬다. 유한군에서 기약표현의 개수가 켤레류의 개수와 같았던 사실이 여기에 대응한다.
...
콤팩트 군에서 스펙트럼이 전부 이산인 것은 $L^2(G)$ 에 작용하는 합성곱 작용소가 콤팩트 작용소여서 스펙트럼 정리가 적용되기 때문이다.
...
회전 대칭을 가진 Hamiltonian 의 고유공간은 $\mathrm{SO}(3)$ 의 표현이다. Peter–Weyl 분해가 상태공간을 각운동량 $\ell$ 별로 쪼개고 축퇴도 $2\ell+1$ 을 준다. 두 입자를 합칠 때의 Clebsch–Gordan 계수는 지표의 내적으로 계산된다.

## 극대 원환면과 Weyl 지표 공식

콤팩트 연결 Lie 군에서는 모든 원소가 극대 원환면 $T$ 의 켤레에 들어가므로 류함수가 $T$ 위의 Weyl 군 불변 함수로 결정된다. 지표 계산이 [근계](root-systems.md) 위의 유한합으로 내려오고 Weyl 지표 공식이 명시적인 답을 준다. $\mathrm{SU}(2)$ 에서는 $\chi_n(\theta)=\sin((n+1)\theta)/\sin\theta$ 다.
