...
축약사상이 아니면 수렴하는 반복 대신 수렴하는 부분열을 찾는다. 시간축을 폭 $h$ 로 나누고 각 칸에서 기울기를 상수로 고정한 Euler 꺾은선 $x_h$ 를 그리면, 꺾은선의 기울기는 어디서나 $\max\vert f\vert$ 이하다. 그러므로 $h$ 를 바꿔 얻은 꺾은선들은 모두 같은 상수로 Lipschitz 이고 같은 유계 영역에 들어간다. 이 두 성질이 Arzelà–Ascoli 정리의 가정이고, 정리는 균등수렴하는 부분열을 내준다. 그 극한이 해다.
