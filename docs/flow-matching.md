...
`(\alpha_t,\sigma_t)=(t,1-t)` 가 직선 보간이고, `(\sin\frac{\pi t}2,\cos\frac{\pi t}2)` 는 확산모형의 분산보존 스케줄에 해당한다. 어느 쪽이든 경계조건 `\alpha_0=\sigma_1=0`, `\alpha_1=\sigma_0=1` 만 맞으면 된다.

결합 `\pi` 는 보통 독립곱 `p_0\otimes p_1` 로 잡지만, 미니배치 안에서 비용행렬 `C_{ij}=\|x_0^{(i)}-x_1^{(j)}\|^2` 에 대한 최적 수송을 풀어 짝지을 수도 있다. 배치가 작으면 Hungarian 알고리즘, 크면 Sinkhorn 을 쓴다.
