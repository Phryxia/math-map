...
이 연산들이 닫힌 모임을 만들려면 조건의 부분순서를 완비 Boolean 대수로 채워 넣으면 된다. 그러면 $\varphi$ 에 그 대수의 원소 하나를 대응시킬 수 있고, 강제 관계는 그 원소가 $p$ 이상인지를 묻는 것이 된다.
...
각 공리마다 필요한 이름을 $V^B$ 안에서 짓는 것이 증명이다. 분리 공리는 논리식의 값으로 정의역의 값을 깎아 만들고, 선택공리는 $V$ 의 정렬을 이름 쪽으로 옮긴다. 양화사의 값이 무한 상한이 되므로 $B$ 의 완비성이 꼭 필요하다. ∎
...
두 접근은 같은 결과를 준다. 가산 추이 모형과 일반 필터를 잡는 쪽이 새 모형을 실제로 만드는 대신 외부 가정을 쓰고, Boolean 값 쪽은 모형을 만들지 않는 대신 진리값을 하나로 좁히지 않는다.

## 독립성 증명

**정리.** $B$ 를 $\mathrm{Add}(\omega,\aleph_2)$ 에 딸린 대수로 잡으면 $V^B$ 에서 연속체 가설의 값이 $0$ 이다.

값이 $0$ 이므로 그 부정의 값이 $1$ 이고, ZFC 의 공리가 모두 값 $1$ 이므로 ZFC 에서 연속체 가설을 증명할 수 없다. 모순을 이끌어내는 데 모형의 존재가 필요 없고 값 계산만 있으면 된다.

## 몫과 초곱

$B$ 의 극대 필터 $U$ 로 $V^B$ 를 나누면 두 값 모형이 되고, 이 조작이 초곱의 일반화다. $U$ 가 $V$ 안에 있으면 얻은 구조는 $V$ 자신과 기본 동치이고, $U$ 가 일반 필터일 때 강제 확대 $V\lbrack G\rbrack$ 이 나온다.
...
- 확률과의 대응을 준다. 측도 대수 위의 Boolean 값 모형이 무작위 실수 강제법에 대응하고, 값이 사건의 확률처럼 다루어진다.
