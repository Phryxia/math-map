...
다. 합이 $0$ 이 되려면 최솟값 $\min_i\lbrace v(a_i)+i\lambda\rbrace$ 가 **적어도 두 개의 $i$ 에서** 달성되어야 한다.
...
$(2,0)$ 은 $(1,1)$ 과 $(3,-1)$ 을 잇는 선분 위에 정확히 놓이므로 꼭짓점이 아니다. 다각형은 두 변이고, 근은 부치 $2$ 짜리 하나와 부치 $1$ 짜리 둘, 모두 세 개다. 차수와 맞는다.
...
변이 **하나뿐**이고 그 기울기가 $-1/n$ 이며 $n=\deg f$ 이면, 모든 근의 부치가 $1/n$ 이다. 부치군이 $\frac1n\mathbb Z$ 를 포함해야 하므로 $[K(\rho):K]\ge n$ 이고 곧 $f$ 가 기약이다. 이것이 **Eisenstein 판정**의 정체다. $a_0$ 이 $p$ 로 나뉘되 $p^2$ 로는 안 나뉘고 중간 계수가 전부 $p$ 로 나뉜다는 조건은, 정확히 "점 $(0,1)$ 과 $(n,0)$ 을 잇는 선분 위쪽에 나머지 점이 다 있다" 는 말이다.
...
> $f$ 의 **Newton 다각형**은 점집합 $\lbrace(i,v(a_i)):a_i\ne0\rbrace$ 의 아래쪽 볼록포다. 즉 $(0,v(a_0))$ 에서 $(n,v(a_n))$ 까지 이어지는, 모든 점을 위쪽 또는 위에 두는 아래로 볼록한 절선이다.
...
유한체 위 다양체의 zeta 함수 분자 $P(T)=\prod(1-\alpha_jT)$ 에서 근은 $\alpha_j^{-1}$ 이므로 다각형의 **기울기가 곧 $\mathrm{ord}\thinspace\alpha_j$ 와 같다**. 타원곡선의 $P(T)=1-a_pT+pT^2$ 에서 점은 $(0,0),(1,v(a_p)),(2,1)$ 이고 두 경우로 갈린다.
