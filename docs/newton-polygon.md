...
세 경우 모두 $(0,1)$ 과 $(n,0)$ 을 잇는 선분 하나이고 부치가 $1/n$ 이므로 분기지수가 $n$ 이며 확대가 완전분기다. 기약성이 볼록포의 모양에서 읽힌다.

## 보통과 초특이의 판정

[Dwork 문서](dwork-rationality.md)의 `count` 함수로 $a_p=p+1-\char35{}E(\mathbb F_p)$ 를 구해 넣는다. 곡선은 $E:y^2=x^3+x+1$ 이다.
...
for p, ap in [(5, -3), (7, 3), (11, -2), (13, -4), (17, 0), (19, -1), (23, -4)]:
    ...
    print(f"  p={p:3d}  a_p={ap:3d}   ord(α),ord(β) = {[str(v) for v in ords]}   {kind}")

#   p=  5  a_p= -3   ord(α),ord(β) = ['0', '1']   보통(ordinary)
#   p=  7  a_p=  3   ord(α),ord(β) = ['0', '1']   보통(ordinary)
#   p= 11  a_p= -2   ord(α),ord(β) = ['0', '1']   보통(ordinary)
#   p= 13  a_p= -4   ord(α),ord(β) = ['0', '1']   보통(ordinary)
#   p= 17  a_p=  0   ord(α),ord(β) = ['1/2', '1/2']   초특이(supersingular)
#   p= 19  a_p= -1   ord(α),ord(β) = ['0', '1']   보통(ordinary)
#   p= 23  a_p= -4   ord(α),ord(β) = ['0', '1']   보통(ordinary)
```

$p=17$ 에서만 $a_p=0$ 이라 다각형이 꺾이지 않고 부치가 $\frac12$ 로 쪼개지며 곡선이 초특이다. 나머지 소수에서는 단위근이 하나 있고 형식군의 높이가 $1$ 이다.
