...
쓰임이 넓다. 체 확대의 분기지수를 읽고(Eisenstein 이 변 하나짜리 다각형이다), $p$ 진 [멱급수](power-series.md)의 영점을 세고, [Dwork 의 이론](dwork-rationality.md)에서 Frobenius 고윳값의 $p$ 진 부치를 읽으며, [타원곡선](elliptic-curves.md)의 보통과 초특이 구분도 다각형으로 정리된다.
...
$(2,0)$ 은 $(1,1)$ 과 $(3,-1)$ 을 잇는 선분 위에 놓이므로 꼭짓점이 아니다. 아래쪽 볼록포는 $(0,3)$ 에서 $(1,1)$ 로 가는 기울기 $-2$ , 수평길이 $1$ 인 변과 $(1,1)$ 에서 $(3,-1)$ 로 가는 기울기 $-1$ , 수평길이 $2$ 인 변으로 이루어진다. 다각형은 두 변이고 근은 부치 $2$ 짜리 하나와 부치 $1$ 짜리 둘로 모두 셋이며 차수와 맞는다.
...
변 $S$ 의 기울기가 $-\lambda$ 이고 수평길이가 $m$ 일 때 $\lambda$ 가 $S$ 의 **부치**, $m$ 이 그 **중복도**다. 부호 관례가 문헌마다 다르고, 여기서는 기울기의 음수를 근의 부치로 잡는다.
