...
$L^p$ 공간은 측도공간 $(X,\mathcal M,\mu)$ 위에서 $\vert f\vert^p$ 가 적분가능한 가측함수들을 모아 만든 노름공간이다. $1\le p\le\infty$ 에서 노름은
...
$1\le p\lt\infty$ 에서 유한개의 값만 갖는 단순함수가 $L^p$ 에서 조밀하다. $X$ 가 국소 콤팩트 Hausdorff 공간이고 $\mu$ 가 Radon 측도이면 콤팩트 받침을 갖는 연속함수도 조밀하다. $p=\infty$ 에서는 둘 다 성립하지 않는다.
