...
Borel–Weil–Bott 정리는 깃발다양체 $G/B$ 위 직선다발의 [코호몰로지](cohomology.md)를 기약표현으로 계산한다.
...
- **기하학적 표현론.** 표현을 공간 위 층으로 실현하는 강령의 최초 사례이며, 기하학적 Satake 와 Beilinson–Bernstein 국소화가 이 길을 잇는다.
