...
Airy 는 음의 실축에서 가장 가까운 극점이 $[6/6]$ 에서 $-2.068$ , $[14/14]$ 에서 $-2.014$ 다. 급수 계수 서른 개만으로 두 안장점의 작용 차이 $A = 2$ 를 세 자리까지 복원했고, $\operatorname{Ai}$ 의 점근에 숨은 항의 크기가 $e^{-2\zeta}$ 임을 알아낸 것이다. $[14/14]$ 목록 맨 앞의 $+1.269$ 는 가짜다. 분자의 영점이 같은 자리에 $1.26921$ 로 소수점 다섯 자리까지 겹치고 낮은 차수에는 없던 것이다.
