...
이 대상이 특별한 이유는 **서로 무관해 보이는 세 가지 기술이 전부 같은 것을 준다**는 데 있다.
...
```mermaid
graph LR
  B["bialternant<br/>(행렬식의 비)"] -->|"대칭성·모듈러성이<br/>바로 보인다"| U["s_λ"]
  T["대각표 합"] -->|"계수가 음이 아님이<br/>바로 보인다"| U
  J["Jacobi–Trudi<br/>(h 의 행렬식)"] -->|"생성함수·대수적<br/>조작에 편하다"| U
```
...
세 정의를 각각 구현해 같은 값을 주는지 확인한다. 계산 경로가 완전히 달라(행렬식의 비, 대각표 열거, 다른 행렬식) 우연히 맞을 여지가 없다.
...
def det(M):
    """여인수 전개. 작은 행렬만 쓰므로 충분하다"""
    n = len(M)
    if n == 1: return M[0][0]
    return sum((-1) ** j * M[0][j] * det([r[:j] + r[j+1:] for r in M[1:]])
               for j in range(n))

def schur_bialternant(lam, x):
    """det(x_i^{lam_j+n-j}) / det(x_i^{n-j}). x 는 서로 달라야 한다"""
    n = len(x); lam = list(lam) + [0] * (n - len(lam))
    top = [[xi ** (lam[j] + n - 1 - j) for j in range(n)] for xi in x]
    bot = [[xi ** (n - 1 - j) for j in range(n)] for xi in x]
    return det(top) / det(bot)

def ssyt(lam, n, k=0, T=None):
    """모양 lam, 값 1..n 인 반표준 Young 대각표를 전부 생성"""
    T = {} if T is None else T
    cells = [(i, j) for i in range(len(lam)) for j in range(lam[i])]
    if k == len(cells):
        yield dict(T); return
    i, j = cells[k]
    lo = max([1] + ([T[i, j-1]] if j else []) + ([T[i-1, j] + 1] if i else []))
    for v in range(lo, n + 1):          # 행은 약증가, 열은 강증가
        T[i, j] = v
        yield from ssyt(lam, n, k + 1, T)
    T.pop((i, j), None)

def schur_tableaux(lam, x):
    total = 0
    for T in ssyt(lam, len(x)):
        m = 1
        for v in T.values(): m *= x[v - 1]
        total += m
    return total
...
def h(k, x):
    ...
    if k < 0: return 0
    if k == 0: return 1
    return sum(prod(c) for c in combinations_with_replacement(x, k))

def schur_jacobi_trudi(lam, x):
    r = len(lam)
    return det([[h(lam[i] - i + j, x) for j in range(r)] for i in range(r)])

for lam in [(2,1), (3,1), (2,2), (1,1,1), (3,2,1)]:
    for x in [(2, 3, 5), (F(1,2), 3, 7), (2, 5, 11)]:
        a, b, c = (schur_bialternant(lam, x), schur_tableaux(lam, x),
                   schur_jacobi_trudi(lam, x))
        assert a == b == c, (lam, x, a, b, c)
    print(f"λ={lam}  세 계산 일치,  SSYT 개수 = {schur_tableaux(lam, (1,1,1))}")
# λ=(2, 1)     세 계산 일치,  SSYT 개수 = 8
# λ=(3, 1)     세 계산 일치,  SSYT 개수 = 15
# λ=(2, 2)     세 계산 일치,  SSYT 개수 = 6
# λ=(1, 1, 1)  세 계산 일치,  SSYT 개수 = 1
