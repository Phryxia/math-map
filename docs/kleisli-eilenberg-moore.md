...
두 끝은 성격이 아주 다르다. Kleisli 는 "부수효과를 가진 함수" 들의 범주로 프로그래밍의 `bind` 와 do 표기가 사는 곳이고, Eilenberg–Moore 는 `T` 가 서술하는 대수 구조의 범주로 monoid, 군, 볼록집합 같은 것들이 사는 곳이다. 같은 monad 를 계산으로 읽을 것인가 대수로 읽을 것인가가 두 구성으로 갈린다.
...
`T` 를 낳는 분해가 주어지면, 자유대수는 어디에나 있어야 하므로 Kleisli 에서 그쪽으로 가는 functor 가 항상 있다. 반대로 임의의 분해에 등장하는 대상은 `T` 로 계산해 내는 구조를 가지므로 Eilenberg–Moore 로 보낼 수 있다. 최소와 최대가 여기서 나온다.
...
Eilenberg–Moore 범주는 "이 monad 가 서술하는 대수 구조가 무엇인가" 에 대한 답이다. 거꾸로 어떤 구체적 범주가 주어졌을 때 그것이 어떤 monad 의 대수 범주인지를 묻는 것이 monadicity 문제이고, 답이 예이면 그 범주의 성질 상당수가 자동으로 따라온다. 자유대상의 존재, 극한의 계산, 여핵의 구성이 전부 일반 정리로 내려온다.

분포 monad 의 대수가 볼록집합이고 초필터 monad 의 대수가 콤팩트 Hausdorff 공간이라는 사실은 "대수 구조" 의 범위가 흔히 생각하는 것보다 훨씬 넓음을 보여 준다.

## 효과의 합성과 monad 변환자

Kleisli 범주를 층층이 쌓아 여러 효과를 조합하는 것이 monad 변환자다. 두 monad 의 합성이 일반적으로 monad 가 아니라는 사실 때문에 필요한 장치이며, 언제 합성이 가능한지를 말해 주는 것이 분배 법칙이다. 분배 법칙은 한쪽 monad 를 다른 쪽의 Eilenberg–Moore 범주로 들어 올리는 것과 같은 데이터다.

## 의미론

부분성, 비결정성, 확률, 상태, 입출력 같은 계산 효과를 monad 로 모형화하면 프로그램의 의미가 Kleisli 사상이 된다. 서로 다른 효과를 같은 틀에서 비교할 수 있게 되고, 프로그램 변환의 정당성이 범주적 등식으로 환원된다. 이것이 Moggi 가 제시한 계산의 monad 의미론이며, 함수형 언어 설계에 직접 반영되었다.
