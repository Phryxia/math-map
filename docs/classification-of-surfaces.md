...
토러스를 `T`, 사영평면을 `P`로 쓰고 `g`개의 연결합을 다음과 같이 줄여 쓴다.
...
6. **손잡이 모으기.** 두 반대 방향 쌍이 서로 얽혀 나타나면(문자 순서가 `a`, `b`, `a`의 역, `b`의 역) 두 번의 자르고 붙이기로 교환자 형태 `abAB`로 모은다.
...
원판, 원환(annulus), Möbius 띠가 각각 `(g,b)=(0,1)`, `(0,2)`, `(k,b)=(1,1)`에 해당한다.
