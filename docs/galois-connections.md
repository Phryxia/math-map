...
두 부분순서 집합 `P,Q` 와 사상 $f:P\to Q$ 와 $g:Q\to P$ 를 잡는다. 단조 판본의 조건은
...
동치인 서술은 이렇다. `f,g` 가 모두 단조이고 $p\le g(f(p))$ 와 $f(g(q))\le q$ 가 성립한다.
...
`f` 에 대해 오른쪽 짝 `g,g'` 가 둘 다 있으면 `g=g'` 다. 정의에서 $p\le g(q)\iff f(p)\le q\iff p\le g'(q)$ 이고, 부분순서에서 모든 `p` 에 대해 $p\le x\iff p\le y$ 라는 것은 `x=y` 를 준다. 그러므로 "왼쪽 짝을 갖는다" 는 성질이지 추가 구조가 아니다.
