...
| `V` | `A(V)` | 기약가군 |
|---|---|---|
| Virasoro `L(c,0)`, 일반 `c` | $\mathbb C[x]$ | `x` 의 값 `h` 마다 하나, 무한개 |
...
가장 작은 비자명 예인 격자 VOA `V_L`, $L=\sqrt2\,\mathbb Z$ (곧 `A_1` 근격자, 레벨 1 아핀 $\mathfrak{sl}_2$) 를 본다. $L^*/L\cong\mathbb Z/2$ 이므로 기약가군이 둘이고 지표는
...
이 예에서 `S` 행렬이 `L^*/L` 의 Fourier 변환이라는 것도 구조적으로 읽힌다. Zhu 대수가 $\mathbb C[L^*/L]$ 이었고, 기약가군이 그 군의 지표로 색인되며, 지표들 사이의 모듈러 변환이 바로 그 유한 아벨군의 Fourier 변환이다. 대수 쪽 분류와 해석 쪽 변환이 같은 유한군에서 나온다.
