...
$n$ 을 $n=D\,m^2$ 으로 쪼개자($D$ 는 제곱인수가 없는 부분). 제곱수를 곱하는 조작은 $D$ 를 바꾸지 않지만 $p$ 를 한 번 곱하는 조작은 $D$ 를 바꾼다. 곧 $n\mapsto np$ 는 서로 다른 판별식 세계를 뒤섞는다. 반면 $n\mapsto np^2$ 는 $D$ 를 고정한 채 $m$ 만 움직인다.
