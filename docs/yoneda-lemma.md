...
[범주](category.md)론에서 대상의 정체는 그 안의 원소가 아니라 그 대상으로 들어오는 사상 전체로 결정된다.
...
보편 성질로 정의한 대상의 유일성 증명, 표현가능 [함자](functors.md)의 판정, 대수기하의 함자적 관점이 이 정리의 응용이다.
...
poset 범주에서 $h_A$ 는 $A$ 이하인 원소들의 집합이고, Yoneda 매장은 원소를 그 아래집합으로 보내는 사상이다. 순서를 아래집합의 포함으로 복원할 수 있다는 것이 이 범주에서의 Yoneda lemma 이고, [격자](order-lattices.md) 이론의 Dedekind–MacNeille 완비화가 같은 구성이다.
...
- **계산의 축약.** 자연변환 전체 대신 한 대상의 한 원소만 보면 된다. [코호몰로지](cohomology.md) 연산의 분류, 자유 대상의 구성, [수반](adjunctions.md)의 존재 판정이 이 축약을 쓰고, 수반 함자 정리도 각 대상마다 어떤 함자가 표현가능한지를 묻는 형태로 서술된다.
