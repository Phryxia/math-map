...
$A$ 의 각 열은 해당 입력 기저 벡터의 상의 좌표다. 기저를 바꾸면 행렬은 $A \mapsto P^{-1}AP$ 꼴로 달라지고 사상 자체는 그대로다. 행렬의 성질 가운데 기저 변환에 불변인 것(계수, [행렬식](determinants.md), 고윳값, 대각합)만이 사상의 성질이다.
...
미분 연산, [적분](riemann-integral.md), 기댓값, 이산 Fourier 변환은 선형이다. 제곱, 절댓값, 정규화, 최댓값 연산은 선형이 아니다. 선형인 연산에서만 기저 하나로 계산이 끝나고 중첩 원리를 쓴다.
