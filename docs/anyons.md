...
Freedman, Larsen, Wang 이 Fibonacci 애니온의 땋임 표현의 상이 $SU(V)$ 에서 **조밀**함을 증명했다. 따라서 임의의 유니터리를 원하는 정확도로 근사할 수 있고, 땋기만으로 보편 양자계산이 된다. 증명은 상이 되는 부분군이 닫힌 리 부분군을 생성함을 보이고 그 리 대수가 전체임을 확인하는 방식이다.
