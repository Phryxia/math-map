...
두 다양체의 연결합 $M_1\# M_2$ 를 생각하자. 양쪽 모두 쌍곡이라 해도 합에는 쌍곡 계량이 들어가지 않는다. 연결합에는 $M_1$ 과 $M_2$ 를 가르는 2 구면이 있고, 그 구면은 어느 쪽으로도 오그라들지 않는다. 그런데 완비 쌍곡 다양체의 보편덮개는 $\mathbb H^3$ 이라 $\pi_2=0$ 이다. 오그라들지 않는 구면이 있을 수 없다.
