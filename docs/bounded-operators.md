...
유계 작용소는 전 공간에서 정의된다. 미분 작용소 같은 비유계 작용소는 조밀한 부분공간에서만 정의되며 정의역을 어떻게 잡느냐가 이론의 일부가 된다.
...
*증명.* $\lVert T\rVert$ 또는 $-\lVert T\rVert$ 가 고유값이다. $\langle Tx,x\rangle$ 를 단위구에서 최대화하는 수열을 잡으면 [콤팩트성](compactness.md)이 부분수열의 수렴을 보장해 최대점이 존재하고 그것이 고유벡터가 된다. 그 고유벡터의 직교여공간으로 제한해 반복한다. 크기가 $\epsilon$ 이상인 고유값이 무한히 많으면 대응 고유벡터들이 서로 $\sqrt2\epsilon$ 이상 떨어져 수렴 부분수열을 가질 수 없으므로 고유값은 0 으로 수렴한다.
