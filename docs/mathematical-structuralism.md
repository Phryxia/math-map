...
수 $2$ 는 무엇인가. 집합론으로 자연수를 만들 때 von Neumann 은 $2 = \{\emptyset, \{\emptyset\}\}$ 라 하고 Zermelo 는 $2 = \{\{\emptyset\}\}$ 라 한다. 두 정의 모두 산술 전체를 재현하지만 서로 다른 집합이다. 그렇다면 $2 \in 3$ 이라는 물음은 참인가. 한쪽에서는 참이고 다른 쪽에서는 거짓이다.
...
자연수 구조에서 $2$ 는 유격수 같은 자리다. $\{\emptyset, \{\emptyset\}\}$ 는 그 자리를 채운 선수 중 하나일 뿐이다. "유격수의 키는 몇인가" 가 자리 자체에 대한 물음으로는 답이 없듯이, "$2$ 의 원소가 몇 개인가" 도 자리에 대한 물음으로는 답이 없다.
