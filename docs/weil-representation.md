...
[theta 급수](theta-series.md)의 모듈러성은 Poisson 합 공식으로 $\theta(-1/\tau)=\sqrt{-i\tau}\thinspace\theta(\tau)$ 를 계산해 얻을 수도 있고, 이 변환을 일으키는 [군의 표현](group-representations.md)에서 얻을 수도 있다. 그 표현이 **Weil 표현**이다.
