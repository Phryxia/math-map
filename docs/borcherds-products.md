...
구성의 장치는 **특이 theta 올림**이다. 약정칙 형식과 [theta 급수](theta-series.md)를 곱해 [모듈러 곡선](modular-curves.md) 위에서 적분하면 $f$ 의 극 때문에 발산하고, Harvey–Moore 식으로 정규화하면 특이점을 갖는 유한한 값이 나온다. 특이점이 놓인 자리가 **Heegner 인자**이며 지수를 취하면 무한곱이 나온다.
...
괴물 Lie 대수의 분모 공식이 Borcherds 곱이라는 사실이 [괴물 달빛 추측](monstrous-moonshine.md) 증명의 마지막 조각이었다. 무한곱의 지수가 $j$ 의 계수이므로 곱을 전개해 얻은 항등식이 [Hecke 작용소](hecke-operators.md)가 주는 재귀식과 같아진다. 그 재귀식이 계수를 유일하게 결정하므로 괴물군의 지표와 일치함이 확인된다.[^1]
