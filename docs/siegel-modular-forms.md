...
차원 $g$ 에서 $\tau$ 는 $g\times g$ 대칭행렬이고 격자 기저를 바꾸는 군은 $\mathrm{Sp}\_{2g}(\mathbb Z)$ 다. 작용이 $(A\tau+B)(C\tau+D)^{-1}$ 이므로 $(c\tau+d)$ 의 자리에 $g\times g$ 행렬 $C\tau+D$ 가 온다. 이것을 수로 만들려면 [행렬식](determinants.md)을 취한다. 붙는 인수가 $\det(C\tau+D)^k$ 이고, 이 인수를 달고 변하는 정칙함수를 모은 것이 가중치 $k$ 의 공간이다.
