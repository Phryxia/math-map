...
채색수는 "충돌하는 자원을 몇 묶음으로 나눌 수 있는가"를 재는 양이므로 시간표 작성, 레지스터 할당, 주파수 배정 같은 실제 문제가 모두 색칠 문제로 번역된다. 동시에 채색수를 계산하는 것은 [NP-완전](np-completeness.md)(nondeterministic polynomial time) 문제이고, 근사조차 어렵다. 하한(클리크 수)과 상한(최대차수 + 1) 사이의 간격에서 그래프 이론의 여러 질문이 나왔고, [평면 그래프](planar-graphs.md)에 대한 4색 정리는 조합론에서 가장 유명한 정리 중 하나다.

# 직관

색칠은 정점 집합의 분할이다. 정점을 $k$ 개의 색으로 칠한다는 것은 정점 집합을 $k$ 개의 독립집합(서로 인접하지 않은 정점들의 모임)으로 쪼갠다는 뜻이다. 따라서 채색수는 "정점 집합을 덮는 데 필요한 독립집합의 최소 개수"다.
...
이 두 경계는 일반적으로 멀리 떨어져 있다. 클리크가 전혀 없으면서도 채색수가 큰 그래프가 존재한다는 사실(Mycielski 구성, 그리고 [Ramsey 이론](ramsey-theory.md)적 구성)이 이 주제를 어렵게 만든다. 색칠은 간선 하나의 제약만 보고 채색수를 알아내는 문제이고, 국소 조건과 전역 양의 간격이 난이도를 만든다.
...
$P(G, k)$ 를 $G$ 의 적절한 $k$ 색칠의 개수로 정의한다. 이는 아래 성질 절에서 보듯 $k$ 에 대한 다항식이며, $\chi(G)$ 는 $P(G, k) \gt 0$ 이 되는 최소의 양의 정수다.
...
이다. 이 부등식은 Ramsey 이론이나 [확률적 방법](probabilistic-method.md)에서 독립수 상한을 채색수 하한으로 바꿔 쓸 때 반복해서 등장한다.
...
- **스케줄링과 매칭**: 이분 그래프의 간선 색칠은 작업-기계 배정을 시간 단위로 쪼개는 문제이며, König 정리로 $\Delta$ 개 시간대로 충분하다. 이 결과는 매칭과 Hall 정리의 완전 매칭 분해로 증명된다.
