...
주의할 점은 이 열거가 **필요조건의 열거**라는 것이다. 두 조건을 만족하는 조합이 23 개라는 것과 각 조합이 실제로 Niemeier 격자를 하나씩 준다는 것은 다른 문제이고, 후자는 격자를 실제로 구성해야 한다. 여기서 확인한 것은 목록의 크기가 우연이 아니라 초등적인 조합 제약에서 나온다는 사실이다.
...
근계가 잘게 쪼개져 있을수록 성분을 뒤섞을 여지가 많아 군이 크다. `A_1^{24}` 에서 성분이 24 개로 최대이고 군도 `M_{24}` 로 가장 크다. 반대쪽 끝 `D_{24}` 는 성분이 하나뿐이라 뒤섞을 것이 없고 군이 자명하다. `M_{24}` 사례가 먼저 발견된 것은 우연이 아니라 가장 풍부한 사례이기 때문이다.
...
곧 **가군이 있다는 것은 알지만 그 가군이 무엇인지는 모른다.** 괴물 달빛에서 `V^\natural` 이 주어졌던 것과 결정적으로 다른 지점이고, umbral moonshine 이 여전히 미해결로 불리는 이유다.

## 왜 K3 시그마 모형으로 설명되지 않는가
