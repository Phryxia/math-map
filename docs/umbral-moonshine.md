...
$N=4$ 초등각대수의 지표 가운데 **짧은**(BPS) 것은 그 자체로 모듈러 변환이 닫히지 않고, 긴 지표들과 섞여야 전체가 모듈러가 된다. 타원 종수 전체는 Jacobi 형식이지만 짧은 조각과 긴 조각으로 가르면 각 조각의 모듈러성이 깨지고, 그 자리에서 mock 이 나온다. 괴물 달빛에는 이 분해가 없어 모듈러 함수만 나온다.
