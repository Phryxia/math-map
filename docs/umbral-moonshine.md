...
이 23 겹 짝짓기가 **umbral moonshine** 이다. 이름은 그림자를 뜻하는 라틴어 *umbra* 에서 왔다. mock 모듈러 형식에는 **shadow** 라 불리는 진짜 모듈러 형식이 딸려 있고, 각 사례에서 그 shadow 가 해당 Niemeier 격자의 근계로 만들어진 theta 급수이기 때문이다.

# 직관

## mock 모듈러 형식과 shadow

정칙함수이면서 모듈러 변환을 만족하라는 조건은 강해서, 주어진 무게와 지표에서 해가 유한차원 공간뿐이다. Ramanujan 의 mock theta 함수는 그 조건을 만족하지 않으면서 모듈러 형식에 가까운 변환을 보였고, 80 년 동안 그 관계가 정식화되지 않았다.
...
$\widehat H$ 는 모듈러이지만 정칙이 아니고, $H$ 는 정칙이지만 모듈러가 아니다. 정칙성과 모듈러성 가운데 하나만 성립하고, 그 차이를 재는 양이 shadow 다. $g=0$ 이면 보통 모듈러 형식으로 돌아온다.

$N=4$ 초등각대수의 지표 가운데 **짧은**(Bogomolny–Prasad–Sommerfield, BPS) 것은 그 자체로 모듈러 변환이 닫히지 않고, 긴 지표들과 섞여야 전체가 모듈러가 된다. 타원 종수 전체는 Jacobi 형식이지만 짧은 조각과 긴 조각으로 가르면 각 조각의 모듈러성이 깨지고, 그 자리에서 mock 이 나온다. 괴물 달빛에는 이 분해가 없어 모듈러 함수만 나온다.

## 색인 구조

괴물 달빛은 군 하나와 그 군의 원소마다 붙는 함수 하나로 이루어진다. umbral moonshine 에는 격자를 고르는 단계가 하나 더 있다.
...
이 근계들에는 눈에 띄는 제약이 있다. 기약 성분들의 Coxeter 수가 전부 같다. 이 제약과 랭크의 합이 $24$ 라는 조건만으로 가능한 조합을 세면 답이 $23$ 개가 되고, 열거로 확인된다.
