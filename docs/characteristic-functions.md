...
특성함수는 확률[측도](measure.md)를 결정하고, 독립합을 곱으로 바꾸며, 약수렴을 각점 수렴으로 번역한다. [확률변수](random-variables.md)의 해석적 도구로 널리 쓰이는 근거가 이 세 성질이다. 적률생성함수는 같은 일을 하되 존재성에서 제약을 받는다.
...
분포의 합성곱이 특성함수의 곱이 되므로 합의 분포 문제가 곱셈 문제가 된다. 독립 정규분포의 합이 정규분포인 것은 $\exp$ 안의 지수를 더하면 나온다.
