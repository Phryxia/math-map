...
따라서 $\mathbb Z[i]$ 는 Euclid 정역이고, 유클리드 정역(Euclidean domain, ED) $\subset$ 주아이디얼정역(principal ideal domain, PID) $\subset$ 유일분해정역(unique factorization domain, UFD) 사슬에 의해 유일분해정역이다.
