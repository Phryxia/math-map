...
높은 준위의 계수는 $\rho^{|S|}$ 로 빠르게 작아진다. 안정성이 크다는 것은 무게가 낮은 준위에 있다는 것이고, $\mathrm{Stab}\_\rho=\rho$ 를 달성하는 것은 준위 $1$ 에 무게를 몰아 준 독재자뿐이다. 검사하려는 성질이 부등식의 등호 조건으로 나타나므로 독재자 검사가 가능하다.
...
- **최적 근사비.** 유일게임 추측 아래에서 최대 절단의 근사 임계가 $\alpha_{\mathrm{GW}}=0.878\ldots$ 로 확정된다. 같은 틀이 최대 $k$ 절단과 여러 제약 만족 문제에 적용되어, 반정부호 완화의 값이 최적 근사비라는 Raghavendra 의 정리로 이어진다.
