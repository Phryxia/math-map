...
수렴의 정의는 극한 $p$ 를 미리 지정해야 쓸 수 있다. 해석학에서는 근삿값의 열을 먼저 만들고 그 극한의 존재를 증명해야 하는 일이 잦다. Cauchy 조건은 극한을 언급하지 않고 항들끼리 가까워진다는 것만 요구하며, Cauchy 수열이 항상 수렴하는 공간이 완비 공간이다. 정의에는 [거리 공간](metric-spaces.md)의 언어만 필요하다.

# 직관

## 구멍

$(0,1]$ 에서 $x_n=1/n$ 은 항들끼리 한없이 가까워지지만 극한 $0$ 이 공간에 없다. 유리수에서 $\sqrt{2}$ 의 소수 전개를 자른 수열도 같다. 완비성은 이런 구멍이 없다는 성질이다.
...
- Cauchy 수열이 수렴하는 부분수열을 가지면 전체가 그 극한으로 수렴한다. 컴팩트 거리 공간이 완비인 것이 이 성질의 따름이다.
...
$\mathbb{R}$ 은 완비지만 유계가 아니고, $(0,1]$ 은 유계지만 완비가 아니다. 거리 공간에서 컴팩트인 것과 완비이면서 전유계인 것이 동치이며, 이 분해가 [컴팩트성](compactness.md)의 표준적인 다룸이다.
...
- 같은 집합에 준 두 거리가 위상동치이기만 하면 완비성을 공유하지 않는다. 강동치인 두 거리는 공유한다. 반복법의 수렴을 논할 때 어떤 거리에서 Cauchy 인지 명시해야 한다.[^1]
