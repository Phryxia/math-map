...
$V(x) = \lvert x\rvert$ 의 바닥 준위는 WKB 근사로 $0.8853$ 이고 정확값은 $0.8086$ 이다. 이 정확값은 $\mathrm{Ai}$ 의 도함수의 첫 영점에서 나온다. $\hbar = m = 1$ 에서 방정식은 $y'' = 2(\lvert x\rvert - E)y$ 이고, $x\gt 0$ 에서 $u = 2^{1/3}(x - E)$ 로 두면 그대로 $y_{uu} = u\thinspace y$ 다. 무한대에서 감쇠해야 하므로 해는 $\mathrm{Ai}$ 뿐이고, 원점에서의 대칭성이 조건을 준다.
