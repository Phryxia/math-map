...
- 두 영역을 잇는다. $\mu\gt 0$ 을 고정하고 $\lambda\to\infty$ 로 보내면 $\mathrm{Ai}(-\mu\lambda^{2/3})$ 의 진동 점근이 $\lambda^{-1/6}$ 를 내놓아 전체가 $\lambda^{-1/3}\cdot\lambda^{-1/6}=\lambda^{-1/2}$ 로 돌아간다. $\mu\lt 0$ 이면 감쇠 점근이 지수적으로 작은 값을 준다. 곧 $\mathrm{Ai}$ 한 함수가 **진동 영역과 지수 영역, 그리고 그 사이의 전이층**을 하나의 식으로 덮는다.
...
[WKB 근사](wkb-approximation.md)는 $V(x) = \lvert x\rvert$ 의 바닥 준위를 $0.8853$ 으로 주고 정확값이 $0.8086$ 이라고만 적었다. 그 정확값이 어디서 오는지가 여기서 보인다. $\hbar = m = 1$ 에서 방정식은 $y'' = 2(\lvert x\rvert - E)y$ 이고, $x\gt 0$ 에서 $u = 2^{1/3}(x - E)$ 로 두면 그대로 $y_{uu} = u\thinspace y$ 다. 무한대에서 감쇠해야 하므로 해는 $\mathrm{Ai}$ 뿐이고, 원점에서의 대칭성이 조건을 준다.
