...
는 계수가 가장 단순하게 부호를 바꾸는 2계 [상미분방정식](ordinary-differential-equations.md)이다. $z \gt 0$ 에서는 해가 지수적으로 감쇠하거나 증가하고, $z \lt 0$ 에서는 진동한다. 부호가 바뀌는 자리 $z = 0$ 이 **회전점**이고, 이 방정식은 회전점 하나를 가진 문제의 **표준형**이다. 계수가 단순 영점을 가지는 임의의 방정식은 회전점 근방에서 좌표를 다시 잡으면 이 방정식이 된다.

해 $\mathrm{Ai}$ 와 $\mathrm{Bi}$ 를 **Airy 함수**라 한다. 이들이 특별한 이유는 두 가지다. 첫째, 적분 표현의 안장점이 정확히 두 개이고 $z = 0$ 에서 그 둘이 합류한다. 둘째, 그 배치가 [Stokes 현상](stokes-phenomenon.md)이 일어나는 가장 작은 예다. 지수적으로 작은 항이 켜지고 꺼지는 전 과정을 초등적인 식 하나로 끝까지 따라갈 수 있고, 그래서 [WKB 근사](wkb-approximation.md)(Wentzel–Kramers–Brillouin)의 연결 공식이 이 함수의 점근에서 그대로 읽힌다. 연결 공식에 나타나는 $\pi/4$ 는 $\mathrm{Ai}(-x)$ 의 진동 점근에 들어 있는 상수가 옮겨 간 것이다.
...
다. $z$ 가 크면 두 안장점이 멀리 떨어져 하나가 압도적으로 지배하고, 점근급수는 그 하나만 본다. $z\to 0$ 에서 두 안장점이 원점으로 합류하면 어느 쪽도 지배하지 않고, 두 안장점의 합류가 회전점에서 근사가 깨지는 이유다.

$z$ 가 큰 양수이면 한쪽 안장점이 지배해 해가 지수적으로 감쇠하고, 큰 음수이면 두 기여의 크기가 같아 간섭이 진동을 만든다.
...
여기서 $\Gamma$ 는 [감마 함수](gamma-function.md)다. 수치로는 $\mathrm{Ai}(0) = 0.355028\ldots$ 이고 $\mathrm{Ai}'(0) = -0.258819\ldots$ 다. Taylor 급수는 $y'' = zy$ 를 계수별로 풀면 바로 나온다. $f(z) = \sum_k \frac{z^{3k}}{(3k)!}\prod_{j\lt k}(3j+1)$ 과 $g(z) = \sum_k \frac{z^{3k+1}}{(3k+1)!}\prod_{j\lt k}(3j+2)$ 가 각각 $y(0)=1,y'(0)=0$ 과 $y(0)=0,y'(0)=1$ 인 해이고
...
점근급수 계수 $u_k$ 는 $k$ 에 대해 계승 규모로 커진다. 점화식에서 $u_k/u_{k-1} \to k/2$ 이므로 $u_k$ 는 $\Gamma(k)/2^k$ 규모이고, $\zeta^k$ 로 나눈 항이 최소가 되는 지점은 $u_k/u_{k-1} = \zeta$ 를 만족하는 곳, 즉 $k \approx 2\zeta$ 부근이다. 거기서 잘랐을 때 남는 오차는 $e^{-2\zeta}$ 규모이며, $2\zeta = \tfrac43 z^{3/2} = A$ 이므로 이는 정확히 두 안장점의 크기 비 $e^{+\zeta}/e^{-\zeta}$ 다. 한 급수의 발산 속도가 보이지 않는 다른 해의 크기를 알려 준다는 resurgence 의 표본이다. $\zeta$ 가 클수록 최적 절단점이 멀어지고 도달 가능한 정밀도가 지수적으로 좋아지므로, $x$ 가 커질수록 점근급수가 정확해진다.
...
를 얻는다. 파수 $\sqrt{\lvert x\rvert}$ 가 커지므로 영점 사이의 간격이 $\lvert a_n\rvert^{-1/2}$ 로 줄어든다. 이 점근은 $n=1$ 에서 이미 1 퍼센트 이내이고 $n$ 이 커지면 오차가 빠르게 작아진다.
...
- 두 영역을 잇는다. $\mu\gt 0$ 을 고정하고 $\lambda\to\infty$ 로 보내면 $\mathrm{Ai}(-\mu\lambda^{2/3})$ 의 진동 점근이 $\lambda^{-1/6}$ 를 내놓아 전체가 $\lambda^{-1/3}\cdot\lambda^{-1/6}=\lambda^{-1/2}$ 로 돌아간다. $\mu\lt 0$ 이면 감쇠 점근이 지수적으로 작은 값을 준다. 곧 $\mathrm{Ai}$ 한 함수가 진동 영역과 지수 영역, 그리고 그 사이의 전이층을 하나의 식으로 덮는다.
...
급수, 점근급수, 적분 표현은 담당하는 $x$ 의 범위가 다르다.

작은 $x$ 에서는 수렴급수를 쓴다. 점근급수의 최적 절단항은 $k \approx 2\zeta$ 이고 $\zeta = \tfrac23 x^{3/2}$ 이므로, $x$ 가 작으면 절단항이 1 보다 작아 더할 항이 없다.

큰 $x$ 에서는 점근급수를 쓴다. 수렴급수의 최대항은 $\mathrm{Ai}(x)$ 보다 훨씬 크고 그 비가 $x$ 와 함께 커지므로, 고정된 정밀도로 더하면 상쇄가 유효숫자를 먹는다.

적분 표현 $\int_0^T \cos(t^3/3 + xt)\thinspace dt$ 는 자른 자리의 꼬리가 남는다. $t$ 가 클 때 피적분함수가 $\cos(t^3/3)$ 처럼 빠르게 진동하므로 부분적분으로 꼬리가 $1/(\pi T^2)$ 규모이고, 이 값이 $\mathrm{Ai}(x)$ 의 지수적 감쇠보다 크면 절단오차가 답을 덮는다. 경로를 안장점을 지나는 방향으로 돌려 피적분함수를 감쇠시키면 꼬리가 지수적으로 줄어든다.

수렴하는 급수가 큰 $x$ 에서 쓸 수 없고 발산하는 급수가 정확해지는 역전이 점근해석이 다루는 현상이다.

## 선형 퍼텐셜의 정확한 준위

[WKB 근사](wkb-approximation.md)는 $V(x) = \lvert x\rvert$ 의 바닥 준위를 $0.8853$ 으로 주고 정확값이 $0.8086$ 이라고만 적었다. 그 정확값은 $\mathrm{Ai}$ 의 도함수의 첫 영점에서 나온다. $\hbar = m = 1$ 에서 방정식은 $y'' = 2(\lvert x\rvert - E)y$ 이고, $x\gt 0$ 에서 $u = 2^{1/3}(x - E)$ 로 두면 그대로 $y_{uu} = u\thinspace y$ 다. 무한대에서 감쇠해야 하므로 해는 $\mathrm{Ai}$ 뿐이고, 원점에서의 대칭성이 조건을 준다.
...
$a_n$ 은 $\mathrm{Ai}$ 의 영점, $a'\_n$ 은 $\mathrm{Ai}'$ 의 영점이다. WKB 가 9% 어긋난 바닥 준위가 Airy 함수의 도함수 첫 영점으로 정확히 결정되고, 준위가 올라갈수록 영점 점근이 좋아지면서 두 값이 만난다.

## 초점과 무지개

Airy 는 1838년 무지개의 **과잉 아치**(supernumerary arc)를 설명하려고 이 적분을 도입했다. 기하광학은 무지개 각도에서 세기가 발산한다고 말하지만 실제로는 유한하고, 안쪽에 옅은 띠가 여러 줄 따라붙는다. 광선 두 개가 합류하는 **초곡선**(caustic) 근방에서 위상차가 편각의 3차식이 되고, 그 적분이 정확히 $\int\cos(t^3/3 + xt)\thinspace dt$ 이기 때문이다. 밝은 쪽의 진동이 과잉 아치이고 어두운 쪽의 지수적 꼬리는 기하광학이 그림자로 예측한 영역에 남는 빛이다.

초곡선 회절, 직선 모서리 근처의 빛의 세기, 전리층에서 되돌아오는 전파, 균일한 힘을 받는 입자의 파동함수가 모두 같은 형태다. "두 해가 합류하는 자리" 라는 조건만 같으면 세부 사정과 무관하게 Airy 함수가 나온다.[^1]
