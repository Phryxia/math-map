...
$0$ 에 질량 $1$ 을 몰아 준 Dirac 측도는 길이측도에 대해 밀도를 갖지 않는다. $\lbrace 0\rbrace$ 의 길이가 $0$ 인데 측도가 $1$ 이기 때문이다. "$0$ 에서만 무한대이고 적분하면 $1$ 인 함수" 는 존재하지 않는다. 물리학에서 델타 함수라 부르는 것은 함수가 아니며, 이것이 초함수 이론이 필요한 이유다.
...
증명은 $\lbrace g \ge 0 : \int_E g \thinspace d\mu \le \nu(E) \text{ for all } E\rbrace$ 라는 함수족을 놓고 그 상한을 취하는 방식이 표준이다. 상한이 실제로 밀도가 됨을 보이는 데 [단조수렴 정리](monotone-convergence.md)가 쓰인다. Hilbert 공간의 사영을 이용한 von Neumann 의 증명도 널리 쓰이며, 조건부 기댓값이 사영이라는 관점과 자연스럽게 이어진다.
...
부분 $\sigma$ -대수 $\mathcal G \subseteq \mathcal F$ 가 주어졌을 때 $E[X \mid \mathcal G]$ 는 $\mathcal G$ 위에서 정의된 측도 $A \mapsto \int_A X \thinspace dP$ 의 $P\rvert_{\mathcal G}$ 에 대한 Radon–Nikodym 도함수다. 조건부 기댓값이 존재하고 거의 확실히 유일하다는 사실이 이 정리에서 곧바로 나온다.
