...
Radon–Nikodym 정리는 이 나눗셈을 정당화한다. 두 측도 사이에 "`μ` 가 무시하는 집합은 `ν` 도 무시한다" 는 관계만 성립하면, `ν` 를 `μ` 로 적분해 복원하는 함수가 반드시 존재한다고 말한다. 미적분학의 기본 정리가 누적량과 변화율을 이었다면, 이 정리는 측도와 그 밀도를 잇는 측도론판이다.
...
밀도가 있다는 것은 `ν` 가 `μ` 를 저울로 삼아 각 지점의 무게를 다시 매긴 것이라는 뜻이다. 그러면 `μ` 가 무게 `0` 이라고 판정한 자리에서 `ν` 도 `0` 이어야 한다. 아무리 큰 밀도를 곱해도 `0` 은 `0` 이기 때문이다.
...
같은 가측 공간 위의 측도 `μ` 와 `ν` 에 대해 `μ(E) = 0` 이면 항상 `ν(E) = 0` 일 때 `ν` 가 `μ` 에 대해 절대연속이라 하고

$$
\nu\ll\mu
$$

로 쓴다. `ν` 가 유한하면 이 조건은 "임의의 `ε > 0` 에 대해 `δ > 0` 이 있어 `μ(E) < δ` 이면 `ν(E) < ε`" 와 동치이며, 이 형태가 절대연속이라는 이름의 유래다.

## 정리

`μ` 와 `ν` 가 `σ`-유한이고 `ν ≪ μ` 이면 음이 아닌 가측함수 `f` 가 존재해 모든 가측집합 `E` 에서

$$
\nu(E)=\int_E f\,d\mu
$$

가 성립한다. `f` 는 `μ`-거의 모든 곳에서 유일하며 Radon–Nikodym 도함수라 하고
...
`σ`-유한성은 뺄 수 없다. `ℝ` 의 Borel 집합에 `μ` 를 셈측도, `ν` 를 Lebesgue 측도로 두면 `ν ≪ μ` 이지만 밀도가 존재하지 않는다. 셈측도가 `σ`-유한이 아니기 때문이다.

# 성질

## 필요조건과 충분조건

필요조건은 즉시 확인된다. `μ(E) = 0` 이면 `E` 위의 임의의 적분이 `0` 이므로 `ν(E) = 0` 이다. 정리의 내용은 `σ`-유한성 아래에서 이것이 충분하다는 것이다.

증명은 `{g ≥ 0 : ∫_E g dμ ≤ ν(E)\ ∀E}` 라는 함수족을 놓고 그 상한을 취하는 방식이 표준이다. 상한이 실제로 밀도가 됨을 보이는 데 [단조수렴 정리](monotone-convergence.md)가 쓰인다. Hilbert 공간의 사영을 이용한 von Neumann 의 증명도 널리 쓰이며, 조건부 기댓값이 사영이라는 관점과 자연스럽게 이어진다.
...
두 번째가 연쇄법칙이고 `ν ≪ μ ≪ λ` 일 때 거의 모든 곳에서 성립한다. `ν ≪ μ` 이고 `μ ≪ ν` 이면 두 측도가 동치이고
...
로 분해된다. `ν_s ⊥ μ` 는 두 측도가 서로소인 집합 위에 실려 있다는 뜻이다. 실수 위의 확률분포를 연속 부분, 이산 부분, 특이연속 부분으로 나누는 고전적 분류가 이 정리의 특수한 경우다.
...
밀도를 곱해 `μ` 로 적분한 결과가 `ν` 로 적분한 것과 같다는 점이 확인된다. 이 등식이 중요도 표본추출과 우도비 검정의 계산 근거다.

## 조건부 기댓값과의 관계

부분 `σ`-대수 `G ⊆ F` 가 주어졌을 때 `E[X | G]` 는 `G` 위에서 정의된 측도 `A ↦ ∫_A X dP` 의 `P|_G` 에 대한 Radon–Nikodym 도함수다. 조건부 기댓값이 존재하고 거의 확실히 유일하다는 사실이 이 정리에서 곧바로 나온다.
...
`ℝⁿ` 에서 `ν ≪ λ` 일 때 도함수는 작은 공에서의 비율의 극한
