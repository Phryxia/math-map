...
$\Lambda$ 가 작다고 가정하고, 계수를 미지수로 둔 지수다항식 $\Phi(z_1,\dots,z_{n-1})$ 을 세운다. 미지수의 개수를 조건의 개수보다 많게 잡으면 Siegel 보조정리가 크기가 통제된 정수 해를 주고, 그 해로 만든 $\Phi$ 는 격자점에서 높은 계수의 도함수까지 $0$ 이 된다.

여기에 최대절댓값 원리를 반복 적용해 $\Phi$ 가 $0$ 이 되는 점의 범위를 넓힌다. 외삽을 충분히 반복하면 $0$ 이 아니어야 할 행렬식이 $1$ 보다 작아져 모순이 나온다. 모순이 나오는 지점을 추적하면 $\vert\Lambda\vert$ 의 하한이 나온다. ∎
