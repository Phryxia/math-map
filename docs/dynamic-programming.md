...
이 기법은 [DAG](dag-topological.md)(directed acyclic graph) 위의 계산이다. 부분문제를 정점으로, "$A$ 를 풀려면 $B$ 의 답이 필요하다" 를 간선으로 삼으면 의존 관계 [그래프](graphs.md)가 만들어진다. 이 그래프에 순환이 없어야 계산이 끝나고, 순환이 없으면 위상순서가 존재하므로 그 순서대로 한 번씩 채워 나간다. 계산 시간은 정점 수와 간선 수의 합이다.
