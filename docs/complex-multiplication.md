...
분열하는 경우 $p=\pi\bar\pi=N(\pi)$ 이므로 **$p$ 를 $K$ 의 노름으로 쓰는 것**과 곡선의 점 개수 $\char35{}\tilde E(\mathbb F_p)=p+1-(\pi+\bar\pi)$ 가 같은 자료다. $p=x^2+ny^2$ 꼴 표현 문제가 CM 이론으로 풀리는 것이 이 때문이다. 예컨대 $p=x^2+27y^2$ 인 것은 $p\equiv1\pmod3$ 이고 $2$ 가 $\bmod\thinspace p$ 세제곱잉여인 것과 같은데, 이는 $h(-108)=3$ 인 순서환의 유체 다항식 $X^3-2$ 로 설명된다.
...
암호에서는 $\char35{}E(\mathbb F_p)$ 가 큰 소인수를 갖거나, 쌍선형 사상을 위해 특정한 매장 차수를 갖는 곡선이 필요하다. 무작위 곡선을 뽑아 [Schoof–Elkies–Atkin](sea-algorithm.md) 으로 위수를 세는 방법은 비싸다. CM 방법은 반대로 간다.
