...
유한 차원에서 `T\colon F^m\to F^n` 의 계수를 `r` 라 하면 `\dim\ker=m-r`, `\dim\mathrm{coker}=n-r` 이므로 지표는 언제나 `m-n` 이다. `T` 를 아무리 흔들어도 두 차원이 같이 움직여 차가 고정된다.
...
`S` 는 단사지만 전사가 아니다. 상이 여차원 1 이라 `\mathrm{ind}(S)=-1` 이다. `S^*` 는 전사지만 핵이 1 차원이라 `\mathrm{ind}(S^*)=+1` 이다. 유한 차원에서는 단사와 전사가 같은 말이지만 여기서는 갈라지고, 그 갈라짐의 크기가 지표다.
...
`a(z)=z-c` 를 직접 계산해 보면 공식이 확인된다. `T_a=S-c` 의 핵은 언제나 `0` 이고, 여핵은 `\ker(S^*-\bar c)=\{(x_0,\bar cx_0,\bar c^2x_0,\dots)\}` 인데 이 수열이 `\ell^2` 에 있으려면 `|c|<1` 이어야 한다. 그래서 지표는 `|c|<1` 에서 `-1`, `|c|>1` 에서 `0` 이다. 감음수도 정확히 그렇게 나뉜다.
...
먼저 유한 차원에서 지표가 아무 정보도 담지 않음을 확인한다. 무작위 정수 행렬 300 개에서 핵과 여핵의 차원을 정확 유리수 계산으로 구해 보면, 그 차가 언제나 행렬의 모양으로만 정해진다.

그다음 Toeplitz 기호의 감음수를 수치로 세어 지표 공식을 확인한다. 감음수는 `a(e^{i\theta})` 가 원점을 도는 횟수이므로, 편각의 증분을 `(-\pi,\pi]` 로 되돌리며 더하면 된다.

```python
import cmath, math, random
from fractions import Fraction as Q

# 1) 유한 차원에서는 지표가 행렬에 전혀 의존하지 않는다
def rank(M):
...
for _ in range(300):
    ...
    M = [[Q(random.randint(-2, 2)) for _ in range(m)] for _ in range(n)]  # n×m, 열 m 개
    r = rank(M)
    ker, coker = m - r, n - r
    ok &= (ker - coker == m - n)
print("유한 차원 : dim ker − dim coker = m − n 이 언제나 성립 :", ok)

# 2) Toeplitz 작용소의 지표는 −(기호의 감음수)
def winding(sym, N=200000):
    tot, prev = 0.0, cmath.phase(sym(1.0 + 0j))
    for k in range(1, N + 1):
        cur = cmath.phase(sym(cmath.exp(2j * math.pi * k / N)))
        d = cur - prev
...
tests = [("z - 1/2",        lambda z: z - 0.5,                    1),
         ("z - 2",          lambda z: z - 2,                      0),
         ("(z-1/2)(z-1/3)", lambda z: (z - 0.5) * (z - 1 / 3),    2),
         ("(z-1/2)/z",      lambda z: (z - 0.5) / z,              0),
         ("1/z",            lambda z: 1 / z,                     -1),
         ("z^3",            lambda z: z ** 3,                     3)]
print(f"{'기호 a(z)':<16} {'감음수':>6} {'예상':>5} {'지표 = −감음수':>14}")
allok = True
for name, f, expect in tests:
    w = winding(f)
    allok &= (w == expect)
    print(f"{name:<16} {w:>6} {expect:>5} {-w:>14}")
print("감음수가 모두 예상과 일치 :", allok)

# 유한 차원 : dim ker − dim coker = m − n 이 언제나 성립 : True
# 기호 a(z)             감음수    예상      지표 = −감음수
# z - 1/2               1     1             -1
# z - 2                 0     0              0
# (z-1/2)(z-1/3)        2     2             -2
# (z-1/2)/z             0     0              0
...
첫 줄이 유한 차원의 무의미함이다. 행렬을 무작위로 뽑아도 지표는 언제나 `m-n` 이다. 지표가 정보를 담으려면 무한 차원이 필요하다.

아래 표에서 두 가지가 눈에 띈다. `z-1/2` 와 `z-2` 는 계수만 조금 다른데 지표가 `-1` 과 `0` 으로 갈린다. 근이 단위원 안에 있느냐 밖에 있느냐가 갈림길이고, 그 사이를 연속적으로 움직이려면 반드시 근이 단위원 위에 놓이는 순간을 지나야 한다. 그 순간에 기호가 `0` 을 값으로 가져 Fredholm 성질 자체가 깨진다. 지표가 불연속으로 뛰는 것이 아니라, 뛰는 길목에서 정의역을 벗어나는 것이다.

`(z-1/2)/z` 의 감음수가 `0` 인 것도 같은 구조다. 분자가 `+1`, 분모가 `-1` 을 주어 상쇄된다. 지표의 곱셈성 `\mathrm{ind}(T_{ab})=\mathrm{ind}(T_a)+\mathrm{ind}(T_b)` 가 감음수의 덧셈성으로 나타난 것이다.
