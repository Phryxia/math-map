...
는 계수 정리로 $T$ 와 무관하게 $\dim V-\dim W$ 이므로, 유한 차원에서 지표는 정보를 담지 않는다.
...
무한 차원에서는 이 상쇄가 자동이 아니다. 왼쪽 이동 작용소 $S^\ast$ 와 오른쪽 이동 작용소 $S$ 를 본다.

$$
S(x_0,x_1,\dots)=(0,x_0,x_1,\dots),\qquad S^\ast(x_0,x_1,\dots)=(x_1,x_2,\dots)
$$

$S$ 는 단사이고 상이 여차원 1 이라 $\mathrm{ind}(S)=-1$ 이다. $S^\ast$ 는 전사이고 핵이 1 차원이라 $\mathrm{ind}(S^\ast)=+1$ 이다. 유한 차원에서 같은 말인 단사와 전사가 여기서 갈라지고, 그 갈라짐의 크기가 지표다.
...
가역원 전체는 열린집합이고 지표는 그 위에서 연속인 정수값 함수이므로 [연결성분](connectedness.md)마다 상수다. 지표가 위상적 불변량이라는 말은 이 뜻이다.
...
> 4. $\mathrm{ind}(T^\ast)=-\mathrm{ind}(T)$ 이고, $T$ 가 자기수반이면 지표가 $0$ 이다.
...
우변은 주기호의 위상적 데이터로만 이루어진다. 이 구조가 [Gauss–Bonnet 정리](gauss-bonnet.md)와 [Riemann–Roch 정리](riemann-roch.md)를 특수한 경우로 포함하고, Fredholm 지표가 그 틀의 기초 층이다.
...
- **작용소 대수와 $K$ 이론.** 지표 사상이 $C^\ast$ 대수의 $K_1$ 에서 $K_0$ 으로 가는 연결 준동형이다. Toeplitz 확대의 지표 계산이 Bott 주기성의 한 증명을 준다.
