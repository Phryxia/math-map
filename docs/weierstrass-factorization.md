...
$\prod_{n\ge1}(1+u_n)$ 은 $\sum_n\vert u_n\vert$ 이 수렴하면 절대수렴하고, 그 값은 어떤 $u_n$ 도 $-1$ 이 아닐 때만 $0$ 이 아니다. 각 $u_n$ 이 영역에서 정칙이고 급수가 콤팩트 집합마다 [균등수렴](uniform-convergence.md)하면 곱도 정칙함수로 균등수렴하며, 그 영점은 인자들의 영점을 중복도까지 합친 것이다.
...
- [감마 함수](gamma-function.md): $1/\Gamma$ 는 위수 $1$ 의 전해석함수이고 영점이 $0,-1,-2,\dots$ 다. Hadamard 인수분해가 $\frac1{\Gamma(z)}=z\thinspace e^{\gamma z}\prod_{n\ge1}\left(1+\frac zn\right)e^{-z/n}$ 을 준다. $\gamma$ 는 Euler–Mascheroni 상수다.
...
- [타원함수](elliptic-functions.md): Weierstrass 시그마 함수는 [격자](lattices.md)점을 영점으로 갖는 기본 인수 곱이고, 그 로그 미분이 제타 함수와 페 함수로 이어진다.
