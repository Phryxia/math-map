...
격자의 근사 **SVP**(shortest vector problem)도 숨은 부분군 문제로 번역되지만 나오는 군이 아벨이 아닌 이면체군이다. 이면체군의 숨은 부분군 문제에는 다항시간 양자 알고리즘이 알려져 있지 않고, 준지수 시간의 Kuperberg 알고리즘이 알려진 최선이며 격자 파라미터에 직접 적용되지 않는다[^1].
