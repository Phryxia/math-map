...
LP 완화가 그 비교 대상을 만들어 준다. 정수 제약 $x_i \in \lbrace 0,1\rbrace$ 을 $0 \le x_i \le 1$ 로 풀면 문제가 [선형계획법](linear-programming.md)이 되어 다항시간에 풀린다. 완화는 제약을 없앤 것이므로 그 최적값이 원래 최적값의 하한이고, 이 하한과 비교하면 근사비를 증명할 수 있다.
...
확률 $\min(1, t\thinspace x^*_S)$ 로 한 번에 뽑는 것도 같은 효과다. 비용이 $t$ 배로 늘고 실패 확률이 $e^{-t}$ 로 줄어드는 교환이 이 기법의 뼈대다.
