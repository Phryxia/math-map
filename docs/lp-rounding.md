...
**LP**(linear programming) 완화가 그 비교 대상이다. 정수 제약 $x_i \in \lbrace 0,1\rbrace$ 을 $0 \le x_i \le 1$ 로 풀면 문제가 [선형계획법](linear-programming.md)이 되어 다항시간에 풀리고, 제약을 없앴으므로 그 최적값이 원래 최적값의 하한이다.
