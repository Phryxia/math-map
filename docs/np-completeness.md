...
일 때 `A` 는 `B` 로 **다항시간 many-one 환원**(Karp 환원)된다고 하고 `A ≤_p B` 로 쓴다.

두 성질이 정의의 전부다. 첫째 `f` 는 다항시간에 계산되어야 하고, 둘째 소속 여부를 정확히 보존해야 한다(예 인스턴스는 예로, 아니오 인스턴스는 아니오로). `f` 가 전단사일 필요도, 역이 계산 가능할 필요도 없다.

**보조정리.** `≤_p` 는 반사적이고 추이적이다. 또 `A ≤_p B` 이고 `B ∈ P` 이면 `A ∈ P` 이며, `B ∈ NP` 이면 `A ∈ NP` 다.
...
**정의.** 언어 `B` 가 **NP-hard**라는 것은 모든 `A ∈ NP` 에 대해 `A ≤_p B` 라는 뜻이다. `B` 가 NP-hard이면서 `B ∈ NP` 이면 `B` 는 **NP-complete**다.

직접적인 따름정리 두 개.

- 어떤 NP-complete 문제가 P에 속하면 `P = NP` 다.
- `P ≠ NP` 이면 어떤 NP-complete 문제도 다항시간 알고리즘을 갖지 않는다.
...
증명 스케치. `L ∈ NP` 를 잡으면 시간 `p(n)` 의 비결정적 Turing 기계 `M` 이 `L` 을 판정한다. 입력 `w` (길이 `n`)에 대해 `M` 의 한 실행을 `p(n) x p(n)` 크기의 **계산 표**로 적는다. `i` 번째 행이 시각 `i` 의 테이프 구성(각 칸의 기호, 헤드 위치, 상태)이다. 시간이 `p(n)` 이므로 헤드는 `p(n)` 칸을 넘어 움직이지 못해 표의 폭도 `p(n)` 이면 충분하다.

변수 `x[i, j, s]` 를 "시각 `i` 에 칸 `j` 의 내용이 기호 `s` 다"로 둔다(`s` 는 테이프 기호이거나 상태-기호 쌍). 변수 개수는 `O(p(n)^2)` 이다. 다음 네 묶음의 절을 만든다.

1. **칸 일관성.** 각 `(i, j)` 에 대해 정확히 하나의 `s` 만 참이다. "적어도 하나"는 `s` 에 대한 논리합 하나, "많아야 하나"는 쌍마다 `(¬x[i,j,s] ∨ ¬x[i,j,t])` 이다.
...
이렇게 만든 CNF 논리식 `φ_w` 의 크기는 `O(p(n)^2)` 이고 `w` 로부터 다항시간에 구성된다. 그리고

$$
\varphi_w \text{ 가 충족가능} \iff M \text{ 이 } w \text{ 를 받아들이는 실행을 갖는다} \iff w \in L
$$

이다. 충족 할당에서 합법적인 계산 표를 읽어 낼 수 있고, 역으로 받아들이는 실행이 표를 통해 할당을 준다. 따라서 `L ≤_p SAT` 이다. ∎
...
NP-완전성을 새로 증명할 때는 Cook–Levin을 반복하지 않는다. 이미 NP-complete인 문제 `B` 에서 새 문제 `C` 로의 환원 `B ≤_p C` 를 만들고 `C ∈ NP` 를 확인하면, 추이성에 의해 `C` 도 NP-complete다. 환원의 방향을 뒤집으면(즉 `C ≤_p B` 를 보이면) 아무것도 증명되지 않는다는 점이 가장 흔한 실수다.
...
임의의 CNF 절을 길이 3의 절들로 바꾼다. 절 `(l_1 ∨ l_2 ∨ ... ∨ l_k)` 에 대해 `k >= 4` 이면 새 변수 `y_1, ..., y_{k-3}` 를 도입해
...
3CNF 논리식이 `m` 개 절 `C_1, ..., C_m` 을 가질 때 [그래프](graphs.md) `G` 를 만든다. 정점은 각 절의 각 리터럴, 즉 `3m` 개다. 두 정점 사이에 간선을 놓는 조건은 (1) 서로 다른 절에 속하고, (2) 서로 모순이 아니다(하나가 `x`, 다른 하나가 `¬x` 가 아니다).
...
- **NP에 속하는 모든 문제가 NP-완전은 아니다.** `P ≠ NP` 라면 Ladner 정리에 의해 두 부류 어디에도 속하지 않는 NP-중간(NP-intermediate) 문제가 존재한다. [그래프 동형](graph-isomorphism.md) 문제와 소인수분해([RSA 암호](rsa-cryptosystem.md)의 기반)가 그런 후보로 거론된다.
...
- **Ladner 정리.** `P ≠ NP` 이면 NP-완전도 P도 아닌 문제가 존재한다. 대각선 논법을 다항시간 틀에서 수행한 결과다.
