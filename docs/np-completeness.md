...
- 어떤 NP-complete 문제가 P에 속하면 `P = NP` 다.
...
명제 변수 `x_1, ..., x_n` 에 대해 리터럴은 변수 또는 그 부정이고, 절(clause)은 리터럴들의 논리합이며, CNF 논식은 절들의 논리곱이다. 각 절의 리터럴이 정확히 세 개인 CNF를 3CNF라 한다.
...
4. **전이.** 표의 모든 `2 x 3` 창(window)이 `M` 의 전이 규칙에 비추어 합법이다. 창 하나의 합법적 내용은 유한 개(알파벳과 상태 집합에만 의존)이므로, 불법 내용마다 그것을 금지하는 절을 쓰면 창당 상수 개 절이다.
...
로 치환한다. `k <= 2` 이면 리터럴을 반복하거나 더미 변수로 채운다.

**충족성 보존.** 원래 절이 어떤 할당에서 참이면 참인 리터럴 `l_i` 를 기준으로 `y_1, ..., y_{i-2}` 를 참, 나머지를 거짓으로 두면 모든 새 절이 참이다. 역으로 모든 `l_i` 가 거짓이면 새 절들의 연쇄가 `y_1` 부터 차례로 참을 강요하다 마지막 절에서 모순이 난다. 변환은 원식 크기에 선형이다.
...
`G` 의 정점 집합 `S` 가 clique인 것과, 여그래프 `G'` (같은 정점, 간선을 뒤집음)에서 `V \ S` 가 정점덮개인 것은 동치다. 따라서 "`G` 에 크기 `k` 의 clique가 있는가"는 "`G'` 에 크기 `|V| - k` 의 정점덮개가 있는가"와 같다. 독립집합(independent set)도 같은 보집합 관계로 묶인다.
...
- **상대화 장벽.** Cook–Levin의 증명은 신탁 기계에도 그대로 상대화되므로, 이런 종류의 논증만으로는 `P` 와 `NP` 를 분리할 수 없다(Baker–Gill–Solovay). 이 사실이 [P 대 NP 문제](p-np.md)가 왜 미해결인지에 대한 첫 번째 설명이다.
