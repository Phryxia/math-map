...
수학적 platonism 은 수, 집합, 함수 같은 수학적 대상이 실제로 존재하고 그것이 추상적이며 우리의 언어와 사고, 관행과 독립적이라고 보는 존재론적 입장이다. 표준적으로 Existence, Abstractness, Independence 의 세 주장으로 정리된다.[^1]
...
물리학자가 전자를 인정하는 근거는 전자가 관측 결과를 설명한다는 데 있고, 그 설명은 미분방정식과 Hilbert 공간을 경유한다. 전자는 인정하면서 실수는 인정하지 않는 태도의 일관성을 묻는 것이 Quine–Putnam 필수불가결성 논증의 출발점이다.

반대 방향의 관찰도 있다. 어떤 실험도 7 이 소수라는 명제를 흔들지 못한다. 7 은 어디에도 위치하지 않고 아무 작용도 하지 않아 감각과 인과적으로 연결될 길이 없는데도 우리는 7 에 관해 안다.
...
첫째 요구는 지시 대상을 요구하므로 platonism 에 유리하다. 그러나 그 대상이 인과적으로 불활성이면 우리 믿음이 그 대상의 사정과 어떻게 신뢰성 있게 상관되는지 설명할 수 없고, 인과적 지식 이론을 버려도 문제가 남는다. Field 의 재정식화는 이를 수학적 믿음과 수학적 사실의 일치를 platonist 가 설명하지 못한다는 신뢰성(reliability) 문제로 적는다.
...
산술만으로는 둘 중 어느 것이 진짜 2 인지 결정할 수 없다. 개별 대상보다 구조가 본질적이라는 결론으로 이어져 구조주의의 동기가 된다. platonist 의 대응은 둘이다. 어느 한쪽이 실제 자연수라고 인정하되 우리가 알 수 없다고 보거나(agnostic platonism), 자연수를 구조 안의 위치로 재해석한다.

## 독립성 결과의 해석

독립적 명제의 존재는 platonism 에 대한 반례가 아니다. 연속체 가설이 ZFC 에서 증명도 반증도 되지 않는다는 사실[^3]은 platonist 에게 공리계가 실재를 완전히 포착하지 못한다는 뜻이고, formalist 에게는 질문 자체가 체계에 상대적이라는 뜻이다. 같은 수학적 사실이 두 해석과 모두 양립하며, 철학적 입장 차이는 정리로 판정되지 않는다.
...
- 기초 이론의 선택. [집합](sets.md) 기반 기초와 범주론, 타입 이론 기반 기초 가운데 옳은 기초를 묻는 질문 자체가 platonism 을 전제한다. 도구주의적 입장에서는 목적에 따라 다른 기초를 쓴다.
