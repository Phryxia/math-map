...
독립적 명제의 존재는 platonism 에 대한 반례가 아니다. [연속체 가설](continuum-hypothesis.md)이 ZFC(Zermelo–Fraenkel 집합론에 [선택공리](axiom-of-choice.md)를 더한 공리계)에서 증명도 반증도 되지 않는다는 사실[^3]은 platonist 에게 공리계가 실재를 완전히 포착하지 못한다는 뜻이고, formalist 에게는 질문 자체가 체계에 상대적이라는 뜻이다. 같은 수학적 사실이 두 해석과 모두 양립하며, 철학적 입장 차이는 정리로 판정되지 않는다.
