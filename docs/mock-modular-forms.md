...
$q\to\zeta$ (단위근) 로 갈 때 $\widehat h$ 는 모듈러이므로 변환법칙에서 곧바로 점근 전개가 나온다. $h=\widehat h-g^\ast$ 이고 $g^*$ 는 그 극한에서 $y^{1-k}$ 정도로 얌전하다. 그러므로 $h$ 의 점근은 모듈러 형식의 점근과 주요항이 같고, 차이는 한 차수 낮은 곳에서 나타난다. Ramanujan 이 "모듈러 형식처럼 생겼는데 어떤 모듈러 형식과도 일치하지 않는다" 고 말할 수 있었던 이유가 이것이다. 그는 주요항을 본 것이고, 보정항은 그가 계산하던 정밀도 아래에 숨어 있었다.
