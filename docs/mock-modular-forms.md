...
고전적 Maass 파형은 $\Delta_0$ 의 고유값이 $0$ 이 아닌 고유함수이고 첨점에서 감쇠한다. 조화 Maass 형식은 고유값이 $0$ 이며 첨점에서 지수적으로 자라도 된다. 전자는 Eisenstein 급수와 함께 $L^2$ 스펙트럼을 이루고 후자는 스펙트럼 바깥의 이산적 대상이다.
...
- **분할수의 계수 합동.** rank [생성함수](generating-functions.md)의 합동이 mock 모듈러성으로 설명된다. Dyson 의 rank 는 mock 을, crank 는 보통 Jacobi 형식을 준다.
