...
닫힘형식이면서 $\delta$ 로도 소멸하는 형식이 최소점이고, 두 조건이 $\Delta\omega=0$ 과 같다.
...
존재는 다르다. 무한차원 공간에서 최소화 수열이 수렴하는 대상은 매끄러운 형식이 아니라 분포일 수 있다. 여기서 $\Delta$ 의 타원성이 쓰인다. 타원 정칙성 정리로 $\Delta\omega$ 가 매끄러우면 $\omega$ 도 매끄러우므로 약한 해가 진짜 해가 되고, 타원 작용소의 핵은 콤팩트 다양체 위에서 유한차원이다. 코호몰로지의 유한차원성이 해석학의 정리로 나온다.
...
Hodge 분해는 게이지 고정의 기하적 정체다. 퍼텐셜 $A$ 의 $d\Omega^0$ 성분이 게이지 변환으로 바꿀 수 있는 부분이고, Lorenz 게이지 $\delta A=0$ 이 그 성분을 제거해 물리적 자유도만 남긴다. 남는 조화 성분이 위상적 자유도이며 de Rham 코호몰로지의 Aharonov–Bohm 효과에 해당한다.
