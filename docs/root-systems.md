...
증명은 단순근을 단위벡터로 정규화한 뒤 남는 조건, 곧 일차독립이고 서로의 내적이 양수가 아니라는 조건만으로 진행한다. 이 조건을 만족하는 벡터 집합을 허용 배치라 하면 그 부분집합도 허용 배치이므로, 작은 도표를 금지하는 것으로 큰 도표를 걸러낼 수 있다. 단계는 다섯이다.

1. 서로 다른 단순근의 Cartan 정수의 곱 $\langle\alpha_i,\alpha_j\rangle\langle\alpha_j,\alpha_i\rangle=4\cos^2\theta$ 는 $0,1,2,3$ 뿐이다. 값이 4 이면 두 근이 비례한다.
2. 도표에 사이클이 없다. 사이클을 이루는 단위벡터들의 합의 노름을 계산하면 $0$ 이하가 되어 일차독립에 어긋난다.
3. 한 꼭짓점에 붙는 선의 수가 3 을 넘지 않는다. 이웃한 단위벡터들이 서로 직교하므로 Bessel 부등식이 상한을 준다.
4. 단선으로만 이어진 사슬은 그 벡터들의 합으로 대체해도 허용 배치가 유지된다. 이 수축으로 이중선이 둘인 도표와 분기점이 둘인 도표가 금지된다.
5. 남는 것은 분기점이 하나인 $T$ 모양뿐이고, 세 가지의 길이 $p,q,r$ 가 $\tfrac1p+\tfrac1q+\tfrac1r\gt 1$ 을 만족해야 한다. 해는 $(1,q,r)$ , $(2,2,r)$ , $(2,3,3)$ , $(2,3,4)$ , $(2,3,5)$ 이고 차례로 $A$ , $D$ , $E_6$ , $E_7$ , $E_8$ 이다.

이중선과 삼중선을 허용하면 $B_n$ , $C_n$ , $F_4$ , $G_2$ 가 같은 계산에서 나온다. 예외가 다섯 개에서 멈추는 것은 5 단계의 부등식에 해가 유한하기 때문이다.
...
이 분류 문제들이 같은 목록을 내놓는 원인은 분류 절의 $\tfrac1p+\tfrac1q+\tfrac1r\gt 1$ 부등식이고, 그 부등식은 구면의 유한 대칭 배치가 몇 개뿐이라는 사실에서 온다. 정다면체의 분류와 삼각군이 구면과 평면과 쌍곡 가운데 어디에 놓이는지도 같은 부등식이 정하며, 해 $(2,3,5)$ 가 $E_8$ 과 정이십면체에 함께 대응한다.
