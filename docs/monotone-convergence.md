...
$c \to 1$ 로 보내고 $f$ 아래의 모든 단순함수 $s$ 에 대해 상한을 취하면 정의로 $\int f$ 가 나온다. 증명이 쓰는 것은 측도의 아래로부터 연속성뿐이다.
...
증가하는 사건열의 확률이 합집합의 확률로 수렴한다는 사실, 음이 아닌 [확률변수](random-variables.md)를 유계 확률변수로 잘라 근사해도 기댓값이 보존된다는 사실, 꼬리합 공식

$$
\mathbb{E}[X]=\int_0^\infty P(X\gt t)\thinspace dt\qquad(X\ge0)
$$

이 모두 이 정리에서 나온다. Martingale 수렴 정리와 [조건부 기댓값](conditional-expectation.md)의 성질을 세울 때도 쓰인다.
