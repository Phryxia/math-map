...
`fₙ` 이 증가하며 `f` 로 간다는 것은 적분값도 증가하며 어딘가로 간다는 뜻이다. 문제는 그 도달점이 `∫f` 보다 작을 수 있는가다.

질량이 새어 나가려면 어딘가로 흘러야 한다. 그런데 `fₙ ≤ f` 이고 각 점에서 값이 줄지 않으므로, 질량이 옆으로 도망가거나 무한대로 달아날 방법이 없다. 각 점에서 값이 제자리이거나 올라가기만 하기 때문이다. 단조성이 하는 일이 바로 이 봉쇄이며, 그래서 다른 가정이 필요 없다.

## 단조성이 빠지면 곧바로 무너진다

`(0,1]` 에서 `fₙ = n·1_{(0,1/n)}` 을 보자. 모든 점에서 `0` 으로 수렴하지만 각 적분은 `1` 이다. 점점 좁아지는 구간에 점점 높은 봉우리를 세우면 질량이 원점으로 달아나 버린다.
...
`fₙ = n·1_{(0,1/n)}` 은 증가하지도 않고 적분 가능한 지배함수도 없어 오른쪽 아래로 떨어진다. Fatou 부등식 `0 ≤ 1` 은 여전히 맞다.

# 정의

## 진술

측도 공간 `(X, Σ, μ)` 에서 각 `n` 에 대해 `fₙ : X → [0,∞]` 가 가측이고 모든 `x` 에서

$$
0\le f_1(x)\le f_2(x)\le\cdots
$$

라 하자. 점별 극한을 `f(x) = lim fₙ(x)` 로 두면

$$
\lim_{n\to\infty}\int_X f_n\,d\mu=\int_X f\,d\mu
$$

가 성립한다. 양변에 `∞` 도 허용한다. 극한 `f` 는 가측함수의 점별 극한이므로 자동으로 가측이고, 적분값의 수열은 단조성으로 증가하므로 극한이 항상 존재한다. 즉 정리의 양변이 늘 정의된다.

# 성질

## 증명

적분의 단조성에서 `∫fₙ ≤ ∫f` 이므로 왼쪽 극한은 오른쪽 이하다.
...
라 두자. `fₙ` 이 증가하므로 `E_n` 은 증가하고, `c < 1` 이므로 `s` 가 양수인 모든 점이 언젠가 `E_n` 에 들어가 `⋃E_n = X` 다. 그러면

$$
\int_X f_n\,d\mu\ge\int_{E_n}f_n\,d\mu\ge c\int_{E_n}s\,d\mu
$$

이고, `s` 가 단순함수이므로 오른쪽은 유한 개 집합의 측도의 합이라 측도의 아래로부터 연속성을 쓸 수 있다. `n → ∞` 를 취하면

$$
\lim_{n\to\infty}\int_X f_n\,d\mu\ge c\int_X s\,d\mu
$$

를 얻는다. `c → 1` 로 보내고 `f` 아래의 모든 단순함수 `s` 에 대해 상한을 취하면 정의에 의해 `∫f` 가 나온다.

증명이 쓰는 것은 측도의 아래로부터 연속성뿐이다. 정리가 사실상 그 성질을 함수 층위로 옮긴 것임을 보여 준다.

## 측도의 연속성

거꾸로 증가하는 가측집합 `E₁ ⊆ E₂ ⊆ ⋯` 에 `fₙ = 1_{E_n}` 을 적용하면
...
`gₙ = inf_{k≥n} f_k` 로 두면 `gₙ` 이 증가하고 `gₙ ≤ fₙ` 이므로 단조수렴 정리를 적용해

$$
\int_X\liminf_n f_n\,d\mu\le\liminf_n\int_X f_n\,d\mu
$$

를 얻는다. 이것이 Fatou 보조정리이고, 음이 아니라는 가정만 있으면 성립한다. 여기에 지배함수를 더해 `g - fₙ` 과 `g + fₙ` 양쪽에 적용하면 지배수렴 정리가 나온다. 세 정리가 하나의 사슬로 이어진다.

## 적분의 선형성

음이 아닌 가측함수 `f, g` 에 대해 `∫(f+g) = ∫f + ∫g` 를 보이는 표준적인 방법도 이 정리다. 단순함수로 근사하는 열 `sₙ ↑ f`, `tₙ ↑ g` 를 잡으면 `sₙ + tₙ ↑ f + g` 이고, 단순함수에서는 선형성이 자명하므로 양변에 정리를 적용하면 된다.

정의에서 곧바로 나오지 않는 기본 성질조차 이 정리를 거쳐야 한다는 점이, 왜 이것이 "첫 번째 정리" 로 불리는지를 설명한다.

## 하강하는 경우는 다르다

감소하는 열에는 같은 결론이 성립하지 않는다. `fₙ = 1_{[n,∞)}` 은 `0` 으로 감소하지만 각 적분이 `∞` 다. 감소하는 경우에는 `∫f₁ < ∞` 라는 조건이 추가로 필요하며, 그때는 지배수렴 정리로 처리된다.
...
부분합 `F_N = Σ_{n≤N} gₙ` 은 각 `gₙ ≥ 0` 이면 증가하므로 정리를 그대로 적용해
