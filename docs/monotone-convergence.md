...
단조 수렴 정리는 음이 아닌 가측함수열 $f_n$ 이 점마다 증가해 $f$ 로 가면 $\int f_n\to\int f$ 라는 정리다. 균등수렴도 지배함수도 요구하지 않는다.
...
$f_n=\min(f,n)$ 은 $f$ 를 높이 $n$ 에서 잘라 낸 함수다. 점마다 증가해 $f$ 로 가고, 각 $\int f_n$ 은 $\int f$ 이하다. 정리는 $\int f_n$ 이 $\int f$ 까지 올라간다고 말한다. $f$ 아래의 단순함수 $s$ 는 유한한 값만 가지므로 어느 $n$ 부터는 $f_n$ 아래에 있고, 그래서 $\int f_n$ 의 극한은 어떤 $\int s$ 보다도 작지 않다.
