...
[Lebesgue 적분](lebesgue-integral.md)은 아래에 놓인 단순함수들의 상한으로 정의된다. 이 정의에서는 선형성조차 곧바로 나오지 않는다.

단조 수렴 정리는 음이 아닌 함수열이 점마다 증가하기만 하면 극한과 적분을 교환할 수 있다고 말한다. 가정에 균등성도 지배함수도 유한성도 없다. 여기서 적분의 선형성, 급수와 적분의 교환, Fatou 보조정리, [지배수렴 정리](dominated-convergence.md)가 차례로 따라 나온다.

# 직관

## 단조성이 막는 것

$f_n$ 이 증가하며 $f$ 로 가면 적분값도 증가하며 어딘가로 간다. 물음은 그 도달점이 $\int f$ 보다 작을 수 있는가다.

$f_n \le f$ 이고 각 점에서 값이 줄지 않으므로 질량이 옆으로 도망가거나 무한대로 달아날 방법이 없다. 단조성이 이 봉쇄를 하므로 다른 가정이 필요하지 않다.

## 단조성이 없는 반례

$(0,1]$ 에서 $f_n = n \cdot \mathbf 1_{(0,1/n)}$ 은 모든 점에서 $0$ 으로 수렴하지만 각 적분이 $1$ 이다. 좁아지는 구간에 높아지는 봉우리를 세우면 질량이 원점으로 달아난다.
...
$f_n = n \cdot \mathbf 1_{(0,1/n)}$ 은 증가하지도 않고 적분 가능한 지배함수도 없어 오른쪽 아래로 떨어진다. Fatou 부등식 $0 \le 1$ 은 성립한다.
...
반대 부등식을 본다. $f$ 아래의 음이 아닌 단순함수 $s$ 를 고정하고 $0 < c < 1$ 을 잡아

$$
E_n=\lbrace x: f_n(x)\ge c\thinspace s(x)\rbrace
$$

라 두자. $f_n$ 이 증가하므로 $E_n$ 이 증가하고, $c < 1$ 이므로 $s$ 가 양수인 모든 점이 언젠가 $E_n$ 에 들어가 $\bigcup E_n = X$ 다.
