...
이고 $P_{w,w}=1$ 이며 $x<w$ 이면 $\deg P_{x,w}\le\frac{\ell(w)-\ell(x)-1}{2}$ 인 원소 $C'_w$ 가 유일하게 존재한다. 차수 상계가 $\frac12(\ell(w)-\ell(x)-1)$ 로 **엄격히** 잡히는 것이 유일성의 핵심이다. 상계를 하나만 느슨하게 해도 자유도가 생긴다.
...
$P_{x,w}$ 를 계산하는 것은 $\char35{}\mathrm P$ 난해로 여겨진다. 재귀는 Bruhat 구간 전체를 훑고, $S_n$ 에서 구간의 크기는 $n$ 에 대해 초지수적으로 자란다. 그럼에도 실무에서는 다음 성질들이 계산을 크게 줄인다.
