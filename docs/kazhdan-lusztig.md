...
놀라운 것은 이 다항식이 전혀 다른 두 곳에서 같은 모습으로 나타난다는 사실이다. 기하에서는 Schubert 다양체 `X_w` 의 교차 코호몰로지 층의 줄기 차원이고, 표현론에서는 범주 $\mathcal O$ 의 기약가군을 Verma 가군으로 전개한 계수다. 세 세계를 잇는 이 일치가 Kazhdan–Lusztig 추측이었고, 지금은 정리다.
...
`q=1` 을 넣으면 `T_s^2=1` 이라 군환으로 돌아온다. `q` 를 소수 거듭제곱으로 두면 이 대수는 유한체 위 군 $G(\mathbb F_q)$ 의 Borel 부분군에 대한 이중잉여류 대수로 실현된다. 곧 $\mathcal H$ 는 **`W` 의 `q` 진 그림자**이고, $q\to1$ 극한에서만 군이 보인다. 변형은 정보를 잃지 않고 오히려 `q` 라는 새 눈금을 얻는다.
...
두 조건이 답을 하나로 묶어 주는 구조는 선형대수의 Gram–Schmidt 와 같다. 대칭성(자기쌍대)과 삼각성(Bruhat 순서에 대한 상삼각 전개), 그리고 차수 제한이 함께 걸리면 자유도가 사라진다. Kazhdan–Lusztig 다항식은 그 유일한 답의 계수이며, 이런 종류의 기저는 이후 양자군의 정준기저, 결정기저 등으로 반복해 나타난다.
...
이 해석은 두 가지를 즉시 설명한다. 계수가 음이 아닌 정수라는 사실(차원이므로)과, `P_{x,w}` 를 계산하는 것이 왜 어려운지(특이점 해소를 요구하므로).

## 언제 `1` 이 아닌가

`A_n` 형, 곧 `W=S_n` 에서는 답이 완전히 조합적으로 알려져 있다. `P_{e,w}=1` 인 것은 `w` 가 두 패턴 `3412` 와 `4231` 을 피하는 것과 같다(Lakshmibai–Sandhya).
...
`S_3` 까지는 모든 Schubert 다양체가 매끄럽고 KL 다항식이 전부 `1` 이다. 비자명한 예가 처음 나오는 곳이 `S_4` 이고 정확히 두 개다.

$$
P_{e,3412}=1+q,\qquad P_{e,4231}=1+q
$$

이 두 순열이 표현론 계산에서 반복해 등장하는 최소 반례가 된 이유가 여기 있다. `n` 이 커지면 매끄러운 쪽이 오히려 희소해진다. 매끄러운 순열의 개수는 큰 Schröder 수 $1,2,6,22,88,366,\dots$ 로 지수적으로 자라지만 `n!` 보다 훨씬 느리다.
...
조합에서 정의하고, 기하에서 의미를 얻고, 표현론에서 쓴다. 세 화살표 가운데 어느 것도 자명하지 않으며, 각각이 하나의 정리다.
...
$\iota\colon\mathcal H\to\mathcal H$ 를 $q^{1/2}\mapsto q^{-1/2}$, $T_w\mapsto T_{w^{-1}}^{-1}$ 로 정의되는 환 대합이라 하자. 각 $w\in W$ 에 대해

$$
\iota(C'_w)=C'_w,\qquad C'_w=q^{-\ell(w)/2}\sum_{x\le w}P_{x,w}(q)T_x
$$

이고 `P_{w,w}=1`, `x<w` 이면 $\deg P_{x,w}\le\frac{\ell(w)-\ell(x)-1}{2}$ 인 원소 `C'_w` 가 유일하게 존재한다. 차수 상계가 $\frac12(\ell(w)-\ell(x)-1)$ 로 **엄격히** 잡히는 것이 유일성의 핵심이다. 상계를 하나만 느슨하게 해도 자유도가 생긴다.
...
`c=1` 이면 `sx<x`, `c=0` 이면 `sx>x` 이고, $\mu(z,v)$ 는 `P_{z,v}` 의 최고차 계수다. 길이에 대한 귀납으로 계산되지만 항의 수가 Bruhat 구간의 크기만큼 늘어난다. `W` 가 크면 계산이 급격히 무거워지고, 실제로 `E_8` 의 전체 KL 다항식 표를 얻는 작업(Atlas 프로젝트, 2007)은 대규모 분산 계산이었다.

## $\mu$ 계수

$\ell(w)-\ell(x)$ 가 홀수일 때 최고차 계수 $\mu(x,w)$ 가 특히 중요하다. 이것이 KL 그래프의 간선 가중치가 되고, 그 그래프의 연결성분이 **셀(cell)** 을 정의한다. 셀은 $\mathcal H$ 의 가군을 쪼개고, Lusztig 은 이를 통해 유한군의 기약표현을 유니포턴트 궤도와 짝짓는 이론을 세웠다.

# 성질

## Kazhdan–Lusztig 추측은 정리다
...
`P_{x,w}` 의 계수가 음이 아니라는 것은 기하적 해석이 있는 Weyl 군의 경우 즉시 따라온다. 차원이기 때문이다. 그러나 일반 Coxeter 군에는 대응하는 다양체가 없어 오랫동안 열려 있었다. Elias 와 Williamson 이 2014 년에 **Soergel 쌍가군**의 범주에 Hodge 이론(강한 Lefschetz, Hodge–Riemann 쌍선형 관계)을 직접 세워 증명했다.[^2] 기하 없이 기하의 결론을 얻은 사례이고, "대수적 대상에 Hodge 구조를 부여한다" 는 방법론 자체가 그 뒤로 여러 곳에 퍼졌다.

## Lusztig 추측과 Williamson 의 반례

표수 `p` 에서 대수군 `G` 의 기약가module 지표를 KL 다항식으로 주려는 것이 Lusztig 추측이다. `p` 가 충분히 크면 성립한다는 것이 알려졌는데, "충분히" 가 얼마인지가 문제였다. 처음에는 Coxeter 수 `h` 에 대해 $p\ge 2h-2$ 정도면 되리라 기대했다.

Williamson 은 $\mathrm{SL}_n$ 에서 반례의 무한족을 만들어 이 기대를 무너뜨렸다.[^3] 필요한 `p` 의 하계가 `n` 의 다항식이 아니라 지수적으로 자란다. 결론은 표수 `p` 표현론이 KL 다항식이 아니라 **`p` 진 KL 다항식**(`p`-canonical basis)을 필요로 한다는 것이었고, 이 대상은 `p` 에 의존하며 조합적으로 훨씬 다루기 어렵다. Riche–Williamson 의 이후 작업이 그 틀을 정리했다.
...
가장 직접적인 쓰임은 범주 $\mathcal O$ 와 유한 Chevalley 군의 지표표다. Weyl 지표 공식이 주지 못하는 자리를 KL 다항식이 채운다. 특히 $\mathcal O$ 의 사영가군 `P_x` 의 Verma 여과 중복도가 `[P_x:M_w]=P_{x,w}(1)` 로 주어지는 BGG 상반성이 있어, 하나의 다항식 표가 지표, 사영분해, Ext 군을 동시에 준다.

## Schubert 다양체의 기하

`X_w` 의 특이점 판정, 국소 교차 코호몰로지, 유리 매끄러움 여부가 모두 `P_{x,w}` 로 읽힌다. Schubert 계산에서 나오는 구조상수가 [Schur 다항식](schur-polynomials.md) 쪽 조합론과 이어지듯, 특이점 쪽 정보는 KL 쪽 조합론과 이어진다. 두 조합론이 같은 다양체의 서로 다른 측면을 본다.

## 정준기저의 원형

양자군 $U_q(\mathfrak g)$ 의 정준기저(Lusztig), 결정기저(Kashiwara), KLR 대수의 범주화가 모두 "자기쌍대성과 삼각성으로 기저를 유일하게 고정한다" 는 KL 의 도식을 따른다. 그 기저들의 구조상수가 음이 아닌 정수라는 사실도 같은 방식으로, 즉 적절한 범주에서 대상의 차원으로 실현해 증명된다. KL 다항식은 이 방법론이 처음 작동한 자리다.

## 범주화라는 관점

`P_{x,w}` 를 수의 열이 아니라 벡터공간의 열로 보는 것이 Soergel 쌍가군의 범주다. 다항식의 등식이 대상의 동형으로 올라가고, 계수의 양수성이 "차원은 음이 아니다" 라는 자명한 사실로 환원된다. 등식을 동형으로 올려 문제를 쉽게 만드는 이 전략이 현대 표현론의 표준 도구가 되었으며, Khovanov 호몰로지가 Jones 다항식에 대해 하는 일과 같은 종류의 상승이다.
...
[^3]: G. Williamson, *Schubert calculus and torsion explosion*, J. Amer. Math. Soc. **30** (2017), 1023–1046. 부록에 Kontsevich 의 관련 계산이 실려 있다. 본문의 패턴 회피 계산은 직접 한 것이고, 매끄러운 순열의 개수가 큰 Schröder 수라는 사실은 Haiman 과 Bóna 의 결과다.
