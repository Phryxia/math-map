...
[동치관계](relations.md)는 집합을 겹치지 않는 조각으로 나눈다. 두 원소를 같은 것으로 보자는 선언이 하나씩 들어와 조각이 합쳐지는 상황에서는 매번 동치류를 다시 계산할 수 없다.
...
임의의 관계에서 반사, 대칭, 추이 폐포를 취하면 가장 작은 동치관계가 된다. 이 자료구조는 그 폐포를 점진적으로 계산하며 동치류의 정규형으로 대표원을 쓴다. [동치관계](relations.md)의 몫 구성이 계산으로 내려오는 자리다.[^1]
