...
$\mathbb P^1$ 과 타원곡선에서 $L(n\cdot P)$ 의 기저를 명시적으로 적을 수 있다. $\mathbb P^1$ 에서는 차수 $n$ 이하의 다항식이고, 타원곡선 $y^2=x^3+ax+b$ 에서는 $x$ 가 $O$ 에서 극 2 위, $y$ 가 3 위이므로 $j\le1$ 이고 $2i+3j\le n$ 인 $x^iy^j$ 가 기저다.
