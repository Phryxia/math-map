...
변분 오토인코더의 해법은 둘이다. 사후분포를 다루기 쉬운 분포족으로 근사하고 그 품질을 [KL divergence](kl-divergence.md)로 재면 $\log p(x)$ 의 하계인 ELBO 가 나오고, 최대가능도가 하계 최대화로 바뀐다. 그 하계를 [경사하강법](gradient-descent.md)으로 올릴 때 기댓값 안에 최적화 대상이 있어 생기는 기울기 추정 문제는 재매개화라는 변수변환이 해결한다.
...
ELBO 가 $\log p(x)$ 의 하계일 뿐 표현 학습의 목적함수가 아닌 것이 원인이다. 대응책으로 KL 항에 가중치를 두거나($\beta$ VAE), KL 에 하한을 두어 일정량의 정보를 강제하거나, 디코더의 표현력을 제한한다. $\beta>1$ 은 잠재변수의 축을 분리해 해석 가능한 표현을 준다.
...
MCMC 는 충분히 오래 돌리면 참 사후분포에 수렴하지만 느리고 수렴 판정이 어렵다. 변분추론은 빠르고 목적함수 값으로 진행을 감시할 수 있지만 분포족의 한계로 참 사후분포에 도달하지 못하고 불확실성을 과소평가한다. $\mathrm{KL}(q\Vert p)$ 의 최소화가 $p$ 가 작은 곳에서 $q$ 도 작기를 요구해 $q$ 가 사후분포의 한 봉우리에 몰리기 때문이다.
