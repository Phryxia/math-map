...
MCMC(Markov chain Monte Carlo)는 충분히 오래 돌리면 참 사후분포에 수렴하지만 느리고 수렴 판정이 어렵다. 변분추론은 빠르고 목적함수 값으로 진행을 감시할 수 있지만 분포족의 한계로 참 사후분포에 도달하지 못하고 불확실성을 과소평가한다. $\mathrm{KL}(q\Vert p)$ 의 최소화가 $p$ 가 작은 곳에서 $q$ 도 작기를 요구해 $q$ 가 사후분포의 한 봉우리에 몰리기 때문이다.
