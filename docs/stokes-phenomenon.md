...
이고, 음의 실축 쪽으로 돌아가면 두 안장점의 기여가 합쳐져 진동하는 꼴이 된다. 그 사이 $\arg z = \pm 2\pi/3$ 에서 두 번째 지수항이 켜진다. "지수적으로 감쇠하는 해" 와 "진동하는 해" 가 같은 함수의 두 영역이라는 사실이 [WKB 근사](laplace-method.md)의 연결 공식이 하는 일이고, Stokes 가 1857 년에 실제로 본 현상이 이것이다.
