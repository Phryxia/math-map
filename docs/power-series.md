...
멱급수(power series)는 다항식의 차수를 무한히 올린 형식적 합이다.

$$
\sum_{n=0}^{\infty} a_n (x - c)^n .
$$

다항식은 [미분](derivative.md)과 [적분](riemann-integral.md)이 항별로 처리되는 대신 지수·삼각·로그 함수를 담지 못한다. 멱급수는 항별 계산을 유지한 채 이 함수들을 표현하고, 그 대가로 수렴하는 영역이 제한된다. 그 영역의 크기가 수렴반경(radius of convergence)이다.

반대 방향으로 함수에서 계수를 뽑은 멱급수가 Taylor 급수다. 무한번 미분가능한 함수는 모두 Taylor 급수를 갖지만, 그 급수의 수렴 여부와 원래 함수와의 일치 여부는 별개의 조건이다. 둘 사이의 간극을 나머지항(remainder)이 재고, 간극이 0 으로 가는 함수가 해석함수(analytic function)다.
...
$$
\frac{1}{R} = \limsup_{n \to \infty} \vert a_n\vert^{1/n}, \qquad R \in [0, \infty] .
$$

역수는 관례적으로 $1/0$ 을 무한대, $1/\infty$ 를 0 으로 읽는다. 계수의 비가 극한을 가지면 더 쉬운 판정도 쓸 수 있다.
...
$c=0$ 인 경우를 Maclaurin 급수라 부른다. 역방향도 성립한다. 멱급수가 어떤 함수를 나타내면 그 계수는 유일하고 위 식으로 복원된다.
...
이다. 첫 포함이 진부분집합인 것이 실해석과 복소해석의 차이다. [정칙함수](holomorphic-functions.md)는 한 번 복소미분 가능하면 해석적이다.
...
**따름정리.** 한 함수를 나타내는 멱급수는 하나뿐이고, 멱급수의 합은 해석함수다. 항별 미분을 $n$ 번 하고 $x=c$ 를 대입하면 $a_n=f^{(n)}(c)/n!$ 이 나온다.

## Abel 정리

**정리.** $\sum a_n R^n$ 이 수렴하면

$$
\lim_{x \to R^-} \sum_{n=0}^{\infty} a_n x^n = \sum_{n=0}^{\infty} a_n R^n .
$$

경계에서의 거동을 다루는 일반 정리는 이것뿐이다. $\log 2$ 의 교대급수 표현과 Leibniz 의 $\pi/4$ 급수가 이 정리로 정당화된다.
...
이 함수는 지지집합이 콤팩트인 매끄러운 함수(bump function)와 단위분할(partition of unity)의 재료이고, [다양체](manifolds.md) 위에서 국소적으로 정의된 대상을 전역적으로 붙이는 데 쓰인다. 해석함수로는 같은 구성을 할 수 없다. 해석함수는 한 점 근방에서 0 이면 연결된 정의역 전체에서 0 이다(항등정리, identity theorem).
...
| $1/(1+x^2)$ | $(-1)^kx^{2k}$ | 1 (복소 특이점 때문) |

지수·삼각 급수는 계수의 분모에 팩토리얼이 있어 $\vert a_n\vert^{1/n}$ 이 0 으로 가고 반경이 무한대다. 이 급수들의 곱셈(Cauchy 곱)은 [생성함수](generating-functions.md)의 조합적 계산과 형식이 같다.

# 활용

## 수치 계산

초월함수의 함숫값은 부분합으로 계산하고 오차는 Lagrange 나머지항으로 상계한다. $\exp$ 의 도함수가 $\exp$ 이므로 $[0,1]$ 에서 $n$ 차 부분합의 오차는 $e/(n+1)!$ 이하이며, 요구 정밀도에서 필요한 항수가 이 상계로 정해진다. 계수의 감소가 느린 급수는 중심을 옮기거나 함수방정식으로 인수를 줄여 쓴다.

## 복소해석, 미분방정식, 조합론

- **[정칙함수](holomorphic-functions.md)**: 복소미분 가능한 함수는 Cauchy 적분 공식으로 멱급수 전개를 얻어 해석적이다. 수렴반경이 중심에서 가장 가까운 특이점까지의 거리라는 것도 이 공식에서 나오며, $1/(1+x^2)$ 의 반경 1 이 그 사례다.
...
- **점근 전개**: 발산하는 급수도 처음 몇 항이 좋은 근사를 준다. Stirling 급수가 그런 예다.[^1]
