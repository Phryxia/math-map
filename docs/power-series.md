...
**정리.** 수렴반경이 $R$ 인 멱급수는 임의의 $r<R$ 에 대해 $|x-c|\le r$ 위에서 [균등수렴](uniform-convergence.md)한다.

**증명 스케치.** $r<\rho<R$ 인 $\rho$ 를 잡으면 $|a_n|\rho^n$ 이 유계이므로 $|a_n|r^n\le M(r/\rho)^n$ 이고, 우변은 등비급수라 수렴한다. Weierstrass M-test 가 균등수렴을 준다.
