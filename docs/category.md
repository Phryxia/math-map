...
모든 집합의 집합이 존재할 수 없듯이 큰 범주를 다룰 때는 크기에 주의해야 한다. 국소적으로 작음을 요구하거나 Grothendieck 우주를 도입하는 것이 표준 처방이며, [ZFC 공리계](zfc-axioms.md)에서 제한한 것과 같은 종류의 문제다.
...
두 범주 사이의 대응인 [functor](functors.md), 그 사이의 비교인 자연변환, 최적 근사 관계인 adjunction 이 이 위에 쌓인다. 대수적 위상수학이 공간의 문제를 대수의 문제로 바꾸는 방식과 프로그래밍 언어가 타입과 함수를 다루는 방식이 이 층위에서 서술된다.[^1]

## 범위

범주론은 계산을 대신하지 않는다. 어떤 구성이 왜 자연스러운지, 무엇이 무엇의 쌍대인지, 어떤 정리가 어떤 정리와 같은 형태인지를 정리한다. 구체적인 계산과 예시 없이 추상만 다루면 내용 없는 형식이 된다.
