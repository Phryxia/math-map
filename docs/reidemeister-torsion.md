...
- [기본군](fundamental-group.md)이 둘 다 $\mathbb Z/7$ 이고,
- 모든 호몰로지군이 같고,
- 실제로 **호모토피 동치**이지만,
- 위상동형이 아니다.
...
$M$ 을 유한 [CW 복합체](cw-complexes.md)(CW complex, closure-finite weak topology), $\rho:\pi_1(M)\to\mathrm{GL}\_n(\mathbb C)$ 를 표현이라 하자. 보편덮개 $\widetilde M$ 의 세포 사슬복합체 $C_\ast(\widetilde M)$ 은 $\mathbb Z[\pi_1]$ 가군이고, 여기에 $\rho$ 를 적용해
...
$L(7,1)$ 과 $L(7,2)$ 는 호모토피 동치이므로 호몰로지, [코호몰로지](cohomology.md) 환, 기본군, 고차 [호모토피군](homotopy-groups.md)이 전부 같다. 비틀림 값의 다중집합은 $\lbrace 0.75,2.45,3.80\rbrace$ 와 $\lbrace 1.36,1.69,3.05\rbrace$ 로 겹치는 값이 없으므로 기저 선택의 애매성으로 맞출 수 없다.
