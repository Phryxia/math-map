...
앞의 것은 호몰로지와 기본군, 연결 합의 불변량으로 판정되지만 뒤의 것은 그렇지 않다. 뒤의 판정을 실제로 해내는 것이 비틀림이다. $p=7$ 과 $q=1$ 과 $q'=2$ 에서 $1\cdot2=2\equiv3^2\pmod7$ 이므로 호모토피 동치이고, 한편 $\pm1^{\pm1}=\{1,6\}$ 에 $2$ 가 없으므로 동형이 아니다.
