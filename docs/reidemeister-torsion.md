...
비틀림은 다양체 하나의 수가 아니라 다양체와 그 기본군 표현의 쌍에 붙는 수다. [Chern–Simons 이론](chern-simons.md)에서 평탄 접속이 $\pi_1$ 의 표현이므로 안장점 근사의 진폭에 비틀림이 나타난다.
...
의 원소다. $\rho$ 가 유니터리이면 $|\det\rho(g)|=1$ 이므로 절댓값 $|\tau|$ 가 잘 정의된다. 아래 계산은 절댓값을 쓴다.
...
$L(7,1)$ 의 값 $0.7530,\thinspace 2.4450,\thinspace 3.8019$ 은 $4\sin^2(2\pi b/7)$ 을 $b=1,2,3$ 에서 계산한 값이다. 렌즈 공간의 RT 불변량을 Gauss 합 상호법칙으로 분해했을 때 각 평탄 접속의 진폭으로 나오는 수와 같다. 점근의 진폭이 비틀림이라는 진술이 이 일치다.
