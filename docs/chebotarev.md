...
`f=x^3-x-1` 은 기약이고 판별식이 `-23` 이라 분해체의 Galois 군이 `S_3` 다. 켤레류는 항등원 1 개, 호환 3 개, 3-순환 2 개이므로 밀도가 각각 `1/6,\ 1/2,\ 1/3` 이어야 한다.

법 `p` 에서의 분해형은 Frobenius 의 사이클형과 같다. 세 근을 다 가지면 완전분해, 근이 하나면 일차와 기약 이차의 곱, 근이 없으면 기약 삼차다. 근의 개수는 $\gcd(x^p-x,f)$ 의 차수로 빠르게 얻는다.
...
F = [-1, -1, 0, 1]                      # f = x³ - x - 1, 판별식 -23, Galois 군 S₃

def mulmod(a, b, p):
    """F_p[x]/(f) 의 곱. x³ = x + 1 로 줄인다."""
    r = [0]*5
    for i in range(3):
        for j in range(3): r[i+j] = (r[i+j] + a[i]*b[j]) % p
    for k in (4, 3):
        c, r[k] = r[k], 0
        r[k-3] = (r[k-3] + c) % p
        r[k-2] = (r[k-2] + c) % p
...
def polymod(a, b, p):
    ...
    while len(a) >= len(b) and a != [0]:
        c, d = a[-1]*pow(b[-1], -1, p) % p, len(a) - len(b)
        for i in range(len(b)): a[d+i] = (a[d+i] - c*b[i]) % p
        a = trim(a)
    return a

def num_roots(p):
    """deg gcd(x^p - x, f) = f 가 F_p 에서 갖는 서로 다른 근의 개수."""
    a = frobenius(p); a[1] -= 1                        # x^p - x
    a, b = F[:], trim([x % p for x in a])
    while b != [0]:
        a, b = b, polymod(a, b, p)
    return len(trim(a)) - 1
...
for p in primes(200000):
    if p == 23: continue                               # 분기하는 유일한 소수
    cnt[num_roots(p)] += 1

tot = sum(cnt.values())
rows = [(3, "완전분해 (1,1,1)", "항등원",        1, 1/6),
        (1, "일차+이차 (1,2)",  "호환 (2-순환)", 3, 1/2),
        (0, "기약 (3)",         "3-순환",        2, 1/3)]
print(f"f = x³ - x - 1 의 법 p 분해형 분포  (p < 200000, 소수 {tot} 개)")
print("  분해형              Frobenius 켤레류   |C|   관측     |C|/|G|")
for r, name, cls, size, e in rows:
    print(f"  {name:<18} {cls:<14} {size}   {cnt[r]/tot:.4f}   {e:.4f}")

# f = x³ - x - 1 의 법 p 분해형 분포  (p < 200000, 소수 17983 개)
#   분해형              Frobenius 켤레류   |C|   관측     |C|/|G|
#   완전분해 (1,1,1)       항등원            1   0.1645   0.1667
#   일차+이차 (1,2)        호환 (2-순환)      3   0.5015   0.5000
#   기약 (3)             3-순환           2   0.3339   0.3333
```

세 비율이 `1/6,\ 1/2,\ 1/3` 에 잘 붙는다. 여기서 흥미로운 점은 관측값이 켤레류의 **크기**에 비례하지 원소의 위수나 분해형의 복잡도와는 무관하다는 것이다. 소수가 Galois 군의 균등표본처럼 행동한다는 주장의 직접적인 확인이다.

완전분해 쪽의 오차가 상대적으로 큰 것도 예상된 일이다. 밀도가 작은 켤레류일수록 표본이 적어 수렴이 느리고, 효과적 Chebotarev 의 오차항이 `|C|/|G|` 에 비례하는 절대오차를 주므로 상대오차는 작은 류에서 커진다.
