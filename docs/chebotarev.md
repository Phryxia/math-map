...
$L/K$ 가 아벨이면 Frobenius 가 광선류군의 원소가 된다. 유체론이 그 대응을 동형으로 만들므로 각 유류에 소수가 균등하게 들어간다는 주장은 각 Dirichlet 지표 $\chi\ne1$ 에 대해 $L(1,\chi)\ne0$ 이라는 사실과 같다. 소수의 분포가 한쪽으로 쏠리면 그 쏠림이 $L$ 함수의 $s=1$ 에서의 영점으로 나타난다.
...
이 갈림은 $\mathbb Q(\sqrt{-23})$ 의 류수가 3 이라는 사실과 연결된다. 그 힐베르트 유체가 $L$ 이고, 완전분해하는 소수가 주 아이디얼에 대응한다.
