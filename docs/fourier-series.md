...
주기함수를 "순수한 진동의 합"으로 분해한다는 발상은 물리적이다. 서로 다른 진동수의 삼각함수들이 적분 내적에 대해 직교하므로 이 분해가 성립한다. 직교하는 방향들로 벡터를 분해하는 일은 유한차원에서 익숙하고, 무한차원에서 그 일을 가능하게 하는 것이 [완비성](completeness.md)이다.
...
$N$ 항 부분합은 Dirichlet 핵과의 합성곱이고, 이 핵은 절댓값 적분이 $\log N$ 처럼 커진다. 그러므로 부분합 연산자는 균등하게 유계가 아니고, 점별 수렴의 병리는 이 비유계성의 결과다.
...
완비성 증명의 표준 경로는 Fejér 정리다. 연속 주기함수에 대해 Cesàro 평균이 균등수렴하므로 삼각다항식이 [연속함수](continuity.md) 공간에서 조밀하고, 연속함수가 $L^2$ 에서 조밀하므로 삼각함수계에 직교하는 벡터는 0뿐이다. 따라서 Parseval은 Hilbert 공간의 일반론에서 자동으로 따라온다.
...
표본을 유한개만 쓰면 적분이 유한합이 되고, 그것이 [이산 Fourier 변환](fourier.md)이다. 실제 계산은 거의 언제나 이 유한 버전과 FFT(fast Fourier transform)로 한다. 주기함수를 [멱급수](power-series.md)로 전개하는 경우와는 다르다. 멱급수는 해석적 함수에만 쓸 수 있고 수렴 반경 안에서만 유효하지만, Fourier 급수는 매끄럽지 않은 함수, 불연속 함수에도 적용되며 전 구간에서 평균적으로 수렴한다.
