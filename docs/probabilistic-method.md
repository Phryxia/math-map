...
나머지 절반은 "나쁜 사건이 일어날 확률이 1보다 작다"는 형태다. 원하는 성질이 깨지는 방식을 나쁜 사건들 $A_1,\dots,A_m$ 으로 나열하고
...
$n$ 이 $k^22^k$ 정도면 조건이 만족되므로, "아무리 강한 $k$ 명을 뽑아도 그들을 모두 이긴 사람이 있는" 토너먼트가 존재한다. 직관을 정면으로 거스르는 결론이고, 확률적 방법이 아니면 세우기 어렵다.
