...
$n$ 개의 대상에 점수를 매겼을 때 평균이 $\mu$ 라면 점수가 $\mu$ 이상인 대상이 반드시 존재한다. 이것이 [비둘기집 원리](pigeonhole-principle.md)의 평균 버전이고, 확률적 방법의 절반은 이 관찰이다. 무작위 대상의 기댓값을 계산하는 것이 개별 대상을 분석하는 것보다 훨씬 쉬울 때가 많다. 기댓값은 선형이므로 의존성을 무시하고 항별로 더하면 되기 때문이다.
...
이 증명에 나오는 그래프는 단 하나도 명시되지 않는다. 그럼에도 70년 넘게 이 하한을 본질적으로 개선한 명시적 구성이 없다. 상한 쪽은 $R(k,k) \le 4^k$ 수준이며 최근에야 지수의 밑이 4보다 엄밀히 작아졌다.
