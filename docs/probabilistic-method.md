...
이 논증이 그대로 $1/2$ [근사 알고리즘](approximation-algorithms.md)이고 아래의 탈무작위화로 결정적 알고리즘이 된다. 최대 절단은 [네트워크 흐름](network-flow.md)의 최소 절단과 달리 NP-난해이며([NP-완전성](np-completeness.md)), 알려진 최선의 근사비는 준정부호 계획법으로 얻는 0.878 이다.
