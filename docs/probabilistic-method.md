...
이 문서는 기본 원리에서 시작해 Erdős의 Ramsey 하한, 기댓값 논법(최대 절단, 토너먼트), 변경 논법(alteration), Lovász local lemma를 다루고, 비구성성과 탈무작위화(derandomization)를 정리한다.

# 직관

"평균보다 좋은 것이 하나는 있다"가 전부다.

`n`개의 대상에 점수를 매겼을 때 평균이 `μ`라면 점수가 `μ` 이상인 대상이 반드시 존재한다. 이것이 [비둘기집 원리](pigeonhole-principle.md)의 평균 버전이고, 확률적 방법의 절반은 이 관찰이다. 무작위 대상의 기댓값을 계산하는 것이 개별 대상을 분석하는 것보다 훨씬 쉬울 때가 많다. 기댓값은 선형이므로 의존성을 무시하고 항별로 더하면 되기 때문이다.

나머지 절반은 "나쁜 사건이 일어날 확률이 1보다 작다"는 형태다. 원하는 성질이 깨지는 방식을 나쁜 사건들 `A_1, …, A_m` 으로 나열하고

$$
\Pr\left[\bigcup_i A_i\right] \le \sum_i \Pr[A_i] < 1
$$

을 보이면 아무 나쁜 사건도 일어나지 않는 결과가 양의 확률로 존재한다. 합집합 상한(union bound) 하나로 대부분의 고전적 결과가 나온다.

union bound가 실패할 때(나쁜 사건이 너무 많을 때)는 두 갈래 길이 있다. 하나는 **변경 논법**이다. 나쁜 사건을 완전히 피하는 대신 조금 일어나게 두고, 사후에 손으로 고친다. 다른 하나는 **Lovász local lemma** 다. 나쁜 사건들이 서로 거의 독립이면 각각의 확률이 아주 작지 않아도 모두 피할 수 있다.
...
유한 [확률 공간](probability.md) `(Ω, Pr)` 과 사건 `A ⊆ Ω` 에 대해

$$
\Pr[A] > 0 \ \Longrightarrow\ A \ne \varnothing
$$

이다. 조합론에서 `Ω`는 보통 대상들의 유한 집합(모든 그래프, 모든 색칠, 모든 부분집합)이고 `A`는 원하는 성질을 만족하는 것들의 집합이다.

## 기댓값 논법

[확률변수](random-variables.md) `X : Ω → ℝ` 에 대해
...
사건들 `A_1, …, A_m` 에 대한 **의존성 그래프**는 정점이 사건이고, 각 `A_i` 가 자신과 인접하지 않은 사건들의 모임 전체와 서로 독립이 되도록 만든 그래프다. 이 그래프의 최대차수를 `d`라 한다.
...
이다. 그런 `S`는 `C(n, k)` 개이므로 union bound에 의해 단색 클리크가 하나라도 생길 확률은 `C(n,k) · 2^{1 - C(k,2)}` 이하다. 이 값이 1보다 작으면 단색 클리크가 전혀 없는 색칠이 양의 확률로 존재한다. `C(n,k) ≤ n^k / k!` 와 Stirling 근사를 쓰면 `n = ⌊2^{k/2}⌋` 근처에서 조건이 성립한다. ∎

이 증명에 나오는 그래프는 단 하나도 명시되지 않는다. 그럼에도 70년 넘게 이 하한을 본질적으로 개선한 명시적 구성이 없다. 상한 쪽은 `R(k,k) ≤ 4^k` 수준이며 최근에야 지수의 밑이 4보다 엄밀히 작아졌다.

## 기댓값 논법: 최대 절단

**정리.** 간선이 `m`개인 모든 그래프는 크기가 `m/2` 이상인 절단을 가진다.

**증명.** 각 정점을 독립적으로 확률 `1/2` 로 `S` 또는 `T`에 넣는다. 간선 `e = uv` 에 대해 지표변수 `X_e` 를 `u`와 `v`가 다른 쪽에 있을 때 1로 두면 `E[X_e] = 1/2` 다. 절단 크기 `X = Σ_e X_e` 이고 선형성에 의해 `E[X] = m/2`. 따라서 `X ≥ m/2` 인 분할이 존재한다. ∎

이 논증은 그대로 `1/2`-근사 알고리즘이고, 아래의 탈무작위화로 결정적 알고리즘이 된다. 참고로 최대 절단은 [네트워크 흐름](network-flow.md)의 최소 절단과 달리 NP-난해이며([NP-완전성](np-completeness.md)), 최선의 근사비는 준정부호 계획법으로 얻는 0.878이다.

## 기댓값 논법: 토너먼트

토너먼트는 완전 그래프의 각 간선에 방향을 준 것이다. 토너먼트가 성질 `S_k` 를 가진다는 것은, 임의의 `k`명 선수 집합에 대해 그들 모두를 이긴 선수가 존재한다는 뜻이다.

**정리 (Erdős).** `C(n, k) (1 - 2^{-k})^{n-k} < 1` 이면 `S_k` 를 만족하는 `n`명 토너먼트가 존재한다.
...
`n`이 `k^2 2^k` 정도면 조건이 만족되므로, "아무리 강한 `k`명을 뽑아도 그들을 모두 이긴 사람이 있는" 토너먼트가 존재한다. 직관을 정면으로 거스르는 결론이고, 확률적 방법이 아니면 세우기 어렵다.
...
**증명.** 각 정점을 독립적으로 확률 `p`로 뽑아 집합 `S`를 만든다. `E[|S|] = pn` 이고, 양 끝이 모두 `S`에 든 간선 수의 기댓값은 `p^2 m` 이다. 이제 그런 간선마다 끝점 하나를 **제거**한다. 남은 집합 `S'` 은 독립집합이고

$$
\mathbb{E}[|S'|] \ge pn - p^2 m
$$

이다. 우변을 `p`에 대해 최대화하면 `p = n/(2m)` 에서 `n^2/(4m)` 을 얻고, `p ≤ 1` 조건을 고려해 정리하면 `n/(d+1)` 형태가 나온다. ∎

여기서 "제거"가 변경 논법이다. 완벽한 대상을 한 번에 뽑는 대신, 결함이 적은 대상을 뽑고 결함을 제거하는 비용을 기댓값으로 계산한다. 같은 기법으로 "짧은 사이클도 없고 독립수도 작은 그래프"의 존재, 즉 국소적으로는 나무처럼 보이지만 채색수가 큰 그래프의 존재가 증명된다([그래프 색칠](graph-coloring.md)). 이 결과는 명시적 구성이 오랫동안 없었던 대표적 사례다.

## Lovász local lemma

나쁜 사건이 매우 많아 `Σ Pr[A_i] ≥ 1` 이 되더라도, 각 사건이 소수의 다른 사건에만 의존한다면 전부 피할 수 있다.

**정리 (대칭형 LLL; Erdős–Lovász, 1975).** 사건 `A_1, …, A_m` 이 각각 `Pr[A_i] ≤ p` 를 만족하고, 의존성 그래프의 최대차수가 `d`이며
...
**비대칭형.** 실수 `x_i ∈ (0,1)` 들이 있어 각 `i`에 대해
...
**응용 1 (`k`-SAT).** 각 절이 정확히 `k`개의 리터럴을 갖고, 각 절이 다른 절과 변수를 공유하는 횟수가 `2^k / (e k)` 이하인 CNF 식은 항상 충족 가능하다. 변수를 독립적으로 균등하게 배정하면 절 하나가 거짓일 확률이 `2^{-k}` 이고 LLL 조건이 위 형태가 된다.

**응용 2 (초그래프 2-색칠).** 모든 간선의 크기가 `k`이고 각 간선이 다른 간선과 `2^{k-1}/e - 1` 개 이하로 만나는 `k`-균등 초그래프는 2-색칠 가능하다(단색 간선이 없게).

**응용 3 (Ramsey 하한 개선).** LLL을 쓰면 `R(k,k) > c k 2^{k/2}` 로 상수 인자가 개선된다.

원래 증명은 존재만 주장하고 해를 찾아주지 않는다. Moser–Tardos의 무작위 재배정 알고리즘[^2]이 LLL 조건 아래에서 기댓값 다항 시간에 실제 해를 찾음을 보이면서 이 부분이 해결되었다. 알고리즘은 놀랄 만큼 단순하다. 위반된 절을 아무거나 골라 그 절의 변수들만 다시 무작위로 뽑는 것을 반복한다. 분석은 entropy 압축 논법으로 한다.

## 비구성성과 탈무작위화

확률적 방법의 결론은 기본적으로 비구성적이다. "존재한다"만 알고 "이것이다"는 모른다. 세 가지 대응이 있다.
...
3. **표본 공간 축소**: 완전 독립 대신 `k`-wise 독립 확률변수를 쓰면 표본 공간이 다항 크기로 줄어 전수 탐색이 가능해진다. 증명이 저차 모멘트만 쓴다면 이 치환이 통한다.

명시적 구성이 확률적 존재 증명을 따라잡지 못한 간극 자체가 연구 주제다. Ramsey 그래프, 확장 그래프(expander), 오류 정정 부호가 대표적이며, 확장 그래프는 Margulis와 Lubotzky–Phillips–Sarnak 이후 명시적 구성이 확률적 경계에 근접했다.

# 활용

## 코드

최대 절단에 대한 무작위 논증과 그 탈무작위화(조건부 기댓값)를 나란히 확인한다.
...
Erdős의 Ramsey 하한 조건을 직접 계산하면 존재가 보장되는 최대 `n`을 얻는다.
...
변경 논법으로 얻는 독립집합 하한도 탐욕 알고리즘으로 달성된다.
...
실전에서 확률적 방법을 쓸 때의 순서는 일정하다. 첫째, 무작위 대상을 어떻게 뽑을지 명확히 정한다(분포 선택이 증명의 절반이다). 둘째, 실패 사건을 나열하고 각각의 확률을 계산한다. 셋째, union bound로 충분하면 끝이고, 부족하면 기댓값 논법으로 목표를 약화하거나, 변경 논법으로 사후 수리를 하거나, 의존 구조가 희박하면 LLL을 쓴다. 넷째, 구성적 해가 필요하면 조건부 기댓값이나 Moser–Tardos로 탈무작위화한다.
