...
위수 $|G|=p^am$ 에서 $p$ 가 $m$ 을 나누지 않으면 $p^a$ 이 $p$ 쪽에서 가능한 최대치이고, Sylow 정리는 이 최대치가 부분군으로 실현된다고 말한다.

증명은 $p$ 군이 집합에 작용할 때 고정점의 개수가 집합 크기와 법 $p$ 로 합동이라는 사실에 의존한다. 궤도의 크기가 $p$ 의 거듭제곱이므로 크기 $1$ 이 아닌 궤도는 모두 $p$ 의 배수여서 세기에서 지워진다. 합성수 위수에는 이 상쇄가 없다.

## 켤레와 정규성

Sylow 부분군들은 서로 켤레이므로 Sylow $p$ 부분군이 하나뿐인 것과 그것이 정규부분군인 것이 동치다. $n_p=1$ 을 강제할 수 있으면 비자명한 정규부분군이 생겨 단순성이 깨지므로, 단순군 판정이 Sylow 정리로 돌아간다.

## 세기 조건

세 번째 정리의 두 조건 $n_p \mid m$ 과 $n_p \equiv 1 \pmod p$ 는 함께 강하다. 위수 15 에서는 두 조건이 $n_3 = n_5 = 1$ 을 강제해 군이 하나뿐임이 나온다. 후보가 여럿 남으면 서로 다른 Sylow 부분군이 단위원 말고는 겹치지 않는다는 사실을 더해 원소를 센다.
...
#group_theory #algebra #number_theory #theorem
