...
단순 가설검정에서 오류 확률은 표본 크기에 대해 지수적으로 감소하고 그 지수가 KL divergence 다(Stein 보조정리). 경험분포가 어떤 집합에 들어갈 확률의 지수 감쇠율을 주는 Sanov 정리도 같은 양을 쓴다. [가설검정](hypothesis-testing.md)과 대편차 이론이 이 양을 단위로 삼는다.
