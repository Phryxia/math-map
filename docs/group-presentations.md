...
- Coxeter 군은 $\langle s_1,\dots,s_n\mid(s_is_j)^{m_{ij}}\rangle$ 꼴의 표시로 정의되며, [근계와 Weyl 군](root-systems.md)의 Weyl 군이 그 예다.
- 계산 군론의 Todd–Coxeter 알고리즘은 표시와 부분군이 주어졌을 때 잉여류를 하나씩 채워 지표를 구한다. 지표가 유한하면 반드시 끝나지만, 무한하면 끝나지 않으며 낱말 문제의 결정불가능성이 그 이유다.
