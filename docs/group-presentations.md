...
증명은 자유군의 보편성질로 $F(S)\to H$ 를 얻은 뒤, 가정에서 그 핵이 $R$ 을 포함하고 핵이 정규부분군이므로 $\langle\langle R\rangle\rangle$ 을 포함함을 보는 것이다. 몫의 보편성질이 나머지를 준다.
...
위상적 증명이 짧다. 계수 $n$ 의 자유군은 꽃다발 [그래프](graphs.md)의 [기본군](fundamental-group.md)이고, 부분군은 덮개공간의 기본군이며, 그래프의 덮개는 다시 그래프다. 그래프의 기본군은 자유군이고, 지표 $k$ 의 덮개는 간선이 $kn$ 개 정점이 $k$ 개이므로 계수가 $kn-k+1$ 이다.
