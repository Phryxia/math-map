...
$\mathbf{Set}$ 이 아닌 토포스를 고르면 전사사상이 갈라지지 않는 예가 나온다. 완비 [Boolean 대수](boolean-algebras.md) 위의 층 토포스를 잡으면 [강제법](forcing.md)의 [Boolean 값 모형](boolean-valued-models.md)이 토포스의 말로 다시 쓰이고, [연속체 가설](continuum-hypothesis.md)의 독립성 증명이 층의 구성으로 옮겨진다.
