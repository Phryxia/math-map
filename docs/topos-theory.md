...
토포스는 집합의 범주가 가진 구조를 공리로 뽑아낸 범주다. 유한 [제한](limits-colimits.md), 지수 대상, 부분대상 분류자 셋을 갖추면 토포스이고, 그 안에서 집합론의 구성 대부분을 그대로 할 수 있다.
...
다른 범주에서 같은 자리를 차지하는 대상을 찾으면 그 범주가 자기만의 진리값 대상을 갖는다. 위상공간 위의 [층](sheaves.md) 범주에서는 그 대상의 점이 두 개가 아니라 열린집합들의 격자이고, 거기서 나오는 논리가 배중률을 만족하지 않는다.
