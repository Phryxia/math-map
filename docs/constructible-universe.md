...
구성가능 우주 $L$ 은 각 단계에서 [1차 논리](first-order-logic.md)식으로 정의되는 부분집합만 취해 초한 재귀로 쌓은 모임이다. ZFC(Zermelo–Fraenkel set theory with choice) 의 모든 공리를 만족하는 내부 모형이고, 모든 집합이 구성가능하다는 명제 $V = L$ 이 그 안에서 성립한다.
