...
$L$ 에서는 선택공리와 일반 연속체 가설이 정리다. ZFC 가 무모순이면 여기에 이 둘을 더해도 무모순임이 이 모형에서 나온다.
...
$\kappa$ 위에 $\kappa$ 완비인 비주요 초필터가 있으면 $\kappa$ 를 측도 가능 기수라 하고, 그 초필터로 만든 초곱이 $V$ 에서 내부 모형으로 가는 **기본 매장**을 준다. 이 매장을 $L$ 에 제한하면 $L$ 의 정의 가능한 정렬순서와 모순되는 임계점이 생긴다. ∎
