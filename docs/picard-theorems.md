...
$f$ 가 뚫린 원판 $0\lt\vert z-a\vert\lt r$ 에서 정칙이고 $a$ 에서의 Laurent 전개에 음의 차수 항이 무한히 많으면 $a$ 를 $f$ 의 **본질적 특이점**이라 한다. $e^{1/z}$ 의 $z=0$ 이 그런 점이다.
...
증명은 귀류법이다. 어떤 $w$ 와 $\varepsilon\gt 0$ 에 대해 상이 $\vert f(z)-w\vert\ge\varepsilon$ 를 만족하면 $g=1/(f-w)$ 가 유계 정칙이므로 $a$ 에서 제거 가능한 특이점을 갖고, 되돌리면 $f$ 는 $a$ 에서 극이거나 정칙이다.

## Montel 정리

콤팩트 집합마다 균등유계인 정칙함수족은 정규족이다. Cauchy 추정으로 도함수가 균등유계임을 얻어 동등연속성을 보이고 [Arzelà–Ascoli 정리](arzela-ascoli.md)를 쓴다.

확장된 형태가 Picard 정리의 증명에 쓰인다. 서로 다른 두 값 $\alpha,\beta$ 를 모두 피하는 $\Omega$ 위의 정칙함수족은 $\widehat{\mathbb C}$ 값 함수로서 정규족이다[^1].
