...
Alexander 다항식은 무한순환덮개의 [호몰로지](homology.md)라는 해석이 있지만 Jones 다항식에는 그런 기하적 해석이 없었고, 볼륨 추측이 그 자리의 후보다.
...
$2\pi\log J_N/N$ 은 참값보다 $2\pi\cdot\tfrac32\log N/N$ 만큼 크다. 이 항은 $0$ 으로 가지만 $\log N/N$ 이라 느리다. $\log J_N$ 에서 $\tfrac32\log N$ 을 빼면 감소 속도가 정련된 판본의 $O(1/N)$ 과 맞는다.
