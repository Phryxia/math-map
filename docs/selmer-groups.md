...
이고 $457\equiv1\pmod8$ 이므로 $457$ 은 $\mathbb Z_2^\times$ 에서 제곱이다. $\mathbb R$ 점은 $x>17^{1/4}\approx2.03$ 에서 있다. 그러므로 $C$ 는 어디서나 국소적으로 풀린다. 대역 점이 없다는 증명은 $\mathbb Q(\sqrt{17})$ 로 올라가 4차 잉여 상호법칙을 쓴다.
