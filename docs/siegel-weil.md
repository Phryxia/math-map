...
$L$ 을 이차형식 $Q$ 가 얹힌 [격자](lattices.md)라 하자. $L$ 의 **류**는 모든 자리에서 국소적으로 동치인 격자들의 $\mathbb Z$ 동치류 모임이고 유한개다. 류 안의 대표를 $L_1,\dots,L_h$ 라 할 때 **질량**은
...
- **격자 이론과 부호 이론.** 질량 공식은 주어진 판별식과 차원에서 격자의 규모를 미리 알려주므로 분류에 쓰인다. Niemeier 의 24 개 분류와 자기쌍대 부호의 분류가 이 계산을 썼다.
