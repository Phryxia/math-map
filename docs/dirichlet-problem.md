...
구멍 뚫린 원판 $0\lt \vert z\vert\lt 1$ 의 원점은 비정칙이다. 경계 성분이 한 점이므로 위 평면 조건이 깨진다. $n\ge 3$ 에서는 경계 성분이 한 점이 아니어도 비정칙일 수 있고, 꼭짓점에서 지수적으로 가늘어지는 뾰족한 구멍인 Lebesgue 의 가시가 그 예다.[^1] 정칙성을 완전히 판정하는 것은 용량으로 쓴 Wiener 기준이다.[^2]
