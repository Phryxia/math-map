...
매끄러운 사상 $F:N\to M$ 은 당김으로 $F^*:H^k(M)\to H^k(N)$ 을 유도한다. 화살표 방향이 뒤집히므로 반변 함자다. 호모토픽한 두 사상은 같은 준동형을 유도한다.
...
de Rham 정리의 따름으로 삼각분할로 센 값과 일치하므로 $\chi$ 는 조합적으로도 해석적으로도 계산된다. Gauss–Bonnet 정리와 지표 정리들이 이 다리를 건넌다.
