...
콤팩트 다양체의 코호몰로지는 유한차원이고 Mayer–Vietoris 로 계산된다. 실계수이므로 비틀림을 보지 못하고 기본군의 비가환성도 대부분 잃는다. $H^1$ 은 $\pi_1$ 의 가환화의 정보만 담는다. 더 정밀한 불변량은 정수 계수 코호몰로지와 [호모토피군](homotopy-groups.md)이 준다.
