...
de Rham 정리는 이 공간이 [단체 호몰로지](homology.md)로 계산한 위상적 코호몰로지와 동형이라고 말한다. 매끄러운 구조와 미분으로 정의한 대상이 위상 불변량이 되며, [지표 정리](index-theorem.md)들의 원형이다.
...
$k$ 형식은 $k$ 차원 부분다양체 위에서 적분되고, Stokes 정리로 완전형식은 경계 없는 순환 위에서 0 이며 닫힘형식은 경계 위에서 0 이다. 적분이 코호몰로지와 호몰로지 사이의 쌍대 짝짓기를 정의하고, 이것이 de Rham 정리의 내용이다.
...
매끄러운 사상 $F:N\to M$ 은 당김으로 $F^\ast:H^k(M)\to H^k(N)$ 을 유도한다. 화살표 방향이 뒤집히므로 반변 [함자](functors.md)다. 호모토픽한 두 사상은 같은 준동형을 유도한다.
...
de Rham 정리의 따름으로 삼각분할로 센 값과 일치하므로 $\chi$ 는 조합적으로도 해석적으로도 계산된다. [Gauss–Bonnet 정리](gauss-bonnet.md)와 지표 정리들이 이 일치를 쓴다.
...
콤팩트 다양체의 코호몰로지는 유한차원이고 Mayer–Vietoris 로 계산된다. 실계수이므로 비틀림을 보지 못하고 [기본군](fundamental-group.md)의 비가환성도 대부분 잃는다. $H^1$ 은 $\pi_1$ 의 가환화의 정보만 담는다. 더 정밀한 불변량은 정수 계수 코호몰로지와 [호모토피군](homotopy-groups.md)이 준다.
...
- de Rham 복합체는 상수층의 분해이므로 이 코호몰로지는 [층](sheaves.md) 코호몰로지의 한 경우다. 이 관점이 대수기하, 복소해석기하, 위상을 하나의 틀로 묶는다.
