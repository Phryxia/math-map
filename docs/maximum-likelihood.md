...
평균이 알려진 경우와 달리 평균을 데이터에서 추정하면 자유도 하나를 잃는다. 그래서 분산의 MLE 는 $n-1$ 이 아니라 $n$ 으로 나눈 값이고 편향되어 있다. 한편 평균만 모르는 정규 모형에서 MLE를 구하는 문제는 제곱합 최소화와 같으므로, 최소제곱법은 정규 오차 가정 아래의 MLE다.
