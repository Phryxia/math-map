...
따라서 부분군의 위수는 전체 위수의 약수다. 한 원소가 생성하는 순환부분군에 적용하면 원소의 위수도 $|G|$ 의 약수이고, $a^{|G|}=e$ 가 따라 나온다. 이 따름정리가 [Fermat 소정리와 Euler 정리](fermat-euler-theorem.md)이며, 위수가 소수인 군은 진부분군이 없어 순환군일 수밖에 없다는 결론도 따라온다. 역은 성립하지 않는다. 약수마다 부분군이 있어야 하는 것은 아니고, 있는지를 부분적으로 보장하는 것이 Sylow 정리다.
