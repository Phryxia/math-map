...
집합 `G` 와 이항연산 `G × G → G` 가 주어졌다고 하자. 연산은 곱으로 표기한다. 임의의 `a, b, c ∈ G` 에 대해 다음이 성립하면 `G` 는 군이다.
...
부분집합 `H ⊆ G` 가 같은 연산으로 다시 군이면 부분군이라 하고 `H ≤ G` 로 쓴다. 비어 있지 않은 `H` 에 대해 다음 한 줄이 부분군 판정의 전부다.
...
두 군 사이의 함수 `φ : G → H` 가 `φ(ab) = φ(a)φ(b)` 를 만족하면 준동형이다. 연산을 보존하므로 항등원과 역원도 자동으로 보존한다. 준동형의 핵과 상은 다음과 같다.
...
`H ≤ G` 와 `a ∈ G` 에 대해 `aH = { ah : h ∈ H }` 를 왼쪽 잉여류라 한다. 잉여류들은 `G` 를 빈틈없이 겹치지 않게 덮으며, 이것이 `a ∼ b ⟺ a^{-1}b ∈ H` 라는 동치관계의 동치류다.
...
`G` 가 유한군이고 `H ≤ G` 이면 모든 잉여류의 크기가 `|H|` 로 같고 서로 겹치지 않으므로 다음이 성립한다.
...
준동형 `φ : G → H` 의 핵은 항상 정규부분군이고, 상은 `H` 의 부분군이며, 다음 동형이 성립한다.
...
한 원소가 전체를 생성하는 군을 순환군이라 한다. 무한 순환군은 정수의 덧셈군과, 위수 `n` 인 순환군은 `Z/nZ` 와 동형이다. 순환군은 모두 가환이지만 역은 거짓이고, `D_4` 는 가환도 순환도 아니다. 유한생성 가환군은 순환군들의 직접곱으로 유일하게 분해된다.

## Cayley 정리

모든 군 `G` 는 자기 자신 위의 치환군의 부분군과 동형이다. `a ∈ G` 를 "왼쪽에서 `a` 를 곱하는 함수" 로 보내면 이 함수는 소거 가능성 때문에 전단사이고, 대응은 단사 준동형이 된다. 추상적인 공리로 정의한 군이 결국 "무언가를 섞는 방법들" 이라는 처음의 직관과 정확히 일치한다는 뜻이다. 이 관점을 일반화한 것이 [군 작용](group-actions.md)이다.
...
정수의 나머지 연산은 덧셈에 대해 순환군 `Z/nZ` 를, 가역인 나머지들은 곱셈에 대해 군 `(Z/nZ)^×` 를 이룬다. 위수가 `φ(n)` 이라는 사실과 Lagrange 정리에서 Euler 정리가 나오고, 이것이 RSA 의 복호화가 왜 원래 메시지를 돌려주는지를 설명한다. 유한체의 곱셈군이 순환군이라는 사실은 이산로그 기반 암호의 바탕이 된다.
