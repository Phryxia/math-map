...
$M\lt N$ 이 정수이고 $f$ 가 $[M,N]$ 에서 $2K$ 번 연속미분가능하면
...
이므로 $f$ 와 $f+e^{-x}$ 는 같은 점근전개를 갖는다. 전개가 놓치는 $e^{-x}$ 같은 항이 **지수적으로 작은 항**이고, 최적 절단에서 남는 오차 $e^{-2\pi N}$ 이 그것이다. 이런 항이 켜지고 꺼지는 현상이 [Stokes 현상](stokes-phenomenon.md)이고, 나머지항 적분이 그 크기를 준다.
