...
수학적으로는 두 확률분포 $\mu,\nu$ 가 주어졌을 때 $\mu$ 를 $\nu$ 로 옮기는 최소 비용을 묻는 것이다. 옮기는 방식을 사상 $T$ 로 쓰면 조건이 [상측도](pushforward-measure.md) 등식 $T_{\char"23{}}\mu=\nu$ 가 된다. 그런데 이 형태의 문제는 해가 아예 없을 수 있다. 한 점에 뭉친 질량을 두 곳으로 나눠야 하는 상황에서 사상은 무력하다.
