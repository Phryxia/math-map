...
다항식환은 환 $R$ 의 원소를 계수로 갖는 형식적 다항식 전체가 이루는 환이다. 계수가 [체](fields.md)일 때 다항식환은 나눗셈 정리를 가지므로 유클리드 정역이 되고, 그 결과 [소수와 유일분해](primes.md)에서 정수에 대해 성립했던 이론이 거의 그대로 옮겨진다. 기약다항식이 소수의 역할을 하고, 최대공약수는 [유클리드 알고리즘](euclidean-algorithm.md)으로 계산되며, 인수분해는 단위원 배수와 순서를 무시하면 유일하다.
...
중요한 점은 기약성이 계수체에 의존한다는 것이다. $x^2+1$ 은 $\mathbb{R}$ 위에서는 기약이지만 $\mathbb{C}$ 위에서는 $(x+i)(x-i)$ 로 쪼개지고, $\mathbb{F}_2$ 위에서는 $(x+1)^2$ 이 된다. 따라서 "인수분해"는 다항식만의 성질이 아니라 다항식과 체의 쌍의 성질이다.
...
몫체에서 $x$ 의 동치류는 $f$ 의 근이 된다. 예컨대 $\mathbb{F}_2[x]/(x^2+x+1)$ 은 원소 4개인 체이고, $\mathbb{R}[x]/(x^2+1)$ 은 $\mathbb{C}$ 와 동형이다.
