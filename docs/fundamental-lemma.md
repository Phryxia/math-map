...
- $\kappa=1$: 조각을 전부 같은 무게로 더한다. **안정 궤도적분** $SO_\gamma$.
- $\kappa\neq1$: 조각마다 $\kappa$ 값을 곱해 더한다. **$\kappa$ 궤도적분** $O^\kappa_\gamma$.
...
진술만 보면 유한 계산처럼 보인다. 양쪽 다 유한합이고, 각 항은 격자 세기다. 그런데 그 세기가 $G$ 의 종류와 $\gamma$ 의 위치에 따라 제멋대로 바뀌고, 일반적인 $G$ 에 대해 직접 셀 방법이 없었다. $\mathrm{SL}_2$, $\mathrm{SL}_3$, $\mathrm{U}(3)$ 같은 작은 경우는 손으로 확인되었지만 그 계산들이 일반화되지 않았다.
...
합은 $G$ 의 내시군들에 걸친다. $G$ 자신도 그 목록에 있고($\kappa=1$), 나머지가 더 작은 군이므로 귀납이 돌아간다. 이것이 Arthur 가 30 년에 걸쳐 완성한 작업의 핵심 입력이었다.
