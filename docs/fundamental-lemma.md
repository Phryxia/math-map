...
$\mathcal A$ 의 한 점 $a$ 위의 올 $h^{-1}(a)$ 를 국소적으로 분해하면 각 점에서의 아핀 Springer 올들의 곱이 된다. 곧 **Hitchin 사상의 올이 궤도적분들을 한데 묶은 것**이다. 대역적 대상이 되었으므로 다음 도구가 생긴다. $\mathcal M$ 이 매끄럽고 $h$ 가 적절히 좋으면 Deligne 의 순수성 정리와 분해 정리를 쓸 수 있다.
