...
- [Random walk와 전기 네트워크](random-walks.md): 이산 Laplace 연산자의 영공간이 조화함수의 이산판이고, 전위가 그 역할을 한다.
- [Kähler 다양체와 Hodge 분해](kahler-manifolds.md): 조화형식은 Laplace–Beltrami 연산자의 핵이고, 각 de Rham 코호몰로지류에 조화 대표원이 하나씩 있다.
