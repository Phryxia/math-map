...
두 정리는 부호의 존재만 주장한다. 실제 부호는 [오류정정부호](error-correcting-codes.md)가 유한체 위의 선형대수로 구성하고, 그 조합 구조가 [Golay 부호](mathieu-groups.md)와 [격자 채우기](sphere-packing.md)로 이어진다.
...
```mermaid
graph TD
  ITO["정보이론 개관"] --> ENT["Shannon entropy"]
  PROB["유한 확률 공간"] --> ENT
...
- [KL divergence와 상호정보량](kl-divergence.md) — 두 분포의 차이와 두 변수가 공유하는 정보. 비음수성에서 정보이론의 부등식들이 나온다

## Shannon 의 두 정리

- [무손실 부호화 정리](source-coding.md) — 평균 부호 길이의 하한이 entropy 이고 그 하한에 임의로 가까이 갈 수 있다
...
- [Sinkhorn 알고리즘과 엔트로피 정규화](sinkhorn.md) — 최적 수송 문제에 entropy 항을 더하면 행렬 스케일링 반복으로 풀린다
