...
$n=0$ 항이 공식에서 벗어나는 것도 보인다. 정리가 충분히 큰 $n$ 을 요구하는 이유가 이 어긋남이다. $X=\Lambda/(p^{m})$ 로 두면 $\char"23{}(X/\omega_nX)=p^{m\thinspace p^{n}}$ 이 되어 $\mu$ 항이 나타난다.
...
4. **결론.** $\char"23{}X_\infty^{(\chi)}$ 가 $L_p$ 쪽이 허용하는 크기 이하임을 얻어 한쪽 나눔이 나온다.
