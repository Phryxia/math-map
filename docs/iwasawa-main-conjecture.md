...
순환체 $\mathbb Q(\mu_{p^n})$ 의 류수 $h_n$ 을 층마다 계산하면 규칙이 보이지 않는다. Iwasawa 는 한 층 대신 탑 전체를 보았다. 층을 무한히 쌓은 $\mathbb Q(\mu_{p^\infty})$ 에서 각 층의 류군을 사영극한으로 모으면 그 극한이 $\Lambda=\mathbb Z_p[[T]]$ 위의 유한생성 가군이 되고, $\Lambda$ 에 구조정리가 있으므로 극한 가군은 유사동형을 무시하면 멱급수 하나로 요약된다. 이것이 **특성 멱급수**다.
...
이다. $\Lambda$ 는 2 차원 정칙 국소환이고 유한생성 가군이 유사동형을 무시하면 순환 조각의 직합으로 분해된다. 류수의 수열은 이 한 가군이 각 층에 남긴 그림자다.
...
둘 다 모든 층에서 같은 유한량을 보간한다. 한 층에서는 해석적 유수 공식이 그 일치를 보장하지만 유수 공식은 크기의 곱만 말하고 $\Lambda$ 가군으로서의 세부는 말하지 않는다. 주추측은 세부까지 같다는 주장이다.
...
에서 나온다. 이 지표 공식이 전체 크기를 고정하므로 한쪽 나눔에 크기 일치를 더하면 등호가 된다. Euler 계가 상한을 주고 유수 공식이 총량을 주는 이 구도는 타원곡선 주추측에서도 반복된다.
