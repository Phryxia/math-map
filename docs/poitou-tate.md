...
Poitou–Tate 완전열은 이 열의 계수 $M$ 판본이고, $M=\mu_n$ 이면 위 열로 돌아온다. 아래의 계산은 이 상호법칙을 선형대수로 전개한 것이다.
...
가 성립한다. 오른쪽은 무한 자리만 보는 조합 계산이다. $K=\mathbb Q$ 이고 $M$ 이 복소켤레에 대해 $\pm$ 로 쪼개지면 오른쪽이 $\char35{}M^{+}/\char35{}M$ 이다.

이 공식은 $H^2$ 를 $H^1$ 로 바꿔 세는 데 쓴다. 변형 이론에서 $H^1$ 이 접공간이고 $H^2$ 가 장애공간이므로, Euler 표수가 장애와 접공간의 차이에 하한을 주고 그것이 변형환의 차원 하한이 된다.

## 국소 조건의 사전

자주 쓰는 조건은 다음과 같다. $M=E[p]$ 이고 $v\nmid p$ 인 경우다.
...
$m$ 하강에서 $\mathrm{Sel}^m(E/K)$ 의 크기는 Greenberg–Wiles 로 센다. $E[m]$ 이 Weil 짝으로 자기쌍대이므로 $L=L^{\ast}$ 를 고르면 좌변이 1 이 되고 국소 항의 곱이 1 이라는 항등식이 나온다. 이 항등식이 각 자리의 국소 지표 $\char35{}(E(K_v)/mE(K_v))$ 사이의 관계를 주고 하강 알고리즘의 정당성과 복잡도 추정의 근거가 된다.
