...
제약이 극단적인데도 이론이 유난히 잘 맞아떨어진다. 결정적 모델(DFA)과 비결정적 모델(NFA)이 같은 표현력을 갖고, 정규 표현식이라는 문법적 기술과도 일치하며(Kleene 정리), 각 정규언어에는 상태 수가 최소인 DFA가 동형을 제외하고 유일하게 존재한다(Myhill–Nerode 정리). 이 마지막 결과는 [동치관계](equivalence-relations.md)로 언어 자체를 분석해 얻어지며, 최소 DFA의 상태가 곧 동치류라는 점에서 이 문서의 중심이다.
