...
제약이 극단적인데도 이론이 유난히 잘 맞아떨어진다. 결정적 모델(DFA)과 비결정적 모델(NFA)이 같은 표현력을 갖고, 정규 표현식이라는 문법적 기술과도 일치하며(Kleene 정리), 각 정규언어에는 상태 수가 최소인 DFA가 동형을 제외하고 유일하게 존재한다(Myhill–Nerode 정리). 이 마지막 결과는 [동치관계](equivalence-relations.md)로 언어 자체를 분석해 얻어지며, 최소 DFA의 상태가 곧 동치류다.
...
이 계층은 결정 문제의 난이도에서 갈린다. 유한 오토마타에서는 모든 자연스러운 질문이 결정가능하지만, Turing 기계로 올라가면 [Rice 정리](rice-theorem.md)에 의해 의미론적 성질이 전부 결정불가능해진다. 표현력과 분석가능성은 맞바꾸는 관계다.
