...
디지털 회로의 게이트는 Boolean 연산이고, 회로 최소화는 같은 원소를 더 짧은 항으로 표현하는 문제다. DNF 는 항상 존재하지만 크기가 지수적일 수 있고, 최소 크기 표현을 찾는 문제는 계산적으로 어렵다. 충족가능성 판정(SAT)은 Lindenbaum 대수에서 "주어진 원소가 $0$ 이 아닌가"를 묻는 것과 같으며, 이 문제가 [NP-완비](np-completeness.md)이고, 계산 복잡도 이론은 이것을 기준 문제로 삼는다.
