...
- **해석 쪽.** $L(E/K,s)$ 가 $f$ 와 $K$ 에 붙는 [theta 급수](theta-series.md)의 [Rankin–Selberg 적분](rankin-selberg.md)으로 표현된다. $s=1$ 에서 미분하면 [Eisenstein 급수](eisenstein-series.md)의 Fourier 계수가 나오고 그 계수들이 국소 항의 합으로 쪼개진다.
