...
허수이차체 $K=\mathbb Q(\sqrt D)$ ($D<0$ 기본판별식)의 정수환 $\mathcal O_K$ 는 $\mathbb C$ 안의 격자이므로 타원곡선 $\mathbb C/\mathcal O_K$ 를 준다. 지표 $N$ 의 아이디얼 $\mathfrak n\subset\mathcal O_K$ 로
...
- **기하 쪽.** 정준 높이가 Néron 국소 높이의 합 $\hat h(y_K)=\sum_v \lambda_v$ 로 분해된다. 각 $\lambda_v$ 는 $v$ 자리에서 두 CM 점의 교차수이고, 유한 자리에서는 준동형사상 개수를 세는 문제로, 무한 자리에서는 상반평면의 Green 함수 적분으로 바뀐다.
- **해석 쪽.** $L(E/K,s)$ 가 $f$ 와 $K$ 에 붙는 theta 급수의 Rankin–Selberg 적분으로 표현된다. $s=1$ 에서 미분하면 Eisenstein 급수의 Fourier 계수가 나오고 그 계수들이 국소 항의 합으로 쪼개진다.
