...
적분 $\int_0^\infty f\,x^{s-1}dx$ 가 수렴하려면 $x\to0$ 에서 $f=O(x^{-a})$ 이고 $x\to\infty$ 에서 $f=O(x^{-b})$ 일 때 $a<\operatorname{Re}s<b$ 가 필요하다. Mellin 변환의 자연스러운 정의역은 점이 아니라 **수직 띠**이고, 띠의 양 끝을 정하는 것이 $f$ 의 양 끝 거동이다. 해석적 연속으로 띠 바깥으로 나가면 극점을 만나는데, 그 극점이 정확히 원래 함수의 전개 항에 대응한다.
