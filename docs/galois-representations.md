...
복소다양체에서는 고전적 위상으로 특이 코호몰로지를 만들면 된다. 유한체 위의 다양체에는 그런 위상이 없고, Zariski 위상은 열린집합이 너무 커서 상수층의 코호몰로지가 전부 0 이 된다. 실제로 Zariski 위상에서 기약 다양체 위의 상수층은 뭉툭해서 $i>0$ 마다 $H^i=0$ 이다.
...
$E\colon y^2=x^3+x+1$ 을 $\mathbb F_5$ 위에서 보자. $\mathbb F_5$ 와 $\mathbb F_{25}$ 와 $\mathbb F_{125}$ 에서 점을 직접 세고, $\mathbb F_5$ 의 결과만으로 나머지가 예측되는지 확인한다.
