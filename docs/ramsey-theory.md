...
계산 복잡도에서 Ramsey류 정리는 결정 트리와 통신 복잡도의 하한 논증에 쓰인다. 단색 구조를 피하는 색칠을 명시적으로 구성하기 어렵다는 점이 [P 대 NP 문제](p-np.md)(nondeterministic polynomial time) 주변의 명시적 구성 문제에서 표준 예로 인용된다.
