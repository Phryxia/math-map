...
이다. 오른쪽 합은 $\lbrace 1,\dots,N\rbrace$ 위의 길이 $k$ 짜리 닫힌 경로 전부를 훑는다. 성분의 평균이 0 이므로, 어떤 변을 한 번만 지나는 경로는 기댓값이 0 이 되어 사라진다. 살아남으려면 모든 변을 정확히 두 번씩 지나야 하고, 그래서 $k$ 가 홀수면 기여가 없다.
...
실 대칭 $N\times N$ 행렬 $H$ 의 성분 $\lbrace h_{ij}\rbrace\_{i\le j}$ 가 독립이고 $i<j$ 마다 $\mathbb E h_{ij}=0$ 이고 $\mathbb E h_{ij}^2 = 1$ 이며, 대각 성분의 분산이 유한하며 모든 적률이 유한할 때 $H$ 를 **Wigner 행렬**이라 한다. 규격화는 $A = H/\sqrt N$ 로 한다. 이 규격화가 옳은 이유는 $\frac1N\mathbb E\operatorname{tr}A^2 = \frac{1}{N^2}\sum_{i,j}\mathbb E h_{ij}^2 \to 1$ 로 2차 적률이 $N$ 과 무관하게 유한해지기 때문이다.
