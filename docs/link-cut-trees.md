...
`connected(v, w)`는 `find_root` 두 번으로 끝난다. 간선 추가와 삭제가 섞여 들어오는 온라인 질의를 그래프가 숲인 한 $O(\log n)$ 에 처리한다. 간선 추가만 있다면 서로소 집합 자료구조가 훨씬 빠르므로, link-cut tree를 쓰는 이유는 삭제 또는 경로 질의가 필요할 때다.
