...
이 사실을 계산이 아니라 이해의 대상으로 바꾸는 것이 전기 네트워크 대응이다. 그래프의 각 변을 저항으로 바꾸면, 무작위 걷기의 도달 확률은 회로의 전압이 되고 탈출 확률과 기대 왕복 시간은 [유효저항](effective-resistance.md)으로 표현된다. 그러면 출발점으로 돌아오는지가 "무한 네트워크의 유효저항이 발산하는가"라는 질문이 되고, 저항의 직렬·병렬 법칙과 Rayleigh 단조성 같은 물리적으로 자명한 규칙이 확률 명제의 증명이 된다. Doyle 과 Snell 의 고전적인 서술이 이 관점을 대중화했다[^1].
