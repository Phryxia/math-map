...
효과적으로 공리화된 무모순 이론 $T$ 의 정리 집합은 recursively enumerable하지만 결정 가능하지 않다. 결정 가능하다면 문장마다 증명 여부를 판정할 수 있어 $T$ 가 완전해지는데, 제1정리가 이를 막는다. 거꾸로 정지 문제의 결정 불가능성에서 제1정리를 얻을 수도 있다. "프로그램 $p$ 가 입력 $x$ 에서 멈추지 않는다"는 문장들이 모두 증명 가능한 완전한 체계가 있다면, 증명 탐색과 실행을 병렬로 돌려 정지 문제를 결정할 수 있게 된다([계산 가능성](computability.md), [Rice 정리](rice-theorem.md)). 두 결과 모두 대각선 논법에 기반하며, [P 대 NP 문제](p-np.md)와 달리 이미 증명된 정리다.
