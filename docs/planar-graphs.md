...
평면 그래프는 평면 위에 간선이 서로 교차하지 않게 그릴 수 있는 [그래프](graphs.md)다. 이 조건이 그래프의 조합적 구조를 강하게 제약한다.

중심에 **Euler 공식** $V-E+F=2$ 가 있다. 간선 수의 상한 $E\le3V-6$ , $K_5$ 와 $K_{3,3}$ 의 비평면성, 평면 그래프에 차수 5 이하 정점이 있다는 사실, 5 색 정리가 이 한 줄에서 따라 나오고 같은 공식이 정다면체가 다섯 개뿐임을 준다. Euler 공식은 [Euler 지표](euler-characteristic.md)의 가장 낮은 차원 사례다.

평면성의 특징 짓기는 **Kuratowski 정리**가 준다. 평면이 아닌 그래프는 $K_5$ 나 $K_{3,3}$ 을 세분한 형태로 품는 그래프뿐이다.

# 직관

그래프를 평면에 그리면 평면이 조각으로 나뉜다. 이 조각을 **면**(face)이라 한다. 바깥의 무한한 영역도 하나의 면으로 센다.

Euler 공식은 간선을 하나씩 더하며 세면 나온다. 트리는 사이클이 없어 면이 하나뿐이고 $E=V-1$ 이므로 $V-E+F=2$ 다. 간선을 하나 추가할 때마다 사이클이 하나 생기고 면이 하나 늘어 $E$ 와 $F$ 가 함께 1 씩 증가하므로 $V-E+F$ 가 보존된다. 연결 평면 그래프는 모두 이렇게 만들어지므로 값이 항상 2 다.

각 면은 최소 3 개의 간선으로 둘러싸이고 각 간선은 정확히 두 면의 경계에 놓인다. 면과 간선의 접촉을 두 방식으로 세면 $3F\le2E$ 이고, Euler 공식에 넣으면 $E\le3V-6$ 이 나온다. 이중 세기 한 번이 평면 그래프의 간선 수를 묶는다.

$K_5$ 는 $V = 5$ , $E = 10 \gt 9 = 3 \cdot 5 - 6$ 이므로 평면이 아니다. $K_{3,3}$ 은 삼각형이 없어 $4F \le 2E$ 라는 더 강한 제약을 받고, $E = 9 \gt 8 = 2 \cdot 6 - 4$ 이므로 역시 평면이 아니다.
...
평면 그리기가 하나 지정된 그래프를 **plane graph** 라 구별해 부르기도 한다. 같은 평면 그래프가 조합적으로 동치가 아닌 그리기를 여럿 가질 수 있기 때문이다(3-연결이면 Whitney 정리에 의해 그리기가 조합적으로 유일하다).
...
$F$ 가 그리기와 무관하게 $E-V+2$ 로 결정된다. $V-E+F$ 가 구면의 Euler 지표이기 때문이고, 종수 $g$ 의 곡면에 그린 그래프는 $V-E+F=2-2g$ 를 만족한다.
...
**따름정리 (차수).** 단순 평면 그래프의 평균 차수는 $2E/V \le 6 - 12/V \lt 6$ 이므로, 차수가 5 이하인 정점이 반드시 존재한다. 이 사실은 [비둘기집 원리](pigeonhole-principle.md)의 평균 논법 형태이며, 평면 그래프의 퇴화도가 5 이하임을 뜻한다.

## K5 와 K3,3

$K_5$ 는 $V = 5, E = 10$ 이고 $3 \cdot 5 - 6 = 9 \lt 10$ 이므로 비평면이다. $K_{3,3}$ 은 이분 그래프라 삼각형이 없고 $V = 6, E = 9$ 인데 $2 \cdot 6 - 4 = 8 \lt 9$ 이므로 비평면이다.
...
두 진술은 최대차수 3 이하인 그래프에 대한 관찰로 동치임이 확인된다. 증명의 어려움은 충분성 방향에 있고, 표준적인 접근은 최소 반례를 3-연결로 환원한 뒤 3-연결 그래프가 볼록 그리기를 갖는다는 Tutte 의 스프링 정리를 쓴다.

Wagner 형태는 그래프 마이너 정리의 첫 사례다. Robertson–Seymour 정리로 마이너에 대해 닫힌 모든 그래프 성질이 유한 개의 금지 마이너로 특징지어지고[^1], 평면성은 그 목록이 $\lbrace K_5,K_{3,3}\rbrace$ 인 경우다.

평면성 판정은 Hopcroft–Tarjan 알고리즘이 $O(V)$ 시간에 처리해 평면 그리기를 찾거나 Kuratowski 부분그래프를 출력한다. [그래프 동형](graph-isomorphism.md) 문제도 평면 그래프로 제한하면 선형 시간에 풀린다.
...
Kempe 는 1879 년 같은 논법으로 4 색 정리를 증명했다고 발표했으나 차수 5 정점에서 두 사슬을 동시에 뒤집는 단계에 오류가 있었고(Heawood, 1890), 논증의 남은 부분이 5 색 정리다. 4 색 정리는 불가피 집합과 축약 가능 배치를 컴퓨터로 검사하는 방식으로 1976 년에 증명되었으며[^2] 자세한 내용은 [그래프 색칠](graph-coloring.md)에 있다.
...
- **쌍대와 절단**: $G$ 의 사이클은 $G^\ast$ 의 절단에 대응하고, 그 역도 성립한다. 이 대응은 [matroid](matroids.md) 쌍대성의 기하적 실현이며, 평면 그래프에서 최소 절단을 쌍대 그래프의 최단 경로로 푸는 알고리즘의 기반이다([네트워크 흐름](network-flow.md)).
...
$$
\frac{1}{p} + \frac{1}{q} - \frac{1}{2} = \frac{1}{E} \gt 0
$$
...
다섯 개뿐이고 각각 정사면체, 정팔면체, 정육면체, 정이십면체, 정십이면체에 대응한다. $(p,q)$ 와 $(q,p)$ 의 교환이 쌍대 그래프에 대응한다.

같은 계산을 $1/p+1/q-1/2$ 의 부호로 분류하면 0일 때 평면 타일링(삼각형, 사각형, 육각형), 음수일 때 쌍곡 타일링이 나온다.

## 위상수학, 알고리즘, 조합 최적화

- **위상수학**: Euler 공식은 Euler 지표의 특수한 경우이고, 구면에 그린 그래프의 셀 분할에 대한 [단체 호몰로지](homology.md) 계산이 같은 값을 준다. 곡면 분류([다양체](manifolds.md))로 이어진다.
...
- **조합 최적화**: 평면 그래프에서의 최대 절단은 다항 시간에 풀리며(쌍대 그래프의 $T$ 조인 문제), 이는 일반 그래프에서 NP(nondeterministic polynomial time)-난해인 것과 대비된다.
