...
$F$ 가 그리기와 무관하게 $E-V+2$ 로 결정된다. $V-E+F$ 가 구면의 [Euler 지표](euler-characteristic.md)이기 때문이고, 종수 $g$ 의 곡면에 그린 그래프는 $V-E+F=2-2g$ 를 만족한다.
...
Wagner 형태는 그래프 마이너 정리의 첫 사례다. Robertson–Seymour 에 의해 마이너에 대해 닫힌 모든 그래프 성질이 유한 개의 금지 마이너로 특징지어지고[^1], 평면성은 그 목록이 $\lbrace K_5,K_{3,3}\rbrace$ 인 경우다.
...
- **위상수학**: Euler 공식은 [Euler 지표](euler-characteristic.md)의 특수한 경우이고, 구면에 그린 그래프의 셀 분할에 대한 [단체 호몰로지](homology.md) 계산이 같은 값을 준다. 곡면 분류([다양체](manifolds.md))로 이어진다.
