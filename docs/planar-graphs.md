...
plane graph $G$ 에 대해 **쌍대 그래프** $G^*$ 를 다음과 같이 만든다. $G$ 의 각 면에 정점 하나를 두고, $G$ 의 각 간선 $e$ 에 대해 $e$ 를 사이에 둔 두 면의 정점을 잇는 간선을 둔다(같은 면이 양쪽이면 고리가 된다). $G^*$ 도 평면이고 $|V(G^*)| = F(G)$ , $|E(G^*)| = E(G)$ , $F(G^*) = V(G)$ 다. 연결 plane graph에 대해 $(G^*)^* \cong G$ 다. 쌍대는 그리기에 의존하므로 추상 그래프의 불변량이 아니다.
