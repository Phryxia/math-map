...
설계행렬의 열이 생성하는 부분공간으로의 정사영이 최소제곱해다. 확률론에서는 $L^2(P)$ 안에서 부분 시그마대수에 대한 가측함수들이 이루는 닫힌 부분공간으로의 정사영이 조건부 기댓값이다. [Markov 연쇄](markov-chains.md)와 martingale 이론이 이 정사영을 쓴다. [특이값 분해](singular-value-decomposition.md)의 최적 저계수 근사도 같은 정사영 논리다.
