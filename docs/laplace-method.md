...
$\varphi$ 가 $[a,b]$ 내부의 유일한 점 $x_0$ 에서 최대이고 $\varphi''(x_0)<0$, $g(x_0)\ne0$ 이면
...
$\varphi$ 가 정칙이고 $\varphi'(z_0)=0$, $\varphi''(z_0)\ne0$ 이면 경로를 $z_0$ 을 지나는 급강하 경로로 변형해
...
각 열의 오차가 $1/(12n)$, $1/(288n^{2})$, $139/(51840n^{3})$ 과 자릿수까지 맞는다. **점근전개의 계수가 실제 오차를 그대로 예측한다**는 것이 이 표의 요점이다. 이 급수도 물론 발산하며, 최적 절단은 Euler–Maclaurin 쪽과 같은 $K\approx\pi n$ 이다.

## 안장점의 선택

안장점이 여럿이면 어느 것을 지나는 경로로 변형할 수 있는지가 문제다. 급강하 경로들은 계곡을 따라 이어지고, 주어진 끝점을 잇는 경로에 **어떤 안장점들이 포함되는지는 매개변수에 따라 불연속적으로 바뀐다**. 그 전환이 Stokes 현상이고, 전환선 근처에서는 지수적으로 작은 항이 갑자기 같은 크기로 올라온다. Euler–Maclaurin 문서에서 "점근전개가 놓치는 항" 이라 불렀던 것이 여기서는 다른 안장점의 기여로 구체화된다.
