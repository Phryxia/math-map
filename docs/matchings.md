...
König 정리는 Hall 정리와 동치이며, 두 정리 모두 [선형계획법](linear-programming.md) 쌍대성을 조합론에서 구체화한 것이다. 이분 그래프의 간선-정점 접합 행렬은 완전 유니모듈러이므로 매칭 LP의 최적해가 정수해가 되고, 그 쌍대가 정점 덮개 LP다. [네트워크 흐름](network-flow.md)의 최대유량 최소절단 정리에서도 같은 구조가 나온다.
