...
복소 미분가능성은 "국소적으로 회전과 등방 확대"라는 조건이다. 실이차원 사상으로 보면 Jacobian이 임의의 2×2 행렬이 될 수 있지만, 복소수 한 개의 곱으로 표현되려면 회전·등방 확대 행렬로 제한된다. 이 제약이 Cauchy–Riemann 방정식이고, 그 결과 도함수가 $0$ 이 아닌 점에서 정칙함수는 각을 보존하는 conformal 사상이 된다.
