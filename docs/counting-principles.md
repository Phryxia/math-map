...
"순서를 무시한 선택 하나"가 정확히 k!개의 순열에 대응하므로, 조합은 순열을 k!로 나눈 값이다. 각 섬유(fiber)의 크기가 같은 사상으로 세는 이 방식이 나눗셈 원리다.
...
$n$ 개 원소에서 $k$ 개를 순서를 구분해 뽑는 방법의 수($k$ -순열)와, 순서를 무시해 뽑는 방법의 수($k$ -조합)를 각각 다음과 같이 정의한다.
...
$k$ 가 $0$ 보다 작거나 $n$ 보다 크면 이항계수는 $0$ 으로 정의한다. 이항계수는 $n$ 원소 집합의 $k$ 원소 부분집합의 개수이며, $0$ 과 $1$ 로 이루어진 길이 $n$ 의 열 가운데 $1$ 이 $k$ 개인 것의 개수이기도 하다.
...
첫 식의 전단사 증명은 다음과 같다. $n$ 원소 집합의 $k$ 원소 부분집합을 특정 원소 $x$ 를 포함하는 것과 포함하지 않는 것으로 분할한다. 전자는 나머지 $n-1$ 개에서 $k-1$ 개를 고르는 것과, 후자는 $n-1$ 개에서 $k$ 개를 고르는 것과 각각 일대일 대응한다. 두 번째 식은 부분집합을 여집합으로 보내는 사상이 전단사이기 때문이다.
...
Vandermonde 항등식은 $m+n$ 원소를 두 덩어리로 나눠 놓고 모두 $r$ 개를 뽑을 때, 첫 덩어리에서 몇 개를 뽑는지로 경우를 나눈 결과다.
