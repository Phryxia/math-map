...
따라서 신뢰구간은 기각되지 않는 모수값들의 집합이다. 이 관점의 실용적 귀결이 두 가지다. 첫째, 95% 구간이 0 을 포함하지 않는 것과 양측 p-값이 0.05 미만인 것은 같은 말이다. 둘째, 구간은 p-값 하나보다 많은 정보를 준다. 기각 여부뿐 아니라 어떤 값들이 데이터와 양립하는지, 그 범위가 실질적으로 의미 있는 크기인지를 보여 준다.
