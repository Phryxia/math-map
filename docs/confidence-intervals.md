...
구성 방법은 대개 두 갈래다. 분포가 모수에 의존하지 않는 피벗량을 뒤집거나, [가설검정과 p-값](hypothesis-testing.md)의 기각역을 뒤집는다. 두 방법은 같은 계산을 방향만 바꿔 쓴 것이다. 대표본에서는 [중심극한정리](central-limit-theorem.md)가 근사 피벗량을 바로 준다.
