...
두 순서 모두 조밀하고 끝점이 없다. 매개변수 $a, b$ 를 쓴 논리식 $\exists x\thinspace (a \lt x \land x \lt b)$ 를 보면, 조밀성으로 이 논리식은 $a \lt b$ 와 같은 값을 갖는다. 양화사가 사라지고 매개변수 사이의 순서 비교만 남았다.
...
$T$ 가 완전하지 않으면 $\sigma$ 와 $\neg\sigma$ 의 모형이 따로 있다. 유한 모형이 없으므로 Löwenheim–Skolem 정리로 두 모형을 크기 $\kappa$ 로 맞출 수 있고, 범주성으로 둘이 동형이 되어 모순이다. ∎
