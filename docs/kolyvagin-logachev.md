...
Kolyvagin–Logachev 는 이것을 모듈러 형식에서 만들어지는 아벨 다양체 $A_f$ 로 확장해, 차원이 1 보다 커도 $L(f,1)\ne0$ 이면 $A_f(\mathbb Q)$ 가 유한함을 보인다. 이 일반화로 모듈러 곡선의 Jacobian 에서 유리점이 유한한 몫을 만들 수 있다.

그 몫이 **겨냥 몫**이다. $J_0(N)$ 에서 $L(f,1)\ne0$ 인 성분만 남긴 것이므로 Mordell–Weil 군이 유한하고, 모듈러 곡선 위의 점을 가두는 그물이 된다. [Merel 의 일양 유계성 정리](merel-theorem.md)가 이 그물을 쓴 예다. 해석적 비소멸이 기하적 유한성이 되고, 그 유한성이 산술적 유계를 낳는다.
...
$X_0(N)$ 의 모듈러 기호 $e=\lbrace 0,\infty\rbrace$ 가 정하는 Hecke 아이디얼 $I_e=\operatorname{Ann}_{\mathbb T}(e)$ 로 몫을 취한 아벨 다양체
...
겨냥 몫의 유리점이 유한하다는 사실이 모듈러 곡선 위의 점을 가둔다. 곡선이나 그 대칭곱에서 $J_e$ 로 보낸 뒤 상이 유한집합에 들어감을 보이고 그 유한집합을 조사하면 점의 목록이 나온다. Mazur 이후의 유리점 연구가 이 통로에 기댄다.

## 일양 유계성

Merel 의 증명은 위 사실을 차수 $d$ 점에 적용한다. 대칭곱 $X_1(p)^{(d)}$ 에서 겨냥 몫으로 보내고 Hecke 작용소로 단사성을 판정한 뒤 유리점의 유한성에서 모순을 끌어낸다. 겨냥 몫이 $\mathbb Q$ 위에 있으므로 체를 바꾸지 않고 모든 차수 $d$ 체를 한꺼번에 다룬다.
