...
는 $p$ 진 주기환에서 대수기하의 곡선처럼 행동하는 대상을 뽑아낸 것이다. 이 곡선 위에서 $G$ 다발의 모듈라이 $\mathrm{Bun}\_G$ 를 만들고, 그 위의 에탈[층](sheaves.md) [범주](category.md)를 보면 $G(\mathbb Q_p)$ 의 스무스 표현론이 통째로 들어 있다. 국소 Langlands 대응이 기하학적 Langlands 의 $p$ 진 판본으로 다시 쓰인다.
