...
[$p$ 진 Hodge 이론](p-adic-hodge-theory.md)이 이 문서의 또 다른 부모인 것은 재료 때문이다. $B_{\mathrm{cris}}$, $A_{\mathrm{inf}}$, 틸팅, perfectoid 가 전부 곡선의 부품으로 쓰인다. 그 이론에서 손으로 다루던 주기환들이 여기서는 한 곡선의 국소 구조로 정리된다.
...
기울기가 모두 같은 것을 **기본(basic)** 이라 한다. $n=2,d=1$ 의 $1/2$, $n=4,d=2$ 의 $1/2,1/2$ 가 그렇다. 나머지는 기울기가 갈라진 비기본 원소다. 이 구분이 층화의 뼈대다.
...
$C$ 를 완비 대수적 닫힌 비아르키메데스체(표수 $p$), $C^\flat$ 를 그 틸팅이라 한다. $B$ 를 대응하는 주기환이라 할 때
