...
[모듈러 형식](modular-forms.md)과 [Hecke 작용소](hecke-operators.md)의 계산에서 $S_k(\Gamma_0(N))$ 의 원소는 무한급수라 손으로 다룰 수 없다. 차원을 알아도 기저를 얻는 방법, 새형식을 나열하는 방법, $L$ 값을 구하는 방법이 따로 필요하다.

모듈러 기호는 이 질문들을 유한 차원 정수 선형대수로 바꾼다. 출발점은 [모듈러 곡선](modular-curves.md) $X_0(N)$ 의 위상이다. 무게 $2$ 의 첨점형식은 $X_0(N)$ 위의 정칙 1형식이므로 형식 대신 그것을 적분할 경로를 다룬다. 첨점을 양 끝으로 허용한 경로의 [호몰로지](homology.md)류
...
Pollack–Stevens 의 **과수렴 모듈러 기호**는 계수 [가군](modules.md) $V_{k-2}$ 를 $p$ 진 분포 공간으로 바꾼 것이다. 고전 기호로의 특수화 사상이 작은 기울기 고유공간에서 동형이 되고(Stevens 의 조절 정리) 그 올림이 $p$ 진 $L$ 함수를 준다. 구성이 유한 계산이라 Iwasawa 이론의 수치 실험이 가능해졌다.
