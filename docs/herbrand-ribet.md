...
[Stickelberger](stickelberger.md) 소멸 정리는 한 방향만 준다. $p$ 가 Bernoulli 수 $B_k$ 를 나누지 않으면 순환체 류군의 $\omega^{1-k}$ 성분이 0 이다. 이것이 Herbrand 의 정리이고, 소멸자가 단원이라는 말이므로 증명은 한 줄이다.
...
Mazur–Wiles 의 [Iwasawa 주추측](iwasawa-main-conjecture.md) 증명이 이 논법을 탑 전체로 밀어 올린 것이고, Skinner–Urban 의 [타원곡선](elliptic-curves.md) 주추측도 같은 구조를 쓴다.
...
로 쪼개진다. $\omega$ 는 Teichmüller 지표다. 이 이론은 각 성분을 따로 묻는다.
...
**정리**[^1]**.** $p$ 가 홀소수, $k$ 가 짝수, $2\le k\le p-3$ 이고 $p\mid B_k$ 라 하자. 그러면 레벨 1, 무게 $k$ 의 고유첨점형식 $f$ 와 그 계수체의 $p$ 위 [소 아이디얼](prime-ideals.md) $\mathfrak p$ 가 존재해
...
무게 $k$ 의 형식 공간에서 Eisenstein 부분을 상수항 1 로 정규화하면 계수의 분모에 $B_k$ 가 들어간다. $p\mid B_k$ 는 그 정규화가 $\mathbb Z_p$ 위에서 불가능하다는 뜻이고, Hecke 대수의 Eisenstein 아이디얼이 극대 아이디얼에 포함된다는 진술과 같다.
...
$k=p-1$ 이면 $\omega^{k-1}$ 이 자명해져 표현이 $\begin{pmatrix}1&\ast\cr 0&1\end{pmatrix}$ 가 되고 위 논법이 무너진다. von Staudt–Clausen 정리로 $(p-1)\mid k$ 이면 $B_k$ 의 분모가 $p$ 를 포함하므로 $p\mid B_k$ 의 뜻도 달라진다. 가정 $2\le k\le p-3$ 이 [Bernoulli 수](bernoulli-numbers.md)의 $p$ 진 성질을 반영한다.
...
$p$ 가 비정칙일 때 어느 성분이 비자명한지가 Herbrand–Ribet 으로 결정된다. $p=37$ 은 $B_{32}$ 의 분자를 나누므로 $A^{(\omega^{-31})}\ne0$ 이고, $A$ 는 위수 37 의 순환군이다. Bernoulli 수만 계산하면 되므로 류군을 직접 다루는 것보다 빠르다.

## 합동을 통한 표현의 구성

고유형식이 Eisenstein 급수와 합동이면 그 표현이 가약이라는 원리는 Serre 추측과 변형 이론에서 쓰인다. [변형환](deformation-rings.md)의 언어로는 Eisenstein 극대 아이디얼에서의 Hecke 대수의 국소 구조를 묻는 문제이고, Mazur 의 Eisenstein 아이디얼 연구가 그 시작이다. $\mathbb Q$ 위 타원곡선의 비틀림점 분류(Mazur 의 정리)가 같은 기술의 산물이다.
