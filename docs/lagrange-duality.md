...
*증명의 요지.* $(g(x),f(x))$ 꼴의 점들이 만드는 볼록집합과 점 $(0,p)$ 를 분리하는 초평면이 존재한다. Slater 조건은 이 초평면이 수직, 곧 $f$ 좌표의 계수가 $0$ 인 퇴화 형태가 되지 못하게 막는다. 초평면의 법선이 최적 승수다.

선형계획 문제는 제약이 모두 아핀이므로 Slater 조건 없이 허용해의 존재만으로 강쌍대성을 갖는다.
