...
를 만족하는 $d$ 정규 그래프가 **Ramanujan 그래프**[^1]이고, 스펙트럼 간극이 최적인 expander 다.
...
증명은 주어진 그래프의 2 겹 덮개를 잘 고르면 $\lambda$ 가 늘지 않음을 보인다. 가능한 덮개들에 걸친 [특성다항식](eigenvalues.md)의 평균인 교대 특성다항식을 잡고, 그 근의 최대값이 개별 다항식 가운데 하나의 근보다 크거나 같음을 쓴다. 이때 도입된 **교대 족(interlacing family)** 개념은 Kadison–Singer 문제 해결에도 쓰였다.
...
확장성이 좋은 그래프는 지름이 작고 연결이 강하다. 부호 이론의 expander 부호, 결함에 강한 통신망 설계, 분산 계산의 통신 패턴이 Ramanujan 그래프를 목표로 삼는다. 간극이 최적이면 같은 차수에서 최선의 견고함을 얻는다.

## 무작위성 추출과 복잡도

Expander 그래프 위의 [무작위 걷기](random-walks.md)는 짧은 시간에 균등분포에 가까워지고, 무작위성을 아끼는 알고리즘이 이 성질을 쓴다. Expander 걷기 보조정리, 결정론적 증폭, $\mathrm{SL}=\mathrm L$ 정리가 확장성의 정량적 경계를 쓰며, 간극이 최적인 expander 가 그 상수를 최선으로 만든다.
