...
를 만족하는 $d$ 정규 그래프가 **Ramanujan 그래프**[^1]이고, 스펙트럼 간극이 최적인 확장자다.
...
확장자 그래프 위의 [무작위 걷기](random-walks.md)는 짧은 시간에 균등분포에 가까워지고, 무작위성을 아끼는 알고리즘이 이 성질을 쓴다. 확장자 걷기 보조정리, 결정론적 증폭, $\mathrm{SL}=\mathrm L$ 정리가 확장성의 정량적 경계를 쓰며, 최적 확장자가 그 상수를 최선으로 만든다.
