...
Ornstein–Uhlenbeck 과정 `V(x)=x^2/2` 를 본다. Fokker–Planck 방정식은 `\partial_t\rho=\partial_x(x\rho)+\partial_{xx}\rho` 이고 정상분포는 `\rho_\infty=N(0,1)` 이다. `\nabla^2V=I` 이므로 `\lambda=1` 이고, 위의 지수 수렴이 `e^{-t}` 와 `e^{-2t}` 로 나와야 한다.

가우시안 초기조건에서는 해가 가우시안으로 남아 평균과 분산이 닫힌 꼴이다. 그래서 두 가지를 확인할 수 있다. 유한차분으로 푼 PDE 가 그 해석해와 맞는지, 그리고 `W_2` 축약과 엔트로피 소산의 지수 비율이 실제로 성립하는지다.

```python
import math

# Ornstein-Uhlenbeck : ∂_t ρ = ∂_x(xρ) + ∂_xx ρ,  V(x)=x²/2,  ρ_∞ = N(0,1)
m0, s0 = 2.0, 0.5

def exact(t):                          # 가우시안은 가우시안으로 남는다
    m = m0 * math.exp(-t)
    s = math.sqrt(s0 ** 2 * math.exp(-2 * t) + (1 - math.exp(-2 * t)))
    return m, s

def W2(m, s):                          # 1 차원 가우시안 사이의 W_2 (ρ_∞ = N(0,1))
    return math.sqrt(m ** 2 + (s - 1) ** 2)

def KL(m, s):                          # 자유에너지 = KL(ρ ‖ ρ_∞) + 상수
    return (m ** 2 + s ** 2 - 1) / 2 - math.log(s)

# 유한차분으로 PDE 를 직접 풀어 해석해와 맞춰 본다
L, n = 8.0, 1601
h = 2 * L / (n - 1)
x = [-L + i * h for i in range(n)]
dt = 0.2 * h * h
rho = [math.exp(-(xi - m0) ** 2 / (2 * s0 ** 2)) / (s0 * math.sqrt(2 * math.pi)) for xi in x]

def step(rho):
    flux = [0.0] * (n + 1)             # 경계면에서의 흐름 : xρ + ∂_xρ
    for i in range(1, n):
        xm = 0.5 * (x[i - 1] + x[i])
        flux[i] = xm * 0.5 * (rho[i - 1] + rho[i]) + (rho[i] - rho[i - 1]) / h
    return [rho[i] + dt * (flux[i + 1] - flux[i]) / h for i in range(n)]

def moments(rho):
    Z = sum(rho) * h
    m = sum(xi * r for xi, r in zip(x, rho)) * h / Z
    v = sum((xi - m) ** 2 * r for xi, r in zip(x, rho)) * h / Z
    return m, math.sqrt(v)

print(f"{'t':>5} {'m(수치)':>10} {'m(해석)':>10} {'s(수치)':>10} {'s(해석)':>10}")
t, targets, k = 0.0, [0.25, 0.5, 1.0, 2.0], 0
while k < len(targets):
    rho = step(rho); t += dt
    if t >= targets[k]:
        mn, sn = moments(rho); me, se = exact(t)
        print(f"{t:>5.2f} {mn:>10.6f} {me:>10.6f} {sn:>10.6f} {se:>10.6f}")
        k += 1

print(f"\n{'t':>5} {'W_2(ρ_t,ρ_∞)':>14} {'e^-t·W_2(0)':>13} {'F(ρ_t)-F(ρ_∞)':>15} {'e^-2t·F(0)':>12}")
w0, f0 = W2(*exact(0.0)), KL(*exact(0.0))
ok_w = ok_f = True
for t in (0.0, 0.5, 1.0, 2.0, 3.0, 5.0):
    m, s = exact(t)
    w, f = W2(m, s), KL(m, s)
    ok_w &= w <= w0 * math.exp(-t) + 1e-12
    ok_f &= f <= f0 * math.exp(-2 * t) + 1e-12
    print(f"{t:>5.1f} {w:>14.8f} {w0*math.exp(-t):>13.8f} {f:>15.8f} {f0*math.exp(-2*t):>12.8f}")
print("W_2 축약  W_2(ρ_t,ρ_∞) ≤ e^{-t} W_2(ρ_0,ρ_∞) :", ok_w)
print("엔트로피 소산  F(ρ_t) ≤ e^{-2t} F(ρ_0)       :", ok_f)

#     t      m(수치)      m(해석)      s(수치)      s(해석)
#  0.25   1.557567   1.557570   0.738311   0.738323
#  0.50   1.213055   1.213061   0.850920   0.850935
#  1.00   0.735752   0.735759   0.947877   0.947892
#  2.00   0.270660   0.270665   0.993095   0.993108
#
#     t   W_2(ρ_t,ρ_∞)   e^-t·W_2(0)   F(ρ_t)-F(ρ_∞)   e^-2t·F(0)
#   0.0     2.06155281    2.06155281      2.31814718   2.31814718
#   0.5     1.22218580    1.25039499      0.75922359   0.85279869
#   1.0     0.73760180    0.75840290      0.27343494   0.31372711
#   2.0     0.27075830    0.27900083      0.03667889   0.04245835
#   3.0     0.09957848    0.10263867      0.00495837   0.00574611
#   5.0     0.01347590    0.01389063      0.00009080   0.00010524
# W_2 축약  W_2(ρ_t,ρ_∞) ≤ e^{-t} W_2(ρ_0,ρ_∞) : True
# 엔트로피 소산  F(ρ_t) ≤ e^{-2t} F(ρ_0)       : True
```

유한차분 해와 해석해가 소수점 다섯째 자리까지 맞는다. 보존형으로 차분했기 때문에 질량이 보존되고, 그래서 모멘트가 정확히 따라간다.

아래 표에서는 두 지수 비율이 모두 성립하고, 게다가 여유가 있다. `\lambda=1` 이 최선의 상수인데도 부등식이 등호로 포화되지 않는 것은 초기분포가 측지선 위의 최악 위치가 아니기 때문이다. `t` 가 커질수록 `W_2` 는 `e^{-t}` 의 상수배에, 자유에너지는 `e^{-2t}` 의 상수배에 붙어 간다. 지수 비율 `1` 과 `2` 가 정확히 예측된 값이다.
