...
경사하강의 암시적 오일러 이산화를 옮기면 **JKO**(Jordan–Kinderlehrer–Otto) **스킴**이 나오고, 그 수렴 증명이 Fokker–Planck 해의 존재 증명이 된다.[^1] 또 $W_2$ 를 따라 잰 볼록성이 확산의 수렴 속도를 주며, 함수부등식의 증명과 생성모형 설계가 이 사실에 기댄다.
...
- **함수부등식.** 로그 Sobolev, Talagrand, HWI(entropy H, Wasserstein distance W, Fisher information I) 부등식이 자유에너지의 변위 볼록성에서 따라 나온다.
