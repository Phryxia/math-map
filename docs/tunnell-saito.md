...
국소체 $F$, 이차 확대 $K/F$, 그리고 $\mathrm{GL}_2(F)$ 의 기약 표현 $\pi$ 를 놓자. 토러스 $K^\times$ 위에서 지표 $\chi$ 로 변환하는 범함수가 존재하는지 묻는다.
...
$F=\mathbb R$, $K=\mathbb C$ 인 경우도 같은 형태다. 무게 $2k$ 의 이산계열과 지표 $\chi$ 의 무게를 비교해 부호가 정해지고, 부호가 $-1$ 이면 콤팩트형 $\mathbb H^\times$ 쪽에 범함수가 있다. 확정 사원수 대수가 쓰이는 상황이 이것이다.
