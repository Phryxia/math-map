...
유한체가 $\mathbb{F}\_p$ 위의 [확대](field-extensions.md)이면서 한 다항식의 분해체라는 점에서 이 분류가 나온다. 구조도 단순하다. 덧셈군은 $\mathbb{F}\_p$ 위 $n$ 차원 [벡터 공간](vector-spaces.md), 곱셈군은 위수 $q-1$ 의 순환군, 자기동형군은 Frobenius가 생성하는 위수 $n$ 의 순환군이다. 이 완결성 덕분에 유한체는 오류정정 부호, 암호, 조합 설계에서 표준 계산 대상으로 쓰인다.
...
$\mathbb{F}\_p$ 와 $\mathbb{F}\_{2^n}$ 의 곱셈군이 순환군이므로 [이산로그](discrete-logarithm.md) 문제가 정의되는 군이 나오고, Diffie–Hellman 키 교환이 그 위에 선다. 원시근을 이용한 선형 되풀이 수열(LFSR)은 최대 주기 $2^n-1$ 의 비트열을 만든다.
