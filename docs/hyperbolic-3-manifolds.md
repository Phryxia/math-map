# 쌍곡 3 다양체와 Mostow 강직성

# 개요

곡면에서는 위상과 기하가 분리된다. 종수 2 인 닫힌 곡면 하나에 곡률 $-1$ 인 계량을 줄 수 있고, 그런 계량은 하나가 아니라 6 차원 가족을 이룬다. 바지 분해의 경계 길이와 비틀림을 손으로 돌리면 서로 다른 쌍곡 곡면이 계속 나온다. 그래서 "이 곡면의 넓이" 같은 말은 위상만으로는 뜻이 없다. 넓이는 [Gauss–Bonnet](gauss-bonnet.md)에 의해 $2\pi|\chi|$ 로 고정되지만, 측지선의 길이 스펙트럼은 계량마다 다르다.

3 차원에서는 이것이 무너진다.

> **Mostow 강직성.** 유한 부피 완비 쌍곡 구조를 갖는 3 차원 다양체에서 그 구조는 등거리사상을 빼면 **유일**하다.

돌릴 손잡이가 없다. 한 다양체에 쌍곡 계량이 들어가면 그 계량은 위상이 이미 결정한 것이고, 따라서 계량에서 읽는 모든 양 — 부피, 최단 측지선의 길이, 등거리군의 위수 — 이 전부 **위상 불변량**이 된다. 이것이 3 차원 위상수학의 판을 바꾼 진술이다. 매듭 여집합의 부피 $2.029883\ldots$ 는 계량을 고른 결과가 아니라 8 자매듭 자체의 성질이다.

부모를 셋 고른 이유가 여기서 드러난다. 쌍곡 구조는 [Riemann 계량](riemannian-metrics.md)의 특수한 경우(상수 단면곡률 $-1$ 이고 완비)이고, 그 구조를 군의 말로 번역하는 도구가 [덮개공간](covering-spaces.md)(보편덮개가 $\mathbb H^3$ 이고 데크 변환군이 기본군)이며, 구조를 실제로 손에 쥐고 계산할 때 나오는 수가 [Lobachevsky 함수](lobachevsky-function.md)의 값이다.

# 직관

## 왜 2 차원은 물렁하고 3 차원은 굳는가

같은 강직성 증명을 2 차원에 적용해 보면 어디서 막히는지가 보인다. 증명은 보편덮개 사이의 동변 유사등각(quasi-conformal) 사상을 만들고, 그것이 무한원 경계로 확장된다는 사실에서 출발한다. 경계는

- 2 차원: $\partial\mathbb H^2=S^1$
- 3 차원: $\partial\mathbb H^3=S^2=\widehat{\mathbb C}$

이다. 핵심은 **유사등각 사상이 어디에 사는가**다. $S^1$ 위의 유사대칭 사상은 무한 차원 공간을 이루고, 그 공간이 바로 Teichmüller 공간이다. 반면 $S^2$ 위에서는 Mostow 의 에르고딕 논법이 작동한다. 극한집합이 $S^2$ 전체이고 군 작용이 에르고딕이면, 경계 사상의 미분이 거의 어디서나 등각이어야 하고, 등각 자기사상은 [Möbius 변환](holomorphic-functions.md)뿐이다. 유사등각이 강제로 등각이 되어 버리는 것이다.

한 줄로 줄이면 이렇다. **1 차원 원에는 등각이 아닌 변형이 널려 있지만, 2 차원 구에는 없다.** 차원이 하나 올라가면서 경계의 등각 구조가 뻣뻣해진다.

```mermaid
flowchart TD
  T["위상 다양체 M³"] -->|"기하화"| G["8 개 기하 중 하나<br/>대부분 H³"]
  G --> D["이산 충실 표현<br/>ρ: π₁(M) → PSL₂(C)"]
  D -->|"Mostow"| U["ρ 는 켤레를 빼고 유일"]
  U --> I["Vol, 길이 스펙트럼,<br/>Isom(M) 이 위상 불변량"]
  I --> V["볼륨 추측이 비교할<br/>기하 쪽 수치"]
```
...
의 편각이고 곱이 $zz'z''=-1$ 이다. 그러면 $n$ 개 사면체를 붙인 다양체는 미지수 $n$ 개짜리 방정식계 하나로 환원된다. 이것이 Thurston 의 **붙임 방정식**이다.
...
> **정의.** $\Gamma\le\mathrm{PSL}_2(\mathbb C)$ 가 이산이고 비틀림이 없으면 $\Gamma$ 를 **Kleinian 군**이라 하고, $M=\mathbb H^3/\Gamma$ 를 완비 쌍곡 3 다양체라 한다. $\mathrm{Vol}(M)<\infty$ 일 때 유한 부피라 한다.
...
> $$H(m)=H(\ell)=1$$
> 여기서 $H$ 는 사면체 모양의 곱으로 주어지는 도함수(홀로노미)다.

이 조건을 만족하는 해 가운데 모든 $z_i$ 의 허수부가 양인 것이 기하적 해다. 부피는 그때
...
인 **Bloch–Wigner 이중로그**로 주어지고, $z=e^{i\theta}$ 처럼 단위원 위의 값에서는 $D$ 가 [Lobachevsky 함수](lobachevsky-function.md)로 되돌아간다. 실제로 이상 사면체의 세 이면각이 $\alpha,\beta,\gamma$ 이면 $D(z)=\Lambda(\alpha)+\Lambda(\beta)+\Lambda(\gamma)$ 다.
...
Mostow 가 닫힌 경우를, Prasad 가 cusp 가 있는 유한 부피 경우를 처리했다. $n\ge3$ 이 결정적이다. $n=2$ 는 Teichmüller 공간이 $6g-6$ 차원이라 완전히 거짓이다.
...
4. 등각 자기사상은 Möbius 변환이고, 그것은 $\mathbb H^n$ 의 등거리사상에서 온다.

곧 **"거의 등거리"가 "정확히 등거리"로 승격되는** 논법이다. 3 단계가 $n\ge3$ 을 쓴다.

따름정리가 실질적인 힘을 갖는다.

- **부피가 위상 불변량이다.** $\mathrm{Vol}(M)$ 을 매듭 표에 적을 수 있고, 두 매듭의 부피가 다르면 두 매듭은 다르다.
- **$\mathrm{Isom}(M)$ 이 유한이고 $\mathrm{Out}(\pi_1 M)$ 과 같다.** 쌍곡 다양체의 대칭을 군론으로 셀 수 있다.
- **길이 스펙트럼, 자취체, cusp 모양이 전부 불변량이다.**

## 두꺼운–얇은 분해

> **Margulis 보조정리.** 차원 $n$ 마다 상수 $\varepsilon_n>0$ 이 있어, 임의의 쌍곡 $n$ 다양체에서 단사반지름이 $\varepsilon_n$ 미만인 부분 $M_{<\varepsilon}$ 은 cusp 이웃과 짧은 측지선 주변의 관 (Margulis 관) 들의 합집합이다.

그래서 쌍곡 다양체는 항상 **두꺼운 부분**(기하가 유계인 콤팩트 조각)과 **얇은 부분**(표준 꼴)으로 나뉜다. 부피가 유계이면 두꺼운 부분의 꼴이 유한 가지뿐이라는 콤팩트성이 여기서 나오고, 그것이 아래 Jørgensen–Thurston 정리의 엔진이다.
...
여기서 $V_3=\Lambda(\pi/6)\cdot3=1.0149416\ldots$ 는 정이면체 이상 사면체의 부피다. 8 자매듭 여집합이 정확히 그런 사면체 **두 개**라는 사실이 위 표의 첫 줄이다.
...
cusp 를 메우면 부피가 **줄고**, 아주 복잡하게 메울수록 원래 부피에 아래에서 수렴한다. 그래서 $\mathcal V$ 안의 각 부피값은 그보다 큰 부피값들의 극한이 되고, 이 과정을 겹쳐 $\omega^\omega$ 가 나온다. 동시에 "대부분의 3 다양체는 쌍곡이다"라는 경험칙의 근거이기도 하다. 수술 기술로 만드는 3 다양체 중 쌍곡이 아닌 것은 유한개의 예외뿐이다.

강직성과 모순되지 않는다는 점을 짚어 둘 만하다. 강직성은 **고정된 위상형**에서 계량이 유일하다는 말이고, Dehn 수술은 위상형 자체를 바꾼다. 붙임 방정식의 말로는, 완비성 조건을 풀어 준 해의 족이 불완비 구조들의 1 복소차원 가족을 이루고, 그 중 홀로노미가 $(p,q)$ 조건을 만족하는 이산점들이 채워진 다양체의 완비 구조가 된다.

## 기하화 정리 안에서의 위치

> **정리 (Thurston 의 기하화 추측, Perelman 증명).** 모든 닫힌 방향 가능 3 다양체는 소수 분해와 원환면 분해를 거쳐 8 개 모형 기하 중 하나를 갖는 조각으로 잘린다.

8 개는 $\mathbb H^3,\ \mathbb E^3,\ S^3,\ S^2\times\mathbb R,\ \mathbb H^2\times\mathbb R,\ \widetilde{\mathrm{SL}_2\mathbb R},\ \mathrm{Nil},\ \mathrm{Sol}$ 이고, 이 가운데 $\mathbb H^3$ 조각이 압도적으로 많다. 나머지 일곱은 Seifert 올뭉치거나 원환면 다발로 전부 분류되어 있어서, 3 차원 위상수학의 실질적 내용이 쌍곡 조각에 몰려 있다. 강직성이 그 조각들을 기하로 완전히 식별해 주므로, 3 다양체의 분류가 **Kleinian 군의 분류**로 바뀐다.
...
이 수가 [볼륨 추측](volume-conjecture.md)이 색 Jones 다항식의 증가율로 재현한다고 주장하는 바로 그 값이다. 조합 쪽 합의 안장점 조건이 여기 모서리 조건과 같은 식이 되는 것이 추측의 기하적 심장이다.

## 이중로그로 부피를 계산하는 코드

붙임 방정식의 해가 주어졌을 때 부피를 구하는 부분은 Bloch–Wigner 함수의 수치 계산이다.
...
# 모서리 조건 z(1-z)w(1-w) = 1 을 z=w 에서 확인
print(abs(z * (1 - z) * z * (1 - z) - 1))   # ~1e-16
```

$|z|=1$ 인 정이면체 해에서는 $\log|z|=0$ 이라 둘째 항이 사라지고 $D(e^{i\theta})=\operatorname{Im}\mathrm{Li}_2(e^{i\theta})=2\Lambda(\theta/2)$ 가 되어 Lobachevsky 값으로 되돌아간다.[^1]

## 매듭을 구별하는 실무

SnapPy 같은 프로그램은 매듭 도식에서 이상 사면체 분할을 자동으로 만들고 붙임 방정식을 Newton 법으로 푼다. 얻어진 부피, cusp 모양, 자취체가 전부 위상 불변량이므로 매듭표에서 항목을 가르는 1 차 기준이 된다. [Alexander 다항식](knot-invariants.md)이나 [Jones 다항식](knot-invariants.md)이 같은 값을 주는 매듭 쌍도 부피가 다르면 즉시 갈린다.

수치 계산이 증명이 되지 못한다는 한계는 있다. 붙임 방정식의 근사해가 진짜 해 근처에 있다는 것을 구간 산술로 검증하는 절차(HIKMOT 류)가 따로 필요하고, 그 검증을 통과해야 "이 다양체는 쌍곡이다"가 정리가 된다.
