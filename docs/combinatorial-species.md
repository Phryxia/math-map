...
조합적 종은 유한집합마다 그 위의 구조를 배정하는 규칙을 함자로 본 것이다. 종에는 합, 곱, 합성, 미분이 정의되고, 종을 지수생성함수로 보내는 사상이 이 네 연산을 급수의 덧셈, 곱셈, 합성, 미분으로 옮긴다.
...
- 집합 분할과 순열 분해의 지수생성함수를 한 공식에서 얻는다. [Bell 수](bell-numbers.md)와 제1종 Stirling 수가 지수 공식의 두 경우다.
