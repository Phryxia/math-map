...
아래에서 환은 모두 가환환이고 곱셈 항등원 $1$ 을 갖는다. 준동형은 $1$ 을 $1$ 로 보낸다.
...
한 가지 조정이 필요하다. $R$ 에 영인자가 있으면 $at = bs$ 라는 관계는 추이적이지 않다. 그래서 [동치관계](equivalence-relations.md)를 어떤 $u$ 가 $S$ 에 있어 $u(at - bs) = 0$ 이 되는 것으로 느슨하게 잡는다. 정역에서는 $S$ 가 $0$ 을 포함하지 않는 한 이 조건이 원래 조건과 같다.
...
이 성립한다([텐서곱](tensor-products.md)). 국소화는 $S^{-1}R$ 과의 텐서곱 [함자](functors.md)이고 완전(exact)하다.
