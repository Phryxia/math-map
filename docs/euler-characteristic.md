...
Euler 지표는 도형을 조각으로 나누었을 때 조각 수의 교대합으로 정의되는 정수다. 평면에 그린 연결 그래프에서는 꼭짓점 수에서 간선 수를 빼고 면 수를 더하면 항상 2이고, 볼록다면체도 모두 이 값을 가진다. 값은 쪼개는 방식에 의존하지 않는 위상 불변량이다. [단체 호몰로지](homology.md)의 언어로는 Betti 수의 교대합이고, [Gauss–Bonnet 정리](gauss-bonnet.md)에서는 곡률의 총합과 일치한다. 정다면체가 다섯 개뿐이라는 분류도 이 등식에서 나온다.
...
증명의 요지는 그래프를 신장트리까지 줄여 값을 확인한 뒤 간선을 되돌려 붙이는 것이다. 같은 불변성이 삼각화의 세분과 면의 병합에서도 유지된다.
...
$$
\frac{1}{p}+\frac{1}{q}\gt\frac{1}{2},\qquad p\ge 3,\ q\ge 3
$$
...
$$
\chi=2-2g\ (\text{방향지음 가능, 종수 } g),\qquad \chi=2-k\ (\text{방향지음 불가능, 사영평면 } k \text{개})
$$
...
[^3]: 콤팩트 연결 곡면의 분류와 종수 g 방향지음 가능 곡면의 지표가 2-2g라는 사실. Wolfram MathWorld, "Surface Classification Theorem". https://mathworld.wolfram.com/SurfaceClassificationTheorem.html
