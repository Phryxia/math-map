...
계수 $\frac{1}{r+1}\binom{k}{r}\binom{k-1}{r}$ 이 Narayana 수이고, $\gamma = 1$ 에서 합이 Catalan 수 $C_k$ 가 된다. 처음 몇 개는 $m_1 = 1$, $m_2 = 1+\gamma$, $m_3 = 1 + 3\gamma + \gamma^2$ 이다. 적률은 $\frac1p\operatorname{tr}(S^k)$ 의 극한이므로 고윳값을 구하지 않고 행렬 곱만으로 확인할 수 있고, 아래 코드가 그렇게 한다.
...
고윳값 분해 없이도 법칙의 내용을 확인할 수 있다. 적률은 $\frac1p\operatorname{tr}(S^k)$ 이므로 행렬 곱만 하면 되고, 최대 고윳값은 거듭제곱법으로 얻는다.

```python
import math, random

def sample_cov(p, n, rng, spike=0.0):
    """S = X X^T / n.  spike>0 이면 첫 좌표의 분산을 1+spike 로 키운다."""
    X = [[rng.gauss(0, 1) for _ in range(n)] for _ in range(p)]
    if spike > 0:
        g = math.sqrt(1 + spike)
        X[0] = [g * v for v in X[0]]
    S = [[0.0] * p for _ in range(p)]
    for i in range(p):
        for j in range(i, p):
            s = sum(X[i][k] * X[j][k] for k in range(n)) / n
            S[i][j] = S[j][i] = s
    return S
...
def trace_power(S, k):
    ...
    M = [row[:] for row in S]
    for _ in range(k - 1):
        M = [[sum(M[i][t] * S[t][j] for t in range(p)) for j in range(p)] for i in range(p)]
    return sum(M[i][i] for i in range(p)) / p

def top_eig(S, iters=2000):
    """거듭제곱법으로 최대 고윳값."""
    p = len(S)
    v = [1.0 / math.sqrt(p)] * p
    lam = 0.0
    for _ in range(iters):
        w = [sum(S[i][j] * v[j] for j in range(p)) for i in range(p)]
...
def mp_moment(k, g):
    """Narayana 수로 쓴 MP 적률."""
    return sum(math.comb(k, r) * math.comb(k - 1, r) * g ** r / (r + 1) for r in range(k))

rng = random.Random(7)
print("  gamma   k     표본 tr(S^k)/p      MP 예측")
for p, n in ((40, 200), (60, 120), (80, 80)):
    g = p / n
    S = sample_cov(p, n, rng)
    for k in (1, 2, 3):
        print(f"  {g:5.3f}  {k}      {trace_power(S, k):.5f}          {mp_moment(k, g):.5f}")

print("\n 받침의 오른쪽 끝 (1+sqrt(gamma))^2 과 최대 고윳값")
for p, n in ((40, 400), (60, 240), (100, 200)):
    g = p / n
    S = sample_cov(p, n, rng)
    print(f"  gamma={g:5.3f}:  최대 {top_eig(S):.4f}   예측 {(1 + math.sqrt(g)) ** 2:.4f}")

print("\n BBP 전이:  p=60, n=240 이므로 gamma=0.25, 문턱 sqrt(gamma)=0.5")
for ell in (0.2, 0.5, 1.0, 2.0):
    g = 0.25
    S = sample_cov(60, 240, rng, spike=ell)
    pred = (1 + ell) * (1 + g / ell) if ell > math.sqrt(g) else (1 + math.sqrt(g)) ** 2
    print(f"  ell={ell:4.1f}:  최대 {top_eig(S):.4f}   예측 {pred:.4f}"
          f"   ({'튀어나옴' if ell > math.sqrt(g) else '묻힘'})")
```

적률이 잘 맞는다. $\gamma = 0.2$ 에서 세 적률이 $0.989, 1.186, 1.626$ 으로 예측 $1, 1.2, 1.64$ 에 $1\%$ 안쪽이고, $\gamma = 0.5$ 에서도 마찬가지다. $\gamma = 1$ 에서 $3$ 차 적률이 $5.64$ 로 예측 $5$ 를 $13\%$ 넘어서는데, $p = n = 80$ 은 받침이 원점에 닿는 임계 상황이라 유한 크기 효과가 가장 크게 나타나는 자리다.

받침의 끝은 늘 조금씩 **아래로** 빗나간다. $\gamma = 0.25$ 에서 $2.233$ 대 예측 $2.25$, $\gamma = 0.5$ 에서 $2.720$ 대 $2.914$ 다. 무작위 요동이면 양쪽으로 갈릴 텐데 한 방향으로만 어긋난다. 위에서 말한 Tracy–Widom 편향이며, 최대 고윳값은 평균적으로 받침 끝보다 안쪽에 있다.

BBP 전이도 보인다. $\ell = 0.2$ 와 $\ell = 0.5$ 에서 최대 고윳값이 $2.17$ 과 $2.22$ 로 잡음만 있을 때의 $2.25$ 와 구별되지 않는다. 문턱을 넘긴 $\ell = 1$ 과 $\ell = 2$ 에서는 $2.40$ 과 $3.06$ 으로 덩어리 밖으로 나온다. 극한값 $2.50$ 과 $3.38$ 에는 아직 못 미치는데, $p = 60$ 인 한 표본이라 튀어나온 고윳값의 $O(p^{-1/2})$ 요동이 그대로 남아 있다. 전이가 있다는 것은 분명히 보이지만 위치를 자릿수까지 재려면 표본을 평균해야 한다. 특히 $\ell = 0.5$ 가 정확히 문턱인데, 참 공분산의 그 방향 분산이 $1.5$ 로 나머지보다 $50\%$ 나 큰데도 고윳값만 보아서는 전혀 드러나지 않는다. 신호의 세기가 아니라 $p/n$ 이 감지 가능성을 정한다는 말의 구체적인 모습이다.
