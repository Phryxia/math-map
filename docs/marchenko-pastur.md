...
참 공분산에 방향 하나가 강조된 $\Sigma = I + \ell vv^{\mathsf T}$ 를 본다. $\ell$ 이 크면 $S$ 의 최대 고윳값이 잡음 덩어리 밖으로 튀어나오고, 작으면 덩어리 안에 묻혀 구별되지 않는다. 그 경계는 $\ell \to 0$ 이 아니라 유한한 문턱 $\ell = \sqrt\gamma$ 다. 그보다 약한 신호는 표본을 아무리 정확히 계산해도 고윳값만으로는 감지되지 않는다. 자료의 양이 아니라 $p$ 와 $n$ 의 비가 정하는 한계다.
