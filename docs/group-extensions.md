...
가 성립한다. 이 보조정리로 임의의 두 부분정규열이 동형인 세분을 갖는다는 Schreier 세분 정리를 얻고, 합성열은 더 세분할 수 없으므로 두 합성열이 동형이다. 이것이 Jordan–Hölder 정리이고, 합성인자가 유일하게 정해진다.

합성인자가 유일하므로 유한군의 분류는 두 문제로 갈라진다. 단순군을 모두 찾는 문제는 유한 단순군 분류가 답했고, 찾은 조각을 다시 붙이는 문제가 확대 문제이자 $H^2$ 의 계산이다.
