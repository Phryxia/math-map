...
시간 간격 $h$ 마다 $\pm\sqrt{h}$ 만큼 움직이는 대칭 무작위 걷기를 본다. 시각 $t$ 까지의 위치는 약 $t/h$ 번의 독립 걸음의 합이므로 분산이 $(t/h) \cdot h = t$ 로 $h$ 와 무관하다. 걸음 크기를 $h$ 가 아니라 $\sqrt{h}$ 로 잡았기 때문이다. 중심극한정리로 극한 분포는 $N(0, t)$ 다. 이 극한이 분포 수준이 아니라 경로 수준에서도 존재한다는 것이 Donsker 불변원리다.
