...
근사비 $\rho$ 를 보장한다는 말은 모든 입력에 대해 다음이 성립한다는 뜻이다(최소화 문제 기준).
...
위의 매칭 논증이 주는 2-근사는 매우 단순하지만 수십 년째 본질적으로 개선되지 않았다. 유일 게임 추측을 가정하면 $2-\varepsilon$ 근사는 불가능하고, 가정 없이도 1.36 보다 나은 근사는 $\mathrm{P} = \mathrm{NP}$ 를 함의한다. 아래 실험에서 보듯 근사비 2 는 실제로 달성되는 값이며 분석이 헐렁해서 생긴 여유가 아니다.

## 집합 덮개: 탐욕과 로그 인자

남은 원소를 가장 많이 덮는 집합을 반복해서 고르는 탐욕 알고리즘은 $H_n \approx \ln n$ 근사를 준다. 각 단계에서 남은 원소 수가 $1 - 1/\mathrm{OPT}$ 배 이하로 줄어들기 때문이다.

$$
r_{t+1}\le r_t\Big(1-\frac{1}{\mathrm{OPT}}\Big)\ \Rightarrow\ \text{단계 수}\le\mathrm{OPT}\cdot\ln n
$$

이 로그 인자는 없앨 수 없다. $(1−o(1))\ln n$ 보다 나은 근사는 $\mathrm{P} = \mathrm{NP}$ 를 함의한다는 것이 알려져 있다. 즉 탐욕 알고리즘이 이미 최선이다.
