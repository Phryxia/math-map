...
표수 $0$ 에서 $E[p]\cong(\mathbb Z/p)^2$ 이던 것이 표수 $p$ 에서 절반 또는 전부 무너진다. 초특이 곡선은 $p$ 비틀림 점이 하나도 없는 쪽이고, 동치인 조건이 $a_p=0$ 이고 곧 $\char"23{}E(\mathbb F_p)=p+1$ 이다.
...
가 된다. 대부분의 곡선은 $\char"23{}\mathrm{Aut}=2$ ($\pm1$ 뿐) 이므로 정점 수가 대략 $(p-1)/12$ 이고, $j=0$ (자기동형 6개) 과 $j=1728$ (4개) 이 있을 때만 보정항 $\varepsilon_p$ 가 붙는다. 위 공식의 $12$ 와 $\varepsilon_p$ 가 이렇게 설명된다.
