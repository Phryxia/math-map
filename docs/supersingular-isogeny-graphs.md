...
로 유한하다. 이 유한집합을 정점으로 두고 $\ell$ 차 동종사상을 간선으로 두면 $(\ell+1)$ 정규 그래프가 되고, 그것이 **Ramanujan 그래프**다. 스펙트럼 간극이 이론적 최댓값에 도달해 [random walk](random-walks.md)가 가장 빠르게 섞인다.

빠르게 섞이는 그래프에서 두 정점 사이의 경로를 찾는 일은 어렵다. 이 어려움을 가정으로 CGL 해시함수와 SIDH 키교환이 설계되었고, 후자는 NIST 후양자 표준화의 최종 후보(SIKE)까지 올라갔다가 2022 년에 깨졌다.
...
정점을 $\mathbb F_{p^2}$ 의 초특이 $j$ 불변량으로, 간선을 $\ell$ 차 동종사상($\ell\ne p$ 인 작은 소수)으로 둔다. 차수 $\ell$ 의 동종사상은 크기 $\ell$ 의 부분군 하나에 대응하고, $E[\ell]\cong(\mathbb Z/\ell)^2$ 의 크기 $\ell$ 부분군은 $\mathbb P^1(\mathbb F_\ell)$ 만큼 $\ell+1$ 개 있다.
...
이 Ramanujan 조건을 준다[^1]. 그 결과 이 그래프 위의 random walk 는 $O(\log p)$ 걸음이면 거의 균등분포에 도달한다.
...
$B_{p,\infty}$ 는 $p$ 와 $\infty$ 에서만 분지하는 유일한 유리 사원수대수다. 이 대응에서 동종사상은 좌아이디얼이 되고 그래프는 아이디얼 류의 그래프가 된다. 곡선 쪽 문제를 격자 쪽으로 옮기는 통로이며 공격과 방어 양쪽의 핵심 도구다.
...
보조점이 없으면 두 걸음을 합성할 수 없고, 그 보조점이 공격의 통로가 되었다.
