...
$\char35{}E(\mathbb F_q)$ 를 정확히 세는 것은 Schoof 알고리즘으로 다항시간에 가능하다. 암호용 곡선을 만들 때 위수가 큰 소인수를 가지는지 확인하는 절차가 이것이다.
...
취약한 곡선도 있다. $\char35{}E(\mathbb F_p)=p$ 인 이상 곡선에서는 가법군으로의 동형이 존재해 이산로그가 다항시간에 풀린다. 매장 차수가 작으면 Weil 쌍으로 문제를 유한체로 옮기는 MOV 공격이 통한다. 그래서 표준 곡선을 쓰거나 생성 시 이 조건들을 검사한다.
