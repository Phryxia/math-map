...
**Expander** 는 차수가 상수로 묶여 있으면서 어떻게 잘라도 잘린 면이 큰 [그래프](graphs.md)다. 완전그래프는 간선이 $n^2$ 개이고 나무는 하나만 끊어도 갈라지는데, expander 는 차수 3 이나 4 로 정점 수와 무관하게 잘 섞이는 성질을 유지한다.
...
전도도가 작으면 그래프에 병목이 있다. 두 덩어리를 잇는 간선이 적을 때 한쪽에서 $+1$ , 다른 쪽에서 $-1$ 인 벡터를 만들면 경계가 작아 $M$ 을 곱해도 거의 변하지 않고, $1$ 에 가까운 고윳값이 존재한다. 이것이 부등식의 쉬운 쪽 $1-\lambda_2\le2h$ 다.
