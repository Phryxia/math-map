...
간극은 [그래프 Laplacian](graph-laplacian.md)의 두 번째로 작은 고윳값이기도 하다. Expander 는 무작위성을 아껴 쓰는 알고리즘, 오류정정부호, PCP 정리의 조합적 증명에서 간극을 증폭하는 장치로 쓰인다.
...
expander 그래프에서 만든 부호(expander code, Sipser–Spielman)는 선형 시간 복호가 가능하면서 상수 비율과 상수 상대 거리를 갖는다. 양자 LDPC 부호와 상수 비율 국소 검사 가능 부호의 구성도 expander 계열의 대상(제곱 복합체, 좌우 Cayley 복합체) 위에서 이루어졌다. 공통 원리는 그래프의 확장성이 부호의 거리로 번역되는 것이다.
