...
중심극한정리는 독립 [확률변수](random-variables.md)들의 합을 표준화하면 그 분포가 원래 분포와 거의 무관하게 정규분포로 수렴한다는 정리다.
...
척도의 선택이 결정적이다. $n$ 으로 나누면 요동이 사라져 상수만 남고, 나누지 않으면 요동이 무한히 커진다. $n$ 의 제곱근으로 나누는 것이 요동을 남기면서 수렴시키는 유일한 척도다.
