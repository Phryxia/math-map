...
확률변수열 $Y_n$ 이 $Y$ 로 분포수렴(convergence in distribution)한다는 것은 누적분포함수가 극한의 연속점마다 수렴한다는 뜻이다. 이는 앞선 수렴 개념들보다 약하다. 확률수렴은 분포수렴을 함의하지만 역은 성립하지 않는다.
...
즉 모든 실수 $x$ 에 대해 좌변이 $x$ 이하일 확률이 $\Phi(x)$ 로 수렴한다. 요구되는 조건은 독립, 동일분포, 유한하고 0이 아닌 분산뿐이다. 분포의 모양이나 대칭성, 고차 모멘트의 존재는 필요하지 않다.

# 성질

## 특성함수를 이용한 증명 개요

$Z_i$ 를 $X_i$ 에서 평균을 빼고 표준편차로 나눈 변수라 하자. 평균 0, 분산 1 이다. 특성함수 $\varphi$ 는 2 차 모멘트가 존재하므로 0 근방에서 2차까지 Taylor 전개된다.
...
분산이 무한하면 정규분포 극한이 사라진다. 꼬리가 무겁게 감소하는 분포(안정지수 $\alpha$ 가 2 보다 작은 경우)에서는 $n^{1/\alpha}$ 으로 나누어야 수렴하고, 극한은 정규분포가 아닌 $\alpha$ 안정분포다. Cauchy 분포가 $\alpha=1$ 의 예로, 표본평균이 다시 Cauchy 분포라서 집중조차 하지 않는다. 독립성이 심하게 깨진 경우도 결론이 달라지며, 약한 의존 아래에서는 극한 분산에 자기상관 항이 더해진다.
