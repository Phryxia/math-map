...
주사위를 두 번 던져 눈의 합을 볼 때 확률이 정의된 공간은 36 가지 순서쌍이고 관심 대상은 $2$ 부터 $12$ 까지의 합이다. 합이 $7$ 일 확률은 합이 $7$ 이 되는 순서쌍들의 확률이다. 값 쪽의 집합을 함수로 되돌려 원래 측도로 재는 것이 상측도이고, [가측함수](measurable-functions.md)의 정의가 이 되돌리기를 보장한다.

[확률변수](random-variables.md)의 분포, 통계량의 분포, 좌표변환 후의 분포가 모두 상측도다. 이 개념 덕분에 원래 표본공간을 잊고 분포만으로 작업할 수 있다.

# 직관

## 질량의 이동

측도를 공간에 흩어진 질량으로 보면 상측도는 함수 $\varphi$ 가 각 점의 질량을 $\varphi(x)$ 자리로 옮긴 결과다. 질량의 총량이 보존되므로 $\mu$ 가 확률측도면 $\varphi_\ast\mu$ 도 확률측도다.

여러 점이 같은 곳으로 실려 가면 질량이 뭉친다. 주사위 눈의 홀짝을 보면 세 점씩 뭉쳐 $1/2$ 씩이 된다. 요약 통계량이 하는 일이 이 뭉침이다.
