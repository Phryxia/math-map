...
측도를 "공간에 흩어져 있는 질량" 으로 생각하면, 상측도는 함수 `φ` 가 각 점의 질량을 `φ(x)` 자리로 실어 나른 결과다. 질량은 사라지지도 생기지도 않으므로 `μ` 가 확률측도면 `φ_*μ` 도 확률측도다.

여러 점이 같은 곳으로 실려 가면 질량이 뭉친다. 주사위 눈의 홀짝을 보면 세 점씩 뭉쳐 `1/2` 씩이 된다. 정보를 잃는 대신 다루기 쉬워지는 이 뭉침이 요약 통계량의 본질이다.

## 정의는 역상으로, 계산은 순방향으로

정의식에 역상 `φ⁻¹(B)` 가 등장하는 것이 헷갈리는 지점이다. 측도는 함수와 같은 방향으로 밀려가는데 정의는 반대 방향의 역상을 쓴다.

이유는 측도가 집합을 받는 함수이기 때문이다. 집합 자체는 함수를 따라 앞으로 갈 수 없다. `φ(A)` 가 가측이라는 보장이 없기 때문이다. 반면 역상은 여집합과 가산 합집합을 그대로 보존하므로 측도의 구조가 온전히 옮겨진다. 그래서 대상은 앞으로 가고 정의는 뒤로 간다.
...
측도 공간 `(X, Σ, μ)` 와 가측 공간 `(Y, T)`, 가측함수 `φ : X → Y` 에 대해

$$
(\phi_*\mu)(B)=\mu\!\left(\phi^{-1}(B)\right),\qquad B\in\mathcal T
$$

를 상측도라 한다. `μ` 가 확률측도이면 `φ_*μ` 도 확률측도이며, 확률변수 `φ` 의 분포 또는 법칙이라 부른다.

## 측도임의 확인

서로소인 `B₁, B₂, …` 의 역상은 서로소이고 역상이 가산 합집합을 보존하므로

$$
(\phi_*\mu)\Bigl(\bigsqcup_n B_n\Bigr)=\mu\Bigl(\bigsqcup_n\phi^{-1}(B_n)\Bigr)=\sum_n\mu(\phi^{-1}(B_n))
$$

이다. `φ_*μ(∅) = μ(∅) = 0` 이므로 `φ_*μ` 는 측도다. 이 확인이 짧게 끝나는 것이 전적으로 역상의 좋은 성질 덕분이다.

# 성질

## 변수변환 공식

음이 아닌 가측함수 `g : Y → [0,∞]` 에 대해
...
같은 분포를 주는 서로 다른 표본공간이 얼마든지 있다. 동전 던지기를 `{앞, 뒤}` 위에서 모형화하든 `[0,1]` 위의 균등분포와 `1_{[0,1/2]}` 로 모형화하든 결과의 분포는 같다.
...
`Y = ℝⁿ` 이고 `φ` 가 미분동형이면 상측도의 밀도가 Jacobi 행렬식으로 주어진다.

$$
p_Y(y)=p_X(\phi^{-1}(y))\,\bigl|\det D\phi^{-1}(y)\bigr|
$$

다변수 치환적분이 이 공식의 특수한 경우다. `φ` 가 단사가 아니면 각 역상 가지의 기여를 더해야 하고, 차원을 줄이면 밀도가 아예 존재하지 않을 수 있다. 정규화 흐름 같은 생성 모형이 가역 변환만 쓰는 이유가 이 공식을 쓰기 위해서다.
