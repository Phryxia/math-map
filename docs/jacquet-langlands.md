...
\thinspace\hookrightarrow\thinspace
...
실용적 가치는 방향에 있다. $\Sigma\ne\varnothing$ 이면 $B^\times$ 쪽 몫공간이 콤팩트해서 첨점도 [Eisenstein 급수](eisenstein-series.md)도 [연속 스펙트럼](speh-representations.md)도 없고, 모든 것이 이산이며 유한차원 선형대수로 계산된다. JL 대응은 어려운 해석이 사는 세계의 문제를 유한 행렬의 문제로 옮긴다. [Waldspurger 정리](waldspurger-formula.md)가 사원수 대수 위에서 주기를 재는 것도 같은 이유다.

# 직관

## 콤팩트 몫공간

$\mathrm{GL}\_2$ 의 자기동형 형식은 [모듈러 곡선](modular-curves.md) 위에 산다. 모듈러 곡선은 비콤팩트하고 첨점이 있어 $L^2$ 스펙트럼에 연속 부분이 생기고 Eisenstein 급수를 따로 다뤄야 한다. 첨점형식이라는 조건이 이 불편을 피하는 장치다.
...
증명의 요지는 [Selberg 대각합 공식](selberg-trace-formula.md)의 비교다. 두 군에서 각각 대각합 공식을 쓰면 양변이 스펙트럼 쪽과 기하 쪽의 등식이 되고, 기하 쪽은 켤레류 위의 궤도적분의 합이다.
...
레벨 $p$ 의 첨점형식 Hecke 고윳값을 $\mathrm{GL}\_2$ 쪽에서 구하려면 [모듈러 기호](modular-symbols.md)나 $q$ 전개를 다뤄야 하지만, $B^\times$ 쪽에서는 $h(p)$ 차원 공간 위의 정수 행렬을 대각화하면 된다. $p=11$ 이면 $2\times2$ 행렬이다. 이것이 Brandt 행렬 알고리즘이고, 무게가 크거나 총실수체 위의 Hilbert [모듈러 형식](modular-forms.md)처럼 $q$ 전개가 어려운 경우에 쓰인다.
...
Deuring 대응에 의해 확정 사원수 대수의 극대차수는 표수 $p$ 의 초특이 타원곡선의 자기준동형환이다. 위에서 센 이념류가 초특이 곡선들과 대응하고, Brandt 행렬은 그 곡선들을 잇는 등원사상 그래프의 인접행렬이다. 모듈러 형식의 Hecke 작용소, 사원수 대수의 이념류, 초특이 곡선의 등원사상 그래프가 같은 유한 행렬로 만난다. 이 그래프는 Ramanujan 그래프이고 등원사상 기반 암호가 여기서 구성된다.
