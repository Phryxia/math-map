...
[Waldspurger 정리](waldspurger-formula.md)는 토릭 주기를 사원수 대수 위에서 잰다. 어느 $B$ 위에서 주기가 소멸하지 않는지가 $\varepsilon$ 부호로 정해지고, 그 $B$ 위의 형식을 다시 $\mathrm{GL}\_2$ 로 옮기는 것이 JL 대응이다. $L$ 함수가 양쪽에서 같으므로 주기가 말하는 중심값이 원래 형식의 중심값이 된다.
