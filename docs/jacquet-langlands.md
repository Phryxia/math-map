...
레벨 $p$ 의 첨점형식 Hecke 고윳값을 $\mathrm{GL}\_2$ 쪽에서 구하려면 모듈러 기호나 $q$ 전개를 다뤄야 하지만, $B^\times$ 쪽에서는 $h(p)$ 차원 공간 위의 정수 행렬을 대각화하면 된다. $p=11$ 이면 $2\times2$ 행렬이다. 이것이 Brandt 행렬 알고리즘이고, 무게가 크거나 총실수체 위의 Hilbert [모듈러 형식](modular-forms.md)처럼 $q$ 전개가 어려운 경우에 쓰인다.
