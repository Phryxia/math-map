...
- **균등수렴**: 모든 점에 대해 공통으로 작동하는 항 번호가 있어야 한다. 극한함수 주위로 폭 $\varepsilon$ 의 띠(tube)를 그렸을 때, 어느 시점부터 모든 그래프가 그 띠 안에 완전히 들어가야 한다.
...
논리 기호로 풀면, 임의의 $x$ 와 임의의 양수 $\varepsilon$ 에 대해 어떤 $N$ 이 존재하여 $n \ge N$ 이면 $|f_n(x) - f(x)| < \varepsilon$ 다. 여기서 $N$ 은 $x$ 와 $\varepsilon$ 에 모두 의존한다.

## 균등수렴

임의의 양수 $\varepsilon$ 에 대해 어떤 $N$ 이 존재하여, $n \ge N$ 이면 **모든** $x$ 에 대해 $|f_n(x) - f(x)| < \varepsilon$ 다. 동치로

$$
\lim_{n \to \infty} \ \sup_{x \in X} \ |f_n(x) - f(x)| = 0 .
$$

이때 $f_n$ 이 $f$ 로 균등수렴한다고 한다. 양화사 순서만 바꾼 것, 곧 $\forall x\thinspace\exists N$ 과 $\exists N\thinspace\forall x$ 의 차이지만, 그 차이가 전부다.
...
$f_n$ 이 **균등 Cauchy** 라는 것은 임의의 양수 $\varepsilon$ 에 대해 어떤 $N$ 이 존재하여 $m, n \ge N$ 이면 모든 $x$ 에서 $|f_n(x) - f_m(x)| < \varepsilon$ 라는 뜻이다. 목표 함수 $f$ 를 미리 알지 못해도 균등수렴을 판정할 수 있다는 점에서 실용적이다.
...
첫째와 셋째 항은 균등수렴으로 $n$ 을 크게 잡아 $\varepsilon/3$ 미만으로 만든다(여기서 $x$ 와 무관하게 잡을 수 있다는 점이 결정적이다). $n$ 을 고정한 뒤 $f_n$ 의 연속성으로 $x$ 를 $a$ 에 충분히 가깝게 하여 가운데 항을 $\varepsilon/3$ 미만으로 만든다.
...
**정리.** 각 $n$ 에 대해 상수 $M_n$ 이 있어 모든 $x$ 에서 $|u_n(x)| \le M_n$ 이고 $sum M_n$ 이 수렴하면, 급수 $sum u_n$ 은 균등(그리고 절대) 수렴한다.
...
**증명 스케치.** $g_n = |f_n - f|$ 는 연속이고 점별로 0 으로 단조 감소한다. 임의의 $\varepsilon$ 에 대해 열린집합 $U_n = \lbrace x : g_n(x) < \varepsilon\rbrace$ 는 증가하는 열린덮개이고, [compactness](compactness.md)에 의해 유한 부분덮개가 있으므로 어떤 $N$ 에서 $U_N$ 이 $K$ 전체를 덮는다. 단조성으로 그 이후 모든 $n$ 에서도 성립한다.
