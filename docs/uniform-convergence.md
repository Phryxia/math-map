...
전형적인 반례가 $[0,1]$ 위의 $f_n(x)=x^n$ 이다. 각 점의 극한이 $x<1$ 에서 0, $x=1$ 에서 1 이라 극한함수가 불연속이고, 1 에 가까운 점일수록 0 에 도달하는 데 오래 걸려 공통 $N$ 을 잡을 수 없다.
...
$f_n$ 이 **균등 Cauchy** 라는 것은 임의의 양수 $\varepsilon$ 에 대해 어떤 $N$ 이 존재하여 $m,n\ge N$ 이면 모든 $x$ 에서 $|f_n(x)-f_m(x)|<\varepsilon$ 라는 뜻이다. 목표 함수 $f$ 를 모르고도 균등수렴을 판정한다.
