...
**증명 스케치.** $g_n = |f_n - f|$ 는 연속이고 점별로 0 으로 단조 감소한다. 임의의 $\varepsilon$ 에 대해 열린집합 $U_n = {x : g_n(x) < \varepsilon}$ 는 증가하는 열린덮개이고, [compactness](compactness.md)에 의해 유한 부분덮개가 있으므로 어떤 $N$ 에서 $U_N$ 이 $K$ 전체를 덮는다. 단조성으로 그 이후 모든 $n$ 에서도 성립한다.
