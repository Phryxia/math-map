...
수를 다루는 Hume 원리는 무모순이고 그것만으로 산술 전체가 나온다. 외연을 다루는 기본 법칙 V 는 모순이다. Frege 는 후자를 통해 전자를 정당화하려 했고, 신논리주의는 전자를 직접 받아들이자고 제안한다.
...
여기서 Frege 의 전략이 나온다. 먼저 "같은 수를 가진다"를 정의하고, 그다음 그 동치관계의 추상화로 수라는 대상을 얻는다. 방향을 잡을 때 쓰는 비유가 직선의 방향이다. 두 직선이 평행하다는 관계를 먼저 정의하고 "직선의 방향"을 그 [동치류](equivalence-relations.md)로 얻듯이, "개념의 수"를 동수성의 동치류로 얻는다.
...
**정의.** 개념에 대상을 대응시키는 연산 $\char35{}$ 에 대해 다음이 성립한다는 것이 Hume 원리다.
...
**정의.** 개념 $F$ 에 그 값범위 또는 외연 $\mathrm{ext}(F)$ 를 대응시키는 연산에 대해 다음이 성립한다는 것이 기본 법칙 V 다.

$$
\mathrm{ext}(F) = \mathrm{ext}(G) \iff \forall x\thinspace(Fx \leftrightarrow Gx)
$$

형태는 Hume 원리와 같지만 우변의 동치관계가 훨씬 촘촘하다. 동수성은 개념들을 크기에 따라 거칠게 나누지만, 외연 동일성은 개념을 거의 낱낱으로 나눈다. 그만큼 많은 대상을 요구하고, 그것이 모순의 원인이 된다.

## Frege 의 수 정의

Frege 의 저작은 셋이다. *Begriffsschrift*(1879)가 양화 논리를 세우고, *Grundlagen der Arithmetik*(1884)가 수를 정의하고, *Grundgesetze der Arithmetik*(1893)가 기본 법칙 V 로 그 정의를 형식화한다. Russell 이 1902 년에 기본 법칙 V 에서 모순을 끌어냈다.
...
*증명 스케치.* 기본 법칙 V 는 서로 다른 개념마다 서로 다른 외연을 요구한다. 개념은 대상들의 부분모임만큼 많고 외연은 대상이므로, 멱집합에서 원래 영역으로 가는 단사가 있어야 한다. Cantor 의 대각선 논법이 이것을 금지한다. 구체적으로 "자기 자신의 외연에 속하지 않는다"는 개념을 잡아 그 외연을 물으면 즉시 모순이 나온다.

Russell 이 1902년 편지로 이를 알렸을 때 Frege 의 저작 2권은 인쇄 중이었다. Frege 는 부록에서 수정안을 제시했으나 그 수정안도 모순이었다. Frege 는 결국 프로그램을 포기했고 말년에는 기하학적 직관에서 산술을 찾으려 했다.
...
대가가 컸다. 분지 구조 때문에 실수의 상한 존재 같은 해석학의 기본 정리가 도출되지 않아 환원 공리(axiom of reducibility)를 도입해야 했고, 산술을 얻으려면 무한 공리가, 기수 이론을 얻으려면 [선택공리](axiom-of-choice.md)에 해당하는 곱셈 공리가 필요했다. 이 셋은 어느 기준으로도 논리 법칙으로 보기 어렵다. 그 결과 "수학은 논리다"라는 강한 주장은 "수학은 논리와 몇 개의 존재 가정이다"로 약해졌다.

## 신논리주의

Wright 는 1983년에, 이후 Hale 과 함께, Frege 정리에서 출발해 프로그램을 재건하자고 제안했다. 요지는 기본 법칙 V 를 버리고 Hume 원리를 직접 공리로 채택하는 것이다. Hume 원리가 수 개념에 대한 분석적 진리이거나 적어도 개념 구성적(implicit definition) 원리라면, 산술의 인식론적 지위가 그로부터 설명된다는 주장이다[^2].
...
**2차 논리의 지위.** Frege 정리는 2차 논리를 필요로 한다. 2차 논리는 표준 의미론에서 완전성과 콤팩트성을 모두 잃는다는 점에서([Löwenheim–Skolem 정리](lowenheim-skolem.md)) 1차 논리와 성격이 다르다. Quine 은 이를 두고 2차 논리가 "양의 탈을 쓴 집합론"이라 했다. 2차 논리가 논리가 아니라면 논리주의의 환원이 논리에 기댄 것이 아니다.
...
유한 우주에서 개념을 부분집합으로, 수를 동수성 동치류로 두면 Hume 원리의 모형이 나온다. 이 모형에서 Peano 공리 가운데 셋은 성립하고 후속자의 존재만 실패한다. 유한 우주에서는 세어질 대상이 바닥난다.

Frege 는 수 자신을 세어지는 대상으로 썼다. "0 부터 $n$ 까지"라는 개념의 수가 $n$ 의 후속자이므로 대상의 공급이 끊기지 않는다. Hume 원리는 이렇게 무한 공리 없이 무한을 얻고, Principia 는 무한 공리를 따로 요구했다.

# 활용

## 집합론과의 관계

기본 법칙 V 의 실패는 "모든 개념에 외연이 있다"는 무제한 내포 원리를 버려야 한다는 교훈으로 이어졌다. [ZFC 공리계](zfc-axioms.md)(Zermelo–Fraenkel 집합론에 선택공리를 더한 것)의 분리 공리꼴은 이미 있는 집합 안에서만 부분집합을 만들도록 제한해 같은 문제를 피한다. 논리주의가 실패한 자리에 공리적 [집합론](sets.md)이 들어섰지만, 그 대가로 집합 존재 주장들은 논리적 진리라는 지위를 잃었다.
