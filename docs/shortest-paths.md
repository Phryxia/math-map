...
이 **Bellman 방정식**은 양쪽에 미지의 $d$ 가 있으므로 정의가 아니라 고정점 방정식이고, [동적 계획법](dynamic-programming.md)이 푸는 대상이다. 그래프가 DAG 면 위상순서대로 한 번 훑어 풀린다. 사이클이 있으면 모든 거리를 $\infty$ 로 두고 우변을 좌변에 대입하기를 되풀이하며, 값이 단조 감소하고 음수 사이클이 없으면 $|V|-1$ 단계 안에 멈춘다. 이것이 Bellman–Ford 알고리즘이다.
...
- **강화학습의 값 반복.** Bellman 방정식의 확률적 판본에서 같은 반복이 돌아간다. 할인율이 수축사상을 만들어 수렴을 보장하는 자리가, 여기서 음수 사이클이 없어 사슬이 멈추는 자리에 대응한다.
