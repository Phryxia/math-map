...
sine 커널 쪽이 본론이다. $n=10$ 과 $n=40$ 이 12 자리까지 같은 값을 내놓는다. 마디 열 개로 무한차원 행렬식을 열두 자리까지 얻은 것이고, 이것이 해석적 커널에서 Nyström 근사가 갖는 지수수렴이다. 마지막 열은 $1 - s$ 에서 벗어난 양을 $s^4$ 로 나눈 것인데, $s = 0.05$ 에서 $0.2738$ 이 나와 알려진 전개 계수 $\pi^2/36 = 0.2742$ 를 네 자리까지 준다. $s$ 를 키우면 이 비가 $0.2684$($s=0.2$)로 내려가고 $s \ge 1$ 에서는 고차항이 지배해 뜻을 잃는다. 급수의 $2$ 차와 $3$ 차 항이 정확히 상쇄되어 $s^4$ 가 첫 보정이라는 사실까지 수치가 확인해 준다.
