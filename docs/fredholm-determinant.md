...
Hadamard 부등식이 $k \times k$ 행렬식을 행 노름의 곱으로 누르므로 급수의 $k$ 번째 항이 $k^{k/2}M^k/k!$ 규모로 눌린다. $k!$ 이 $k^{k/2}$ 를 압도하므로 급수가 모든 $K$ 에 대해 완전함수로 수렴한다. 수렴 반경이 무한대라는 것이 Fredholm 이론의 출발점이다.
