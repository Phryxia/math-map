...
**정의(GCH).** 일반화 연속체 가설은 모든 무한 기수에 대해 다음이 성립한다는 주장이다. 여기서 `κ` 의 오른쪽 위 첨자는 바로 다음 기수를 뜻한다.
...
일반 필터는 이 조각들을 이어 붙여 `κ` 개의 새 실수를 만들어 낸다.
...
*증명 스케치.* `L` 은 추이적이고 모든 서수를 포함하므로 ZF 의 공리들이 상대화되어 성립한다. 선택공리는 `L` 의 원소들이 구성 순서에 따라 정의 가능하게 잘 정렬되기 때문에 나온다. GCH 는 응축 보조정리(condensation)에서 나온다. `L_α` 의 기본 부분구조를 잡아 추이 붕괴하면 다시 어떤 `L_β` 가 되고, 이 성질에서 `L` 안의 `κ` 의 부분집합들이 모두 `L` 의 낮은 단계에 이미 나타남을 얻어 개수 상한이 정해진다.
...
*증명 스케치.* 가산 추이 모형 `M` 에서 시작해 위의 `P` 로 확장을 만든다. `κ` 를 `M` 이 알레프 둘이라고 여기는 기수로 잡으면, 일반 필터가 서로 다른 `κ` 개의 실수를 준다. "서로 다르다"는 것은 임의의 두 첨자에 대해 어느 자리에서 값이 갈리도록 만드는 조건들의 모임이 조밀하기 때문이다.
