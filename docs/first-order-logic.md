...
- 하향 Löwenheim–Skolem: 가산 언어의 이론이 무한 모델을 가지면 가산 모델을 가진다. 이 정리에서 ZFC가 무모순이면 ZFC에 가산 모델이 있다는 Skolem 역설이 따라온다. 역설처럼 보일 뿐 모순은 아니다. 모델 내부에서 "비가산"이라는 판정은 그 모델이 가진 함수들만으로 이루어지기 때문이다([가산성과 비가산성](cardinality.md)).
