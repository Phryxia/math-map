...
하나의 증명은 유한히 많은 공리만 쓴다. 따라서 모순이 유도되면 유한 부분집합에서 이미 모순이 유도된다. 이를 의미론 쪽으로 옮기면 "유한 부분집합이 모두 만족 가능하면 전체가 만족 가능하다"가 된다. [콤팩트성](compactness.md)라는 이름은 이 성질이 적절한 위상에서 실제로 compact성과 같다는 점에서 왔다.
