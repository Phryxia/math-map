...
답이 정수라는 것, 그 크기를 아르키메데스 쪽에서 안다는 것, $p$ 진 쪽에서 유한 정밀도로 계산할 수 있다는 것 셋이 모이면 근사가 정확해진다. Dwork 의 유리성 증명에서 Borel–Dwork 판정이 두 절댓값을 묶은 것과 같은 논법이고, 거기서는 존재 증명이며 여기서는 계산이다.
...
$p\le71$ 에서 이 곡선이 초특이가 되는 소수는 $17$ 하나다. 초특이 소수는 희박하며 Elkies 가 무한히 많음을 증명했다. Newton 다각형의 일반적 위치가 보통이라는 것과 같은 내용이다.
...
- **수치 실험.** 대량의 $a_p$ 표가 [Sato–Tate 분포](sato-tate.md)와 BSD(Birch–Swinnerton-Dyer) 추측의 수치 검증, 모듈러성 판정의 근거가 된다. LMFDB(L-functions and modular forms database) 의 곡선 자료가 이 계열 알고리즘의 산출물이다.
