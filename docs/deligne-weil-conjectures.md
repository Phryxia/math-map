...
무게 개념은 Deligne 이 두 번째 논문(*Weil II*, 1980)에서 크게 확장했다. 층 `\mathcal F` 가 **무게 `\le w`** 라 함은 모든 닫힌점에서 Frobenius 고윳값의 절댓값이 `q_x^{w/2}` 이하라는 뜻이다. 핵심 정리는 다음이다.
...
이 **Ramanujan–Petersson 추측**이다. `k=12`, `N=1` 이 판별식 `\Delta` 이고 `a_p=\tau(p)` 다.
...
`\alpha_p\beta_p=p^{k-1}` 이므로 `\alpha_p=p^{(k-1)/2}e^{i\theta_p}`, `\beta_p=\overline{\alpha_p}` 로 쓸 수 있고
...
곡선(`d=1`)의 경우는 Weil 자신이 1948 년에 증명했다. 곡면에 대해서는 Deligne 이전에도 부분적 결과가 있었지만, 임의 차원은 1974 년까지 열려 있었다.
...
`\tau(2)=-24`, `\tau(3)=252`, `\tau(5)=4830` 은 잘 알려진 값과 일치한다. 비율이 `0.16` 에서 `0.59` 사이에 흩어져 있고 한 번도 `1` 을 넘지 않는다. 1916 년의 추측을 1974 년의 정리가 닫은 그 부등식이다.
...
두 번째 등식 `\tau(p^2)=\tau(p)^2-p^{11}` 이 바로 `\alpha_p+\beta_p=\tau(p)`, `\alpha_p\beta_p=p^{11}` 에서 나오는 `\alpha_p^2+\beta_p^2` 다. Frobenius 고윳값 두 개가 Hecke 작용소의 고윳값으로 보이는 자리다.
