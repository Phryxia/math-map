...
유리성과 함수방정식은 코호몰로지 이론이 존재하면 형식적으로 따라 나온다. 유리성은 대각합의 [생성함수](generating-functions.md)가 유리함수라는 선형대수이고 함수방정식은 Poincaré 쌍대성이다. 절댓값은 아르키메데스 크기에 관한 진술이라 $\ell$ 진 계수의 코호몰로지에 들어 있을 이유가 없다. 대수적 구성에서 해석적 부등식을 뽑아내야 한다.

Grothendieck 의 계획은 표준 추측[^1]을 세워 [Hodge 이론](hodge-theory.md)의 양성을 [유한체](finite-fields.md)로 옮기는 것이었고, 그 추측은 아직 증명되지 않았다[^2]. Deligne 은 1974 년에 다른 길을 갔다[^3]. 곡선의 다발로 다양체를 자르고, 단행성군의 크기를 재고, $L$ 함수의 계수가 실수이고 음이 아니라는 사실을 $2k$ 제곱으로 증폭해 부등식을 조였다. 해석적 정수론의 Rankin 의 수법이 산술기하의 정리를 증명했다.
...
Deligne 은 $X$ 를 사영공간에 심고 초평면 다발로 $\mathbb P^1$ 위의 족으로 만든다. 각 올이 $X$ 의 초평면 절단이라 차원이 하나 낮고, $X$ 의 코호몰로지가 $\mathbb P^1$ 위 층의 코호몰로지가 되어 귀납이 가능하다.
...
[층](sheaves.md) $\mathcal F$ 가 **무게 $w$ 이하**라는 것은 모든 닫힌점에서 Frobenius 고윳값의 절댓값이 $q_x^{w/2}$ 이하라는 뜻이다. Deligne 이 *Weil II*(1980)에서 증명한 핵심 정리는 다음이다.

> 무게 $\le w$ 인 층의 고차 순상 $R^if_!\mathcal F$ 는 무게 $\le w+i$ 다.

무게가 사상 아래에서 어떻게 변하는지를 말하는 이 정리가 혼합 Hodge [가군](modules.md), 편향 층, 결정 대수의 대칭성으로 이어지는 무게 이론의 출발이다.
...
Kloosterman 합에서 $\alpha,\beta$ 는 두 절댓값에서 서로 다른 성질을 갖는다. $p$ 진 부치는 [Newton 다각형](newton-polygon.md)이 주고, 복소 절댓값 $\sqrt q$ 가 어려운 쪽이다.
...
- Kloosterman 합을 비롯한 지수합의 최적 상계가 이 정리에서 나온다. Katz 의 *Gauss Sums, Kloosterman Sums and Monodromy Groups* 가 이 응용을 체계화했다.
