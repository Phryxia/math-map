...
반대로 뺄셈과 덧셈, 곱셈은 살아남지만 나눗셈은 그렇지 않다. 정보를 버렸기 때문이다. `mod 6` 에서 `2 × 2 = 4` 이고 `2 × 5 = 10 ≡ 4` 이므로, `4` 를 `2` 로 나눈 답이 `2` 인지 `5` 인지 알 수 없다. 어떤 수로는 나눌 수 있고 어떤 수로는 없다는 이 비대칭이 이 이론의 대부분을 결정한다.
...
이 정의가 대표원 선택과 무관하다는 것이 아래 성질에서 증명된다. 확인되고 나면 `Z/mZ` 는 항등원을 가진 가환환이고, 몫사상 `Z → Z/mZ` 는 환 준동형이다.

## 역원과 오일러 함수

`ab ≡ 1 (mod m)` 인 `b` 가 있으면 `a` 는 법 `m` 에서 가역이고 `b` 를 역원이라 한다. 가역인 원소들의 집합을 `(Z/mZ)^×` 로 쓰며 곱셈에 대해 [군](groups.md)을 이룬다. 그 크기를 오일러 함수 `φ(m)` 이라 한다.
...
거듭제곱도 곱셈의 반복이므로 보존된다. 다만 지수 자리는 다르다. `a^e mod m` 에서 `e` 를 `mod m` 으로 줄이면 틀린다. 지수는 `φ(m)` 을 법으로 줄여야 하고 그 근거가 [Euler 정리](fermat-euler-theorem.md)다. 밑과 지수의 법이 다르다는 이 사실이 초보자가 가장 자주 틀리는 지점이다.

## 소거와 가역성

`m ≥ 2` 일 때 `a` 가 법 `m` 에서 역원을 갖는 것과 `gcd(a, m) = 1` 인 것은 동치다.
...
`gcd(c, m) = d` 일 때 `ca ≡ cb (mod m)` 에서 `c` 를 소거하려면 법도 함께 줄여야 한다.

$$
ca\equiv cb\pmod m\iff a\equiv b\pmod{m/d}
$$

`mod 6` 에서 `2·2 ≡ 2·5` 이지만 `2 ≢ 5` 였던 이유가 `d = 2` 라서 법이 `3` 으로 줄기 때문이다. 실제로 `2 ≡ 5 (mod 3)` 이다.

## 빠른 거듭제곱

지수를 이진 전개하면 곱셈 횟수가 `O(log e)` 로 줄고, 매 단계 나머지를 취하므로 중간값이 `m²` 를 넘지 않는다.
...
`m = m₁m₂` 이고 `gcd(m₁, m₂) = 1` 이면 환 동형

$$
\mathbb{Z}/m\mathbb{Z}\;\cong\;\mathbb{Z}/m_1\mathbb{Z}\times\mathbb{Z}/m_2\mathbb{Z}
$$

이 성립한다. 이것이 [중국인의 나머지 정리](chinese-remainder-theorem.md)의 구조적 형태이고, `φ` 가 곱셈적이라는 사실이 여기서 따라 나온다. 법이 소수 `p` 일 때는 `0` 이 아닌 모든 원소가 가역이므로 `Z/pZ` 는 체이며, `(Z/pZ)^×` 는 크기 `p-1` 의 순환군이다.
