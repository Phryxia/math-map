...
무한한 정수 집합을 유한 개의 부류로 줄이면서 덧셈과 곱셈을 그대로 물려받는다. [동치관계](relations.md)의 몫 구성 가운데 가장 구체적인 사례이고, 대표원을 바꿔도 답이 같은지를 확인하는 잘 정의됨 검사가 실제로 통과하는 예다. 여기서 얻은 유한 구조가 [환](rings.md)과 [유한체](finite-fields.md)의 첫 예이며 [RSA](rsa-cryptosystem.md)를 비롯한 공개키 암호의 계산 기반이다.
