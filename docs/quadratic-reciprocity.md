...
$x^2\equiv a\pmod p$ 에 해가 있는가. 이 물음은 정수론에서 가장 오래되고 기본적인 판정 문제다. 답 자체는 어렵지 않다. [Fermat 소정리](fermat-euler-theorem.md)에서 나오는 Euler 판정법이 $a^{(p-1)/2}\bmod p$ 한 번의 계산으로 결론을 준다.

이상한 것은 그 답들 사이의 관계다. $p$ 가 법 $q$ 에서 제곱수인지와 $q$ 가 법 $p$ 에서 제곱수인지는 완전히 별개의 계산인데, 두 답이 서로를 결정한다. 둘 중 하나라도 $4k+1$ 이면 두 답이 같고, 둘 다 $4k+3$ 이면 정반대다. 그게 전부다.

$$
\Big(\frac pq\Big)\Big(\frac qp\Big)=(-1)^{\frac{p-1}2\cdot\frac{q-1}2}
$$

전혀 연결될 이유가 없어 보이는 두 계산이 이렇게 단순하게 묶이는 이유를 Gauss 는 평생 여덟 가지 증명으로 파고들었다. 오늘날의 답은 두 소수가 사실 하나의 더 큰 대상, 곧 원분체의 Galois 군을 통해 서로를 보고 있다는 것이다. 이 관점이 유체론으로 자라고, 아벨이 아닌 경우로 넓히려는 시도가 Langlands 강령이 되었다. 현대 정수론에서 "상호법칙" 이라 불리는 정리들의 원형이다.

# 직관

## 왜 절반이 제곱수인가

$p$ 가 홀소수면 $(\mathbb Z/p\mathbb Z)^\times$ 는 위수 $p-1$ 인 순환군이다. 제곱 사상 $x\mapsto x^2$ 의 핵이 $\{\pm1\}$ 로 크기 2 이므로 상의 크기는 $(p-1)/2$ 다. 정확히 절반이 제곱잉여다.

제곱잉여 전체가 지표 2 인 부분군이므로, 몫군이 $\{\pm1\}$ 이고 몫사상이 준동형이다. Legendre 기호가 바로 이 준동형이며, "제곱수 곱하기 제곱수는 제곱수, 비제곱수 곱하기 비제곱수는 제곱수" 라는 규칙이 여기서 나온다. 법 $p$ 의 유일한 비자명 실수 [Dirichlet 지표](dirichlet-l-functions.md)이기도 하다.

## 상호법칙이 왜 놀라운가

Legendre 기호 $\left(\frac ap\right)$ 는 $a$ 를 법 $p$ 에서 보는 것이다. 분자와 분모의 역할이 완전히 다르다. 위쪽은 $\bmod p$ 로만 의미가 있고 아래쪽은 계산의 무대다.

그런데 $p$ 와 $q$ 를 맞바꾼 두 기호가 서로를 결정한다. 무대와 배우를 맞바꿨는데 답이 연동된다는 뜻이다. 더 이상한 것은 그 연동 규칙이 $p,q$ 를 $4$ 로 나눈 나머지, 곧 법 4 의 정보만 쓴다는 점이다.
...
두 기호가 서로를 보는 것이 아니라, 둘 다 하나의 대상인 원분체를 보고 있었다는 것이 이 증명의 요점이다.
...
Legendre 기호를 가중치로 삼은 $p$ 차 단위근의 합이다. 제곱잉여 쪽과 비제곱잉여 쪽이 얼마나 치우쳐 있는지를 재는 양이며, 아래에서 보듯 그 제곱이 정확히 $\pm p$ 다.
...
핵심은 $n$ 을 인수분해하지 않아도 된다는 점이다. Jacobi 기호가 합성수 분모에서도 상호법칙을 만족하기 때문에 가능하고, Euler 판정법을 쓰는 것보다 빠르다.
...
Carmichael 수가 Fermat 판정을 통과해 버리는 것과 달리, 이 판정을 모든 $a$ 에 대해 통과하는 합성수는 없다. 실무에서는 더 빠르고 오류율이 $1/4$ 인 Miller–Rabin 을 쓰지만, 무작위화 알고리즘이 결정적 문제를 실질적으로 해결한다는 사실을 보인 역사적 사례다.
...
Jacobi 기호가 쉽게 계산되는데 제곱잉여인지는 인수분해 없이 알기 어렵다는 비대칭이 이차 잉여 가정이며, Goldwasser–Micali 암호가 그 위에 세워졌다. 최초의 의미론적 안전성을 갖춘 확률적 공개키 암호이고, $\left(\frac an\right)=1$ 인 비제곱잉여를 잡음으로 써서 비트를 감춘다.
...
아벨이 아닌 확대로 넓히는 것이 Langlands 강령이다. Galois 표현과 자기동형 표현을 대응시키려는 이 계획에서, 원분체의 Galois 군이 $(\mathbb Z/p\mathbb Z)^\times$ 라는 사실이 출발점의 자리를 그대로 지키고 있다. 200 년 전의 관찰 하나가 아직도 분야의 중심 문제를 규정한다.
