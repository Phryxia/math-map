...
유리성은 계수열 $N_n=\char35{}X(\mathbb F_{q^n})$ 이 선형점화식을 만족한다는 뜻이고, 선형점화식은 어떤 작용소의 $n$ 제곱의 대각합이라는 형태에서 나온다. Dwork 는 점 개수를 덧셈 지표의 지수합으로 바꾸고 그 지표를 $p$ 진 해석함수의 값으로 실현해, 지수합을 무한차원 $p$ 진 Banach 공간 위 작용소의 대각합으로 만들었다. 유한차원 코호몰로지 대신 무한차원이되 완전연속인 작용소를 쓴 것이다. 완전연속 작용소의 Fredholm 행렬식이 $p$ 진 정함수이므로 $Z$ 는 정함수의 비가 되어 $p$ 진 유리형이고, 여기에 Borel–Dwork 판정을 대면 유리함수가 나온다.
