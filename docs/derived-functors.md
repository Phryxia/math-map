...
유도 함자는 완전열을 보존하지 못하는 [함자](functors.md)의 실패를 측정하는 함자의 열이다. [가군](modules.md) [범주](category.md)에서 [텐서곱](tensor-products.md) $M \otimes_R -$ 은 우완전이고 $\mathrm{Hom}\_R(M, -)$ 은 좌완전이며, 둘 다 반대쪽 완전성을 잃는다. 잃어버린 정보는 사라지지 않고 $\mathrm{Tor}$ 과 $\mathrm{Ext}$ 라는 새 가군에 남는다.
