...
지배함수를 찾기 어려워도 교환이 성립할 수 있다. 그때 쓰는 조건이 [균등적분가능성](uniform-integrability.md)이며, 확률론에서 martingale 수렴을 다룰 때 쓴다. 지배함수가 있으면 균등적분가능하다.
...
이다. 적분값의 수렴보다 강한 $L^1$ [노름](banach-spaces.md)에서의 수렴이며, $L^1$ 의 [완비성](completeness.md) 증명과 근사 논증에 쓰인다.
...
- 확률변수열의 기댓값과 극한을 교환할 때, Monte Carlo 추정량의 수렴을 정당화할 때 쓰인다. 유계 [연속함수](continuity.md)의 기댓값 수렴으로 약수렴을 정의하는 것도 유계 수렴 정리가 배경이다.
