...
$f_n \to f$ 여도 $\int f_n \to \int f$ 는 일반적으로 성립하지 않는다. [균등수렴](uniform-convergence.md)을 가정하면 성립하지만 이는 강한 조건이다. 더 나쁜 것은 적분 가능한 함수들의 점별 극한이 적분 가능하지조차 않을 수 있다는 점이다. $[0,1]$ 의 유리수를 $q_1, q_2, \ldots$ 로 나열하고 $f_n$ 을 $\lbrace q_1,\ldots,q_n\rbrace$ 의 지시함수로 두면 각 $f_n$ 은 적분 가능하지만 극한은 Dirichlet 함수다.
