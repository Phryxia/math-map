...
정의는 어떻게 쪼개고 어디서 표본을 뽑든 같은 값에 수렴할 것을 요구한다. 이 요구가 적분 가능성의 내용이고, 그것이 깨지는 곳에서 [Lebesgue 적분](lebesgue-integral.md)이 필요해진다. [연속함수](continuity.md)의 [균등연속성](uniform-continuity.md)이 적분 가능성의 충분조건을 준다.
