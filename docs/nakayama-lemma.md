...
$R\subseteq S$ 가 환의 확대이고 $S$ 가 $R$ 가군으로 유한생성이면, 위 행렬식 꼴이 $S$ 의 원소가 $R$ 계수 일계수 다항식을 만족함을 준다. 정수적 확대의 기본 성질이 이 논증에서 나오며, [Dedekind 정역](dedekind-domains.md)의 정수적 폐포 조건이 그 무대다.
