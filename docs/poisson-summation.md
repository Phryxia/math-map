...
감쇠 조건에서 $\sum_nf(x+n)$ 이 [균등수렴](uniform-convergence.md)하므로 $F$ 는 연속이고, $\sum_k|\hat f(k)|\lt\infty$ 이므로 $F$ 의 Fourier 급수가 절대수렴해 $F$ 에 균등수렴한다. 두 사실을 $x=0$ 에서 맞추면 증명이 끝난다. $\square$
...
로 쓰고 적분을 $t=1$ 에서 자른 뒤 앞쪽 조각에 $t\mapsto1/t$ 와 $\theta(1/t)=\sqrt t\thinspace\theta(t)$ 를 적용하면 $s\mapsto1-s$ 에 대해 대칭인 표현이 남는다. 이것이 Riemann 의 두 번째 증명이고 대칭의 근원은 Poisson 합이다. [Riemann zeta 함수](riemann-zeta.md)의 [해석적 연속](analytic-continuation.md)과 [소수 정리](prime-number-theorem.md)의 증명이 이 함수방정식을 쓴다.
