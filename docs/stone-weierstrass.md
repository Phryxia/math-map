...
Stone–Weierstrass 정리는 [콤팩트](compactness.md) 공간 위의 [연속함수](continuity.md)를 어떤 함수족으로 균등근사할 수 있는지 정한다. 점을 분리하고 상수함수를 담은 부분대수는 연속함수 전체에서 균등수렴 위상으로 조밀하다.
...
가 $f$ 로 균등수렴한다. 이 식은 성공확률 $x$ 인 시행 $n$ 회의 성공 비율에 $f$ 를 적용한 값의 기댓값이고, [큰 수의 법칙](law-of-large-numbers.md)이 수렴을 준다.
