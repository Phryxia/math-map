...
Stone–Weierstrass 정리는 콤팩트 공간 위의 연속함수를 어떤 함수족으로 균등근사할 수 있는지 정한다. 점을 분리하고 상수함수를 담은 부분대수는 연속함수 전체에서 균등수렴 위상으로 조밀하다.
...
- **구면조화함수의 완전성.** 구면 위의 다항식 제한이 부분대수이고 점을 분리하므로 $C(S^2)$ 에서 조밀하고, $L^2$ 에서도 조밀하다. [구면조화함수](spherical-harmonics.md)가 기저를 이룬다는 진술이 여기서 나온다.
