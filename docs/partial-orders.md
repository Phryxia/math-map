...
비교 불가능한 쌍의 상대 순서는 자유롭게 정할 수 있으므로 선형 확장은 대개 여럿이다. 선형 확장의 개수는 poset 이 얼마나 전순서에서 먼지를 재는 척도이고, 이 값을 세는 문제는 $\#\mathrm P$ 완전이다. 위상정렬 결과 하나를 유일한 정답처럼 다루면 안 되는 이유가 여기 있다.
