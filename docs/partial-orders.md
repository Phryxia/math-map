...
[동치관계](equivalence-relations.md)의 세 공리 가운데 대칭성을 반대칭성으로 바꾸면 부분순서가 된다. 동치관계는 차이를 지우고 부분순서는 차이에 방향을 준다.
...
모든 두 원소가 상한과 하한을 가지는 poset 이 [격자](order-lattices.md)다. 상한을 $x \vee y$ , 하한을 $x \wedge y$ 로 쓴다. 멱집합은 합집합과 교집합으로 격자이고, 양의 정수의 나눔 관계는 최소공배수와 최대공약수로 격자다. 격자에 분배법칙과 보원을 더하면 [Boolean algebra](boolean-algebras.md)이고, 보원 대신 더 약한 조건을 두면 [Heyting algebra](heyting-algebras.md)가 된다.
