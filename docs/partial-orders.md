...
$\lbrace 1\rbrace$ 과 $\lbrace 2\rbrace$ 는 어느 쪽도 다른 쪽을 포함하지 않는다. 이것을 "아직 정하지 못한 순서" 로 보고 억지로 하나의 점수를 매기면, 원래 없던 정보를 지어내는 셈이 된다. 부분순서는 이 미결정 상태를 구조의 일부로 인정한다.
...
세 원소 집합의 멱집합이다. 아래에서 위로 포함이고, $\lbrace a,b\rbrace$ 와 $\lbrace a,c\rbrace$ 사이에는 선이 없다. Hasse 도형에서 선이 없다는 것과 관계가 없다는 것은 다르다. $\lbrace\rbrace$ 에서 $\lbrace a,b\rbrace$ 로 가는 직접 선은 없지만 두 원소는 관계되어 있다.
...
$P$ 와 $\le$ 를 함께 지정한 대상을 부분순서 집합(poset)이라 한다. $x \le y$ 이고 $x \ne y$ 이면 $x < y$ 로 쓴다. 두 원소가 $x \le y$ 도 $y \le x$ 도 아니면 비교 불가능하다고 하고, 모든 쌍이 비교 가능하면 전순서다.
...
최소·극소·하계·하한은 방향을 뒤집어 같게 정의한다. $\lbrace 2, 3, 6\rbrace$ 을 나눔 관계로 보면 $2$ 와 $3$ 이 극소원소이고 최소원소는 없으며 $6$ 은 최대원소다. 최대와 극대를 혼동하면 "극대원소가 유일하니 최대원소" 라는 잘못된 추론을 하게 되는데, 유한 poset 에서는 참이지만 무한에서는 거짓이다.

## 쌍대성

$x \le^* y$ 를 $y \le x$ 로 정의하면 $\le^*$ 도 부분순서다. 이를 쌍대 순서라 하고 $P^{\mathrm{op}}$ 로 쓴다. 부분순서에 관한 모든 정리는 부등호 방향을 전부 뒤집은 정리를 쌍으로 가진다. 최대와 최소, 상한과 하한, 극대와 극소가 그렇게 짝지어진다. 증명을 한 번만 하면 되는 이 절약은 [범주](category.md)의 쌍대성으로 그대로 이어진다.

## 격자

모든 두 원소가 상한과 하한을 가지면 격자다. 상한을 $x \vee y$ , 하한을 $x \wedge y$ 로 쓴다. 멱집합은 합집합과 교집합으로 격자이고, 양의 정수의 나눔 관계는 최소공배수와 최대공약수로 격자다. 격자에 분배법칙과 보원을 더하면 [Boolean algebra](boolean-algebras.md)이고, 보원 대신 더 약한 조건을 두면 [Heyting algebra](heyting-algebras.md)가 된다.
...
비교 불가능한 쌍의 상대 순서는 자유롭게 정할 수 있으므로 선형 확장은 대개 여럿이다. 선형 확장의 개수는 poset 이 얼마나 전순서에서 먼지를 재는 척도이고, 이 값을 세는 문제는 $\char"23{}\mathrm P$ 완전이다. 위상정렬 결과 하나를 유일한 정답처럼 다루면 안 되는 이유가 여기 있다.
...
$P$ 의 원소를 대상으로 삼고 $x \le y$ 일 때 $x$ 에서 $y$ 로 가는 사상을 정확히 하나 두면 [범주](category.md)가 된다. 반사성이 항등사상, 추이성이 합성이고, 반대칭성은 동형인 두 대상이 같다는 조건에 해당한다. 이 관점에서 상한은 쌍대곱, 하한은 곱, 최대원소는 종대상이다. 사상이 많아야 두 대상 사이에 하나뿐이므로 poset 은 범주론의 명제를 부등식으로 확인해 보는 가장 작은 시험장이 된다.
