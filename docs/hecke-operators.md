...
[모듈러 형식](modular-forms.md)의 $q$ 전개로 적은 $(T_pf)\_n=a_{np}+p^{k-1}a_{n/p}$ 가 Hecke 작용소다. 원래 정의는 [격자](lattices.md)에 있다. 모듈러 형식을 격자의 함수로 보면 $T_p$ 는 지표 $p$ 의 부분격자 전체에 걸친 합이고, 계수의 곱셈성은 부분격자를 세는 일이 소수마다 독립이라는 데서 따라온다.
