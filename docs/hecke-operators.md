...
[모듈러 형식](modular-forms.md) 문서에서 $q$ 전개로 적은 $(T_pf)\_n=a_{np}+p^{k-1}a_{n/p}$ 가 Hecke 작용소다. 원래 정의는 격자에 있다. 모듈러 형식을 격자의 함수로 보면 $T_p$ 는 지표 $p$ 의 부분격자 전체에 걸친 합이고, 계수의 곱셈성은 부분격자를 세는 일이 소수마다 독립이라는 데서 따라온다.
...
$\gcd(m,n)=1$ 일 때 지표 $mn$ 의 부분격자는 중국인의 나머지 정리에 의해 지표 $m$ 인 것과 지표 $n$ 인 것으로 유일하게 쪼개지므로 $T_mT_n=T_{mn}$ 이다. $\gcd$ 이 1 이 아니면 겹침이 생겨 관계식이 하나 더 붙는다.
...
Ramanujan–Petersson 추측이라 불리던 이 부등식을 Deligne 이 Weil 추측에서 끌어냈다. $a_p$ 는 어떤 $\ell$ 진 Galois 표현의 Frobenius 자취이고, 그 표현의 고윳값이 절댓값 $p^{(k-1)/2}$ 를 갖는다는 것이 Weil 추측의 Riemann 가설 부분이다.
