...
Laplace 작용소는 $\Delta_d$ , $\Delta_\partial$ , $\Delta_{\bar\partial}$ 세 종류인데 $d\omega=0$ 이 이들을 하나로 만든다. 그러면 조화형식 공간이 $(p,q)$ 성분으로 쪼개지고 Betti 수가 복소구조를 반영하는 세밀한 수들로 분해된다. $b_1$ 이 짝수라는 제약과 복소 다양체가 사영공간에 들어가는 조건이 여기서 나온다.
