...
$\gcd(p,a)=1$ 인 소수 $p$ 가 $ab$ 를 나누면 $sp+ta=1$ 의 양변에 $b$ 를 곱해 $p\mid b$ 를 얻는다. 이것이 [소수](primes.md)의 유일분해를 증명하는 핵심 단계다. 유일분해라는 정수론의 기초 정리가 나눗셈 알고리즘에서 나온다는 사실은, 나머지 있는 나눗셈이 되는 환(유클리드 정역)이 왜 특별한지를 설명한다.
