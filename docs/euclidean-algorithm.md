...
- **RSA**(Rivest–Shamir–Adleman)의 개인키 지수는 $ed \equiv 1 \pmod{\varphi(n)}$ 의 해이므로 확장 유클리드 알고리즘 한 번으로 얻는다. 타원곡선 암호의 점 연산, [중국인의 나머지 정리](chinese-remainder-theorem.md)를 이용한 복호 가속, 유한체 산술의 나눗셈도 이 절차에 의존한다.
