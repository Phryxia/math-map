...
lambda 항은 "이름 붙이지 않은 함수"를 적는 표기법이다. `x` 를 받아 `x + 1` 을 주는 함수를 `λx. x + 1` 로 쓰고, 인자에 적용하는 것은 나란히 쓰는 것으로 표현한다. 계산이란 `(λx. M) N` 이라는 모양을 발견해 `M` 안의 `x` 를 `N` 으로 치환하는 일의 반복이다. 이 한 가지 규칙(β 축약)이 전부다.
...
재귀는 더 미묘하다. 이름이 없으니 정의 안에서 자기를 부를 수 없다. 해결책은 "자기 자신을 인자로 받는" 항을 만들어 고정점을 취하는 것이다. Y combinator가 정확히 그 일을 한다. `Y g` 가 `g (Y g)` 로 축약되므로 `g` 는 자기 자신의 결과를 인자로 받아 쓸 수 있고, 이렇게 이름 없이 재귀가 생긴다. 여기에는 대가가 있다. 같은 장치가 축약이 영원히 끝나지 않는 항도 만들어 내며, 실제로 `(λx. x x)(λx. x x)` 는 자기 자신으로 축약되기를 무한히 반복한다.
...
`x` 는 변수, `λx. M` 은 추상, `M N` 은 적용이다. 관례적으로 적용은 왼쪽 결합(`M N P` 는 `(M N) P`)이고 추상의 몸통은 최대한 오른쪽으로 뻗는다(`λx. M N` 은 `λx. (M N)`).

자유변수 집합은 `FV(x) = {x}`, `FV(λx. M) = FV(M) \ {x}`, `FV(M N) = FV(M) ∪ FV(N)` 로 정의한다. 자유변수가 없는 항을 닫힌 항 또는 combinator라 한다.

## 세 가지 변환

**α-변환.** 속박변수의 이름은 의미를 갖지 않는다. `λx. M` 과 `λy. M[x := y]` (`y` 가 `M` 에서 자유롭지 않을 때)를 같은 항으로 본다. 이후 모든 항은 α-동치류로 다룬다.
...
여기서 치환은 포획 회피(capture-avoiding)여야 한다. 즉 `N` 의 자유변수가 `M` 안의 추상에 붙잡히지 않도록 필요하면 α-변환을 먼저 한다. `(λx. M) N` 꼴의 부분항을 redex라 하고, redex가 없는 항을 정규형(normal form)이라 한다.

**η-변환.** 외연성(extensionality)을 표현한다. `x` 가 `M` 에서 자유롭지 않을 때
...
`→*` 로 여러 단계 축약을, `=_β` 로 β-축약으로 생성되는 동치관계를 쓴다.
...
즉 `0 = λf.λx. x`, `1 = λf.λx. f x`, `2 = λf.λx. f (f x)` 다. 산술 연산은 다음과 같다.

- 후자: `succ = λn.λf.λx. f (n f x)`
- 덧셈: `plus = λm.λn.λf.λx. m f (n f x)`
- 곱셈: `mult = λm.λn.λf. m (n f)`
- 거듭제곱: `exp = λm.λn. n m`

진리값과 조건은 선택자로 정의한다. `true = λx.λy. x`, `false = λx.λy. y`, `ifelse = λb.λt.λe. b t e`. 그러면 `iszero = λn. n (λz. false) true` 가 된다. 순서쌍은 `pair = λa.λb.λs. s a b`, `fst = λp. p true`, `snd = λp. p false` 다. 전자(predecessor)는 Kleene의 기법으로 `(n, n-1)` 쌍을 반복 갱신해 얻는다.

## 고정점 combinator

**정의.** `Y = λf. (λx. f (x x)) (λx. f (x x))`.
...
이고 마지막 항은 `g (Y g)` 와 같은 항이다. 따라서 `Y g =_β g (Y g)` 이며, `Y g` 는 `g` 의 고정점이다.
...
에 대해 `fact = Y F` 다. 값 호출(call-by-value) 언어에서는 `Y` 가 발산하므로 한 단계 지연을 넣은 `Z = λf. (λx. f (λv. x x v)) (λx. f (λv. x x v))` 를 쓴다.
...
두 전략은 결과가 같지만 종료성이 다르다. 예컨대 `(λx. λy. y) Ω` (여기서 `Ω = (λx. x x)(λx. x x)`)는 정규 순서에서는 `λy. y` 로 끝나지만 값 호출에서는 `Ω` 를 먼저 줄이려다 발산한다.

# 성질

## Church–Rosser 정리

**정리 (합류성, confluence).** `M →* N_1` 이고 `M →* N_2` 이면, 어떤 `P` 가 존재하여 `N_1 →* P` 이고 `N_2 →* P` 다. βη 축약에 대해서도 성립한다.
...
1. **정규형의 유일성.** 항이 정규형을 가지면 그것은 α-동치를 제외하고 유일하다. 두 정규형이 공통 축약형을 가져야 하는데, 정규형은 더 줄지 않으므로 서로 같아야 한다.
2. **무모순성.** `true` 와 `false` 는 서로 다른 정규형이므로 `=_β` 로 동일시되지 않는다. 즉 모든 항을 같다고 증명하는 붕괴가 일어나지 않는다.
3. **표준화 정리.** 항이 정규형을 가지면 정규 순서 축약이 반드시 그 정규형에 도달한다. 정규 순서는 "종료성 면에서 최선"이다.

## 계산 능력과 결정불가능성

**정리 (Church, Kleene, Turing).** 함수 `f : N^k -> N` 가 lambda-정의가능한 것과 Turing 계산가능한 것은 동치다. 즉 Church 수로 입출력을 부호화할 때 `F ⌜n⌝ =_β ⌜f(n)⌝` 인 항 `F` 가 존재하는 것과 `f` 를 계산하는 Turing 기계가 있는 것이 같다[^1].
...
- 두 항이 `=_β` 인지도 결정불가능하다(Church의 정리). Scott–Curry 정리는 더 강하게, β-동치로 닫힌 자명하지 않은 항 집합은 결정불가능하다고 말한다. 이는 [Rice 정리](rice-theorem.md)의 lambda calculus 판본이다.
- 반면 [유한 오토마타](finite-automata.md)처럼 계산 능력이 제한된 모델에서는 이런 질문들이 모두 결정가능하다. 표현력과 분석가능성의 교환은 여기서도 같다.

## 단순 타입과 정규화

**단순 타입 lambda calculus(STLC).** 타입을 기본 타입과 함수 타입 `A -> B` 로 정의하고, 항에 타입 판정 규칙을 준다. 변수는 문맥에서 타입을 받고, `λx : A. M` 은 `M : B` 일 때 `A -> B` 를, 적용 `M N` 은 `M : A -> B` 이고 `N : A` 일 때 `B` 를 갖는다.
...
강한 정규화의 대가는 표현력이다. `λx. x x` 는 타입을 붙일 수 없고(자기 적용은 `A = A -> B` 를 요구한다), 따라서 `Y` 도 타입을 갖지 못한다. STLC에서 표현 가능한 수치 함수는 확장 다항식 수준에 그치므로 Turing 완전하지 않다. 실제 언어는 재귀를 원시 연산으로 추가하거나(`fix`), 다형성·의존 타입 같은 더 강한 체계를 쓴다.

**Curry–Howard 대응.** 타입을 명제로, 항을 증명으로 읽으면 STLC는 [직관주의 논리](intuitionistic-logic.md)의 함의 단편과 정확히 대응한다. 함수 타입은 함의, 곱 타입은 논리곱, 합 타입은 논리합이며, β-축약은 증명의 정규화(cut 제거)에 해당한다. 이 대응이 형식 증명 보조 도구(Coq, Agda, Lean)의 설계 원리다[^2].
...
- **combinator 논리.** 변수 속박을 없애고 `S`, `K` 두 combinator만으로 같은 계산 능력을 얻는다(`S = λx.λy.λz. x z (y z)`, `K = λx.λy. x`). 변수 이름 처리를 피하려는 구현에서 쓰인다.
