...
lambda 항은 "이름 붙이지 않은 함수"를 적는 표기법이다. $x$ 를 받아 $x + 1$ 을 주는 함수를 $\lambda x.\, x + 1$ 로 쓰고, 인자에 적용하는 것은 나란히 쓰는 것으로 표현한다. 계산이란 $(\lambda x.\, M)\,N$ 이라는 모양을 발견해 $M$ 안의 $x$ 를 $N$ 으로 치환하는 일의 반복이다. 이 한 가지 규칙(β 축약)이 전부다.
...
**α-변환.** 속박변수의 이름은 의미를 갖지 않는다. $y$ 가 $M$ 에서 자유롭지 않을 때 $\lambda x.\, M$ 과 $\lambda y.\, M[x := y]$ 를 같은 항으로 본다. 이후 모든 항은 α-동치류로 다룬다.
...
여기서 치환은 포획 회피(capture-avoiding)여야 한다. 즉 $N$ 의 자유변수가 $M$ 안의 추상에 붙잡히지 않도록 필요하면 α-변환을 먼저 한다. $(\lambda x.\, M)\,N$ 꼴의 부분항을 redex라 하고, redex가 없는 항을 정규형(normal form)이라 한다.
...
$\to^*$ 로 여러 단계 축약을, $=_\beta$ 로 β-축약으로 생성되는 동치관계를 쓴다.
...
**정리 (합류성, confluence).** $M \to^* N_1$ 이고 $M \to^* N_2$ 이면, 어떤 $P$ 가 존재하여 $N_1 \to^* P$ 이고 $N_2 \to^* P$ 다. βη 축약에 대해서도 성립한다.
...
- 두 항이 $=_\beta$ 인지도 결정불가능하다(Church의 정리). Scott–Curry 정리는 더 강하게, β-동치로 닫힌 자명하지 않은 항 집합은 결정불가능하다고 말한다. 이는 [Rice 정리](rice-theorem.md)의 lambda calculus 판본이다.
