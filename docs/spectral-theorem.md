...
대칭이 왜 직교성을 낳는지는 짧게 확인된다. $Av=\lambda v$ , $Aw=\mu w$ 이고 $\lambda\neq\mu$ 이면 대칭성에서 $\lambda\langle v,w\rangle=\langle Av,w\rangle=\langle v,Aw\rangle=\mu\langle v,w\rangle$ 이므로 $\langle v,w\rangle=0$ 이다. 즉 서로 다른 고윳값의 고유공간은 자동으로 직교한다.

# 정의

$V$ 를 실수체 또는 복소수체 위의 유한차원 내적 공간, $T$ 를 $V$ 의 선형연산자라 하자. $T$ 의 수반연산자 $T^{*}$ 는 다음 항등식으로 유일하게 정해진다.
...
$Q$ 의 열이 정규직교기저이면 $Q$ 는 직교행렬(복소수에서는 unitary)이며 역행렬이 전치다.

$$
Q^{\mathsf T}Q=I
$$

정리의 진술은 다음과 같다. $A$ 가 실대칭이면 직교행렬 $Q$ 와 실대각행렬 $\Lambda$ 가 존재하여 다음이 성립하고, 역도 성립한다[^2].
...
자기수반 $T$ 의 고윳값은 실수다. $Tv=\lambda v$ 에 $v$ 를 내적하면 다음이 되고, 좌변은 자기수반성 때문에 자기 자신의 켤레와 같아 실수이며 노름 제곱은 양수이므로 $\lambda$ 가 실수다[^1].
...
복소 내적 공간에서 귀납법으로 증명한다. 차원이 1 이상이면 특성다항식이 대수학의 기본정리로 근을 가지므로 고윳값 $\lambda$ 과 단위 고유벡터 $v$ 가 있다. $v$ 의 직교여공간은 $T$ -불변이다: $w$ 가 $v$ 와 직교하면 $\langle Tw,v\rangle=\langle w,Tv\rangle=\lambda\langle w,v\rangle=0$ 이므로 $Tw$ 도 $v$ 와 직교한다. 직교여공간은 차원이 1 작고 그 위로 제한한 연산자도 자기수반이므로 귀납가정을 쓸 수 있다. 이렇게 얻은 고유벡터들을 모으면 정규직교 고유기저다.

실대칭행렬은 복소 Hermitian으로 보면 고윳값이 실수이고, 그러면 $\lambda I-A$ 가 실행렬이므로 kernel에서 실고유벡터를 고를 수 있다. 같은 고윳값의 고유공간 안에서는 Gram–Schmidt로 정규직교기저를 잡고, 서로 다른 고윳값 사이는 위 직교성 논증이 처리한다. 결과적으로 기하적 중복도가 항상 대수적 중복도와 같다.

## 스펙트럼 분해와 함수 계산

서로 다른 고윳값을 $\mu_1,\dots,\mu_k$ , $P_i$ 를 그 고유공간으로의 정사영이라 하면 다음이 성립한다.
...
실대칭 $A$ 가 $0$ 이 아닌 모든 $x$ 에서 이차형식이 양수이면 양의 정부호라 한다. 스펙트럼 정리에 의해 이는 모든 고윳값이 양수인 것과 동치다. 행렬식만으로 하는 판정으로 Sylvester 기준이 있다: 선행 주요 소행렬식(왼쪽 위 $k\times k$ 부분행렬의 [행렬식](determinants.md))이 모두 양수인 것과 동치다[^3].
