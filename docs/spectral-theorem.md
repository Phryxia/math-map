...
실대칭행렬은 복소 Hermitian 으로 보면 고윳값이 실수이고, 그러면 $\lambda I-A$ 가 실행렬이므로 핵에서 실고유벡터를 고른다. 같은 고윳값의 고유공간 안에서는 Gram–Schmidt 로 정규직교기저를 잡고 서로 다른 고윳값 사이는 위 직교성이 처리한다. 기하적 중복도가 항상 대수적 중복도와 같다.
