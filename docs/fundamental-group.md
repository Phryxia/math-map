...
$X$ 를 위상공간, $x_0$ 를 $X$ 의 한 점(기준점)이라 하자. $X$ 의 $x_0$ 에 기반한 **고리**(loop)는 단위구간에서 $X$ 로 가는 연속함수로 양 끝이 모두 $x_0$ 인 것이다. 두 고리 $f,g$ 가 **경로 호모토피**(path-homotopic)라는 것은 양 끝을 고정한 연속 변형이 있다는 뜻이다.
