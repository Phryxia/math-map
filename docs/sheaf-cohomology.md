...
$H^i(X,\mathcal A) = 0$ 이 모든 $i \ge 1$ 에서 성립하는 층을 **비순환층**이라 한다. 연약층과 연속함수 환 위의 세밀층이 비순환층이다.
...
- 지수열 $0 \to \mathbb Z \to \mathcal O \to \mathcal O^{\ast} \to 0$ 의 긴 완전열이 선다발을 분류한다. $H^1(X,\mathcal O^{\ast})$ 가 Picard 군이고 첫 Chern 류가 $H^2(X,\mathbb Z)$ 로 가는 연결 사상이다.
- 연접층의 Euler 지표 $\sum_i (-1)^i \dim H^i(X,\mathcal F)$ 가 Riemann–Roch 공식의 왼쪽 항이다. 고차 항의 소멸을 보이면 전역 단면의 차원이 계산된다.
