...
Deligne 은 무게 $k\ge2$ 의 고유형식마다 2 차원 $\ell$ 진 Galois 표현이 있어 $\mathrm{tr}\thinspace\rho_f(\mathrm{Frob}_p)=a_p$ 임을 증명했다. 모듈러 곡선의 코호몰로지에서 잘라내는 방식이다. 이것이 자기동형에서 Galois 로 가는 방향이고, 어려운 것은 반대 방향이다.

## 모듈러성 정리와 Fermat

> $\mathbb Q$ 위의 모든 타원곡선 $E$ 는 모듈러다. 곧 무게 2, 레벨 $N=\mathrm{cond}(E)$ 의 고유형식 $f$ 가 있어 모든 $p\nmid N$ 에서 $a_p(E)=a_p(f)$ 다.

$a_p(E)=p+1-\char"23{}E(\mathbb F_p)$ 이므로 좌변은 점 개수를 세는 순수한 유한체 계산이고, 우변은 복소해석적 대상의 Fourier 계수다. 두 수열이 같다는 주장이다.
