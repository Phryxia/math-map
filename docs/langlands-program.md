...
모든 자기동형 표현은 국소 표현의 제한 [텐서곱](tensor-products.md) $\pi=\bigotimes'\_v\pi_v$ 로 분해되고, 거의 모든 $v$ 에서 $\pi_v$ 가 불분기라 $n$ 개의 복소수 Satake 매개변수 $\alpha_{1,v},\dots,\alpha_{n,v}$ 로 결정된다. $L$ 함수는 이들로 만든다.
...
함수체의 Galois 군은 곡선의 [기본군](fundamental-group.md)이라 기하학적 대상이고 모듈라이 공간 위에서 논증을 펼 수 있어 먼저 풀렸다. 이 관점을 복소 곡선으로 옮긴 것이 기하적 Langlands 강령이고 물리의 게이지 이론과도 연결된다.
