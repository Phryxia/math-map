...
`n=1` 이 유체론이다. `n=2` 이고 `K=\mathbb Q` 인 경우의 한 조각이 모듈러성 정리이며, 유리수체 위의 모든 [타원곡선](elliptic-curves.md)이 모듈러 형식에서 온다는 그 정리가 Fermat 마지막 정리의 증명을 완성했다. 강령 전체는 여전히 열려 있고, 정수론에서 가장 큰 조직 원리로 받아들여진다.
...
두 세계가 주는 데이터의 모양이 같다. Galois 쪽은 소수마다 `n` 개의 근(Frobenius 고윳값)을 주고, 자기동형 쪽은 소수마다 `n` 개의 Satake 매개변수를 준다. 유체론이 `n=1` 에서 이 둘이 실제로 같음을 이미 증명했다.
...
모든 자기동형 표현은 국소 표현의 제한 텐서곱 `\pi=\bigotimes'_v\pi_v` 로 분해되고, 거의 모든 `v` 에서 `\pi_v` 가 불분기라 `n` 개의 복소수 **Satake 매개변수** `\alpha_{1,v},\dots,\alpha_{n,v}` 로 결정된다. `L` 함수는 이들로 만든다.
