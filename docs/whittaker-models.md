...
$\mathrm{GL}\_n$ 의 자기동형 $L$ 함수 이론은 이 유일성 위에 서 있고, 유일성이 없는 군에서는 같은 이론을 세우기 어렵다.
...
이 지표는 이웃한 단순근 자리의 성분만 본다. 모든 단순근 성분이 살아 있으므로 **비퇴화**다.
...
$p$ 진체 대신 [유한체](finite-fields.md)를 놓으면 같은 현상이 유한군의 지표 계산이 된다. $G=\mathrm{GL}\_n(\mathbb F_q)$ 라 하고, $N$ 을 상삼각 멱단군, $\psi_N$ 을 위와 같은 지표라 할 때 유도표현
...
Whittaker 함수가 첨점형식을 복원하므로 두 첨점형식의 Whittaker 함수가 같으면 형식이 같다. 여기에 유일성을 더하면 다음 정리가 따라 나온다.
