...
한 점을 품는 집합 전체는 초필터이고 이를 주 초필터라 한다. 유한 집합의 여집합 전체가 만드는 필터를 확장하면 주가 아닌 초필터를 얻고, 그 확장에 [선택공리](axiom-of-choice.md)를 쓴다.
...
의미론적 개념인 기본 동치가 대수적 개념인 동형으로 바뀐다. Keisler 가 일반화 [연속체 가설](continuum-hypothesis.md) 아래 증명했고 Shelah 가 가정 없이 증명했다.
