...
Boolean algebra 가 고전 명제논리의 대수적 뼈대인 것처럼, Heyting algebra 는 [직관주의 논리](intuitionistic-logic.md)의 대수적 뼈대다. 차이는 단 하나, 배중률에 해당하는 항등식이 성립하지 않아도 된다는 점이다. 그럼에도 분배법칙, de Morgan 법칙의 절반, 이중부정 도입 같은 성질은 그대로 남는다.
...
특징짓는 방법은 "가장 큰 것"으로 정의하는 것이다. `a → b` 는 "`a` 와 만나도 `b` 를 넘지 않는 원소들 중 최대 원소"다. 즉 `a` 로부터 `b` 를 끌어내기에 충분한 정보를 가장 느슨하게 모은 것이다. 이를 상대 의사보수(relative pseudo-complement)라 한다.

부정은 여기서 파생된다. `¬a` 는 `a → 0`, 즉 `a` 와 만나면 바닥이 되는 최대 원소다. 이것이 "여집합"과 다를 수 있다는 데 모든 차이가 있다.

실직선의 열린집합만 생각해 보자. `A` 를 0을 제외한 실직선이라 하면 여집합은 한 점이고, 그 안에 들어가는 가장 큰 열린집합은 공집합이다. 따라서 `¬A` 는 공집합이고 `A ∨ ¬A` 는 전체가 아니다. 배중률이 깨지는 이유는 신비롭지 않다. 여집합을 취하면 열린집합 밖으로 나가므로 안쪽으로 잘라 들여야 하고, 그 과정에서 경계가 버려진다.
...
`(H, ∧, ∨, 0, 1)` 이 최소원 `0` 과 최대원 `1` 을 가지는 격자라 하자. 원소 `a`, `b` 에 대해 다음 조건을 만족하는 원소 `c` 가 있으면, `c` 를 `b` 에 대한 `a` 의 상대 의사보수라 하고 `a → b` 로 쓴다.

$$
\forall x \in H:\quad x \wedge a \le b \iff x \le c
$$

이 조건은 `c` 를 유일하게 결정한다. 두 원소가 같은 조건을 만족하면 서로를 넘지 않기 때문이다. 동치로, `a → b` 는 집합
...
**정의.** Heyting algebra 는 유계 격자 `H` 이며 모든 `a`, `b` 에 대해 상대 의사보수 `a → b` 가 존재하는 것이다. 부정은 다음으로 정의한다.

$$
\neg a \;:=\; a \to 0
$$

수반 조건은 [부분순서](partial-orders.md)의 언어로 보면 Galois 연결이다. 각 `a` 마다 단조사상 `(-) ∧ a` 가 좌수반이고 `a → (-)` 가 우수반이다. [범주](category.md)의 관점에서는 `H` 를 얇은 범주로 보았을 때 `(-) ∧ a` 가 좌수반 [functor](functors.md) 라는 진술과 같다.
...
성립하면 `a → b` 를 위의 집합의 상한으로 정의하면 된다.

## 표준 예

**열린집합 격자.** [위상공간](topology.md) `X` 의 열린집합 전체 `O(X)` 는 포함관계로 complete Heyting algebra 다. 연산은 다음과 같다. 여기서 `int` 는 내부, `\` 는 차집합이다.

$$
U \to V = \operatorname{int}\big((X \setminus U) \cup V\big), \qquad \neg U = \operatorname{int}(X \setminus U)
$$

**상향닫힌 집합 격자.** 부분순서 집합 `(W, ≤)` 에 대해 상향닫힌 집합 전체 `Up(W)` 는 교집합과 합집합으로 complete Heyting algebra 다. 함의는 다음과 같다.
...
**Lindenbaum–Tarski 대수.** 직관주의 명제논리의 논리식들을 "상호 도출 가능"으로 나눈 몫은 Heyting algebra 이며, 이때 `[φ] ≤ [ψ]` 는 `φ ⊢ ψ` 를 뜻한다. 이 구성은 [동치관계](relations.md)로 나누는 전형적인 방식이다.
...
*증명 스케치.* `(-) ∧ a` 가 좌수반이므로 존재하는 모든 상한을 보존한다. 특히 이항 상한을 보존하므로 `a ∧ (b ∨ c) = (a ∧ b) ∨ (a ∧ c)` 다. 수반 하나에서 분배법칙이 따라 나온다는 점이 Heyting 구조가 강한 이유다.
...
반면 `¬¬a ≤ a` 와 `¬(a ∧ b) = ¬a ∨ ¬b` 와 `a ∨ ¬a = 1` 은 일반적으로 성립하지 않는다. 세 번째 것이 배중률이다.

## Boolean 이 되는 조건

**정리.** Heyting algebra `H` 에 대해 다음은 동치다.

1. 모든 `a` 에 대해 `a ∨ ¬a = 1` 이다.
2. 모든 `a` 에 대해 `¬¬a = a` 다.
3. `H` 는 [Boolean algebra](boolean-algebras.md) 이며 `a → b = ¬a ∨ b` 다.

*증명 스케치.* 1에서 2로는 `a ∨ ¬a = 1` 의 양변에 `¬¬a` 를 만나게 하고 `¬a ∧ ¬¬a = 0` 을 쓴다. 2에서 3으로는 `¬` 가 대합(involution)이 되어 de Morgan 법칙이 양방향 모두 회복되고, 의사보수가 보수가 된다. 3에서 1은 자명하다.

## 정칙 원소와 Glivenko 정리

`¬¬a = a` 인 원소를 정칙(regular)이라 한다. 정칙 원소 전체는 `H` 의 부분격자는 아니지만, 만남과 `¬` 를 그대로 쓰고 이음을 `a ⊔ b := ¬(¬a ∧ ¬b)` 로 정의하면 Boolean algebra 가 된다. 사상 `a ↦ ¬¬a` 는 Heyting algebra 에서 이 Boolean algebra 로 가는 전사 준동형이다.

논리 쪽 대응물이 Glivenko 정리다. 명제논리식 `φ` 에 대해 고전논리에서 `φ` 가 증명 가능한 것과 직관주의논리에서 `¬¬φ` 가 증명 가능한 것은 동치다. 고전논리는 직관주의논리 안에 이중부정을 통해 충실하게 들어앉는다.
...
*증명 스케치.* 건전성은 공리마다 수반 조건으로 확인하면 된다. 완전성은 Lindenbaum–Tarski 대수를 반례 모형으로 쓴다. `φ` 가 증명 불가능하면 몫 대수에서 `[φ] ≠ 1` 이므로 그 대수 자체가 반례다.
...
두 세계 `w0 < w1` 로 된 프레임에서 상향닫힌 집합 격자를 만들고 함의와 부정을 계산한다. 배중률이 깨지는 최소 예다.
...
원소는 공집합, `{w1}`, `{w0,w1}` 세 개다. `A = {w1}` 에서 `¬A` 는 공집합이므로 `A ∨ ¬A` 는 전체가 아니고, `¬¬A` 는 전체이므로 이중부정 제거도 실패한다. 세 원소 사슬은 배중률을 만족하지 않는 가장 작은 Heyting algebra 이며, 이를 Gödel 의 3치 대수라고도 부른다.

# 활용

## 직관주의 수학의 의미론

Heyting algebra 는 [수학적 직관주의](mathematical-intuitionism.md)의 논리를 대수로 고정시킨다. 증명을 구성으로 읽는 관점에서 볼 때, `a → b` 가 "최대"로 정의된다는 것은 함의가 자료가 아니라 변환 가능성의 한계로 규정된다는 뜻이다. Tarski 의 위상적 해석은 열린집합 격자만으로 IPC 가 완전함을 보이며, 실직선 하나만으로도 반례를 모두 만들 수 있다[^1].

## 중간논리

IPC 와 고전논리 사이에 놓이는 논리들을 중간논리라 한다. 각 중간논리는 Heyting algebra 의 어떤 부분 종족에 대응한다. 예를 들어 `(a → b) ∨ (b → a) = 1` 을 더하면 선형 순서에 가까운 대수만 남고 Gödel–Dummett 논리가, `¬a ∨ ¬¬a = 1` 을 더하면 de Morgan 법칙이 회복된 논리가 나온다. 대수 쪽 종족의 격자 구조가 논리 쪽 확장의 격자 구조와 같으므로, 논리학의 질문이 보편대수의 질문으로 옮겨진다.
