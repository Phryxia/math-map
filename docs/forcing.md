...
아무 사슬이나 늘리면 $M$ 에 이미 있는 비트열이 나온다. 뒤를 전부 $0$ 으로 채우면 그렇다. $M$ 안의 실수 $x$ 를 피하려면 $x$ 와 값이 갈리는 자리를 한 번은 지나야 하는데, 어떤 유한 비트열에서 출발하든 한 자리만 더 붙이면 $x$ 와 갈리게 할 수 있다. 이처럼 어디서 출발해도 늘려서 도달할 수 있는 $M$ 안의 비트열 모임을 관문이라 부르자. 사슬이 관문을 전부 지나면 얻은 비트열은 $M$ 의 어느 실수와도 다르다.

$M$ 을 가산으로 잡으면 관문도 가산 개이므로 하나씩 차례로 통과시켜 사슬을 완성할 수 있다. 얻은 비트열은 $M$ 밖에 있지만 $M$ 안의 유한 비트열들로만 만들어졌으므로, 각 유한 비트열이 "내가 사슬에 들어가면 새 우주에서 이런 일이 성립한다"고 정하는 바를 $M$ 안에서 계산할 수 있다. 관문은 조밀집합, 사슬은 일반 필터, 계산되는 관계는 강제 관계다.
