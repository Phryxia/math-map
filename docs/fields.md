...
[벡터 공간](vector-spaces.md)과 [선형사상](linear-maps.md)은 계수가 체일 때 정의된다. 체에서 나눗셈이 가능하다는 성질이 Gauss 소거법의 피벗 나눗셈, 기저의 존재, 차원의 유일성을 보장한다. 계수를 $\mathbb{Z}$ 로 바꾸면 기저가 없는 유한생성 가군(예: $\mathbb{Z}/2\mathbb{Z}$ )이 생긴다.
