...
고전적 분할, 불완전 분해, 다중격자, 영역 분할이 표준 선택이다. 이 가운데 어느 것도 모든 행렬에서 좋지 않고, 문제를 만든 미분방정식이나 그래프의 구조가 선택을 정한다.
...
격자를 여러 단계로 두고, 각 단계에서 Gauss–Seidel 몇 번으로 고주파 오차를 줄인 뒤 남은 저주파 오차를 성긴 격자로 옮겨 푼다. 이것을 재귀로 쌓은 것이 V 사이클이고, 한 사이클의 비용이 $O(n)$ 이면서 오차를 상수배로 줄인다. 수렴률이 격자 간격과 무관하므로 스펙트럼 등가 상수가 $h$ 에 의존하지 않는 전처리가 된다.

행렬의 성분만으로 성긴 단계와 옮기는 연산자를 만드는 것을 대수적 다중격자라 한다. 격자가 명시적으로 없는 문제에 쓴다.

## 영역 분할

정의역을 부분영역으로 쪼개고 각 부분에서 계를 따로 푼 결과를 더하는 것을 가법 Schwarz 전처리라 한다. $R\_i$ 를 부분영역으로의 제한이라 할 때 $M^{-1} = \sum_i R\_i^{\top} A\_i^{-1} R\_i$ 이고 $A\_i = R\_i A R\_i^{\top}$ 다. 부분영역을 겹치게 잡고 성긴 격자 성분을 하나 더하면 조건수 상한이 부분영역의 개수와 무관해진다.

각 $A\_i^{-1}$ 의 적용이 독립이므로 병렬 계산에 쓴다.
