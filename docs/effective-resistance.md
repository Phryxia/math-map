...
유효저항은 그래프를 전기 회로로 보고 간선 가중치를 전도도로 삼아, 두 정점 사이에 단위 전류를 흘릴 때의 전위차를 잰다. 경로가 많으면 전류가 나뉘어 흘러 저항이 낮아지므로 최단 거리가 아니라 연결의 두께를 재는 양이다.

계산은 [그래프 Laplacian](graph-laplacian.md)의 선형방정식 하나로 끝난다. Laplacian 이 회로의 Kirchhoff 법칙을 표현하기 때문이다. 이 양은 무작위 행보의 도달 시간, 신장트리의 개수, 간선의 중요도 샘플링으로 이어진다.
...
$R$ 은 정점 집합 위의 거리함수다. 대칭성과 $R(u,v) = 0 \iff u = v$ 는 정의에서 나오고 삼각부등식은 에너지의 볼록성에서 따라온다.
...
확률적 대상과 전기적 대상이 같은 방정식을 만족하기 때문이며, 두 언어의 사전은 [Random walk와 전기 네트워크](random-walks.md)에서 다룬다.
