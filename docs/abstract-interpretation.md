...
정확한 분석은 불가능하므로 무엇을 잃을지 먼저 정한다. 변수 $x$ 의 정확한 값 집합 $\lbrace 2,5,7\rbrace$ 대신 구간 $[2,7]$ 만 기억하기로 하면, 집합의 격자에서 구간의 격자로 내려온 것이다. $\alpha(\lbrace 2,5,7\rbrace)=[2,7]$ 이고 $\gamma([2,7])=\lbrace 2,3,4,5,6,7\rbrace$ 이라 원래보다 커진다. 커지는 방향이 항상 같다는 것이 요점이다. **과대근사만 하고 과소근사는 절대 하지 않는다.**
...
2. 임의의 수열 $(b_n)$ 에 대해 $a_0=b_0$ 이고 $a_{n+1}=a_n\nabla b_{n+1}$ 로 정의한 수열이 유한 단계에 안정화
