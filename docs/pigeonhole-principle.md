...
비둘기집 원리는 합과 평균의 부등식을 쓰는 이중 계산의 특수한 형태다. 같은 논리를 기댓값으로 바꾼 것이 어떤 원소가 기댓값 이상의 값을 갖는다는 평균 논증(averaging argument)이고, [확률적 방법](probabilistic-method.md)이 이를 [확률 공간](probability.md) 위에서 쓴다.
