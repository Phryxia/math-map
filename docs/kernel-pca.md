...
$\tilde K$ 가 $n\times n$ 이므로 저장에 $O(n^2)$ 이, 고유분해에 $O(n^3)$ 이 든다. 원 공간 차원 $d$ 와 무관하지만 표본 수에는 세제곱으로 커지므로, 표본이 많으면 선형 PCA 보다 훨씬 비싸다. 실무에서는 Nyström 근사나 무작위 특징으로 $K$ 를 저계수 근사한다.
