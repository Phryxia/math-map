...
방법은 측도를 직접 비교하는 대신 **시험함수로 재는 것**이다. 유계 연속함수 $f$ 를 하나 고정하고 평균 $\int f\thinspace d\mu_n$ 을 재면 수 하나가 나온다. 모든 시험함수에 대해 이 수들이 수렴하면 측도가 수렴한다고 부른다. 함수해석의 약$^\ast$ 위상을 확률측도에 적용한 것이고, "약"이라는 이름도 거기서 온다.
