...
부호 하나를 뒤집으면 합이 그 수의 두 배만큼 움직이므로 $1,2,4$ 에서는 값이 흩어진다. 같은 값을 여러 번 받으려면 뒤집은 자리들이 서로 상쇄해야 하는데, 수가 커질수록 상쇄할 조합이 줄어든다. 모든 수의 절댓값이 $1$ 이상이라는 조건은 뒤집기 하나가 합을 최소 $2$ 만큼 움직인다는 뜻이다.
...
**정리**[^3]**.** $a_i$ 가 [노름공간](banach-spaces.md)의 벡터이고 $\Vert a_i\Vert \ge 1$ 이면, 지름이 $2$ 미만인 집합 하나에 들어가는 부호합의 개수는 $\binom{n}{\lfloor n/2\rfloor}$ 을 넘지 않는다.
