...
선형회귀는 반응변수의 조건부평균을 설명변수들의 선형결합으로 모형화하고 계수를 잔차제곱합 최소화로 정하는 방법이다. 이 최소화는 기하학적으로 관측 벡터를 설계행렬의 열공간 위로 정사영하는 것이고([내적 공간](inner-product-spaces.md)), 확률적으로 오차가 정규분포일 때의 [최대가능도 추정](maximum-likelihood.md)이다.

정사영 정리가 해의 유일성과 총제곱합의 직교분해를 증명한다. 오차에 정규분포를 가정하면 추정량의 분포가 정해지고 계수의 검정과 신뢰구간이 따라온다. Gauss–Markov 정리는 분포 가정 없이 2 차 모멘트 가정만으로 최소제곱추정량이 선형불편추정량 중 최소분산임을 말한다.

# 직관

평면에 흩어진 점들에 직선을 맞출 때 적합의 기준을 세로 거리의 제곱합으로 잡으면 계수가 유일하게 정해지고 식으로 쓸 수 있다.
...
로 정의된다. $Xb$ 의 집합이 $X$ 의 열공간 $C(X)$ 이므로 이 문제는 부분공간 위의 최근접점 찾기다. 내적 공간의 정사영 정리로 최근접점이 유일하게 존재하고, 그 점 $\hat y$ 는
...
대칭이고 멱등이므로 [고윳값](eigenvalues.md)이 0 과 1 뿐이고, 1 의 중복도가 $\mathrm{rank}(X)=p$ 라 대각합이 $p$ 다. 잔차는 $e=(I-H)y$ 이고 $I-H$ 역시 $C(X)$ 의 직교여공간으로 가는 사영행렬이다. 대각원소 $h_{ii}$ 가 관측 $i$ 의 leverage 이고 $0\le h_{ii}\le1$ 이며 합이 $p$ 다. $h_{ii}$ 는 $\partial\hat y_i/\partial y_i$ 와 같으므로 leverage 가 큰 관측은 자기 적합값을 크게 움직인다.
...
등호는 $d=0$ 일 때만 성립한다. 증명에 오차의 분포를 쓰지 않았다. 선형과 불편이라는 제약을 풀면 더 좋은 추정량이 있다. ridge 추정량은 편향을 감수하고 평균제곱오차를 줄인다.
...
을 얻는데 아래로 편향되어 있어 실무에서는 $n-p$ 로 나눈 값을 쓴다. 오차가 Laplace 분포이면 같은 계산이 절대편차합 최소화, 곧 중앙값 회귀를 낸다. 손실함수를 고르는 것이 오차분포를 가정하는 것이다.
...
설명변수들이 서로 거의 선형종속이면 $X^\top X$ 가 거의 특이해져 계수의 분산이 커진다. $j$ 번째 변수를 나머지 변수들로 회귀했을 때의 결정계수를 $R_j^2$ 라 하면 분산팽창계수(variance inflation factor, VIF)는
...
- **가중최소제곱과 GLS**: 일반화최소제곱(generalized least squares, GLS)은 오차의 분산이 다르거나 상관이 있을 때 쓴다. $\mathrm{Var}(\varepsilon) = \sigma^2 \Sigma$ 로 두고 $\Sigma^{-1}$ 을 내적으로 쓰는 정사영을 한다. 내적을 바꾸어도 정사영의 기하는 그대로다.
