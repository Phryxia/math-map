...
접선 근사 `f(x) ≈ f(x_k) + f'(x_k)(x - x_k)` 의 영점을 취하는 것이 반복식의 전부다. 즉 "비선형 문제를 한 번의 선형 문제로 바꿔 푸는 일"을 반복한다.

왜 빠른가. 해에서 접선 근사의 오차는 이차 항부터 시작한다. 따라서 새 오차는 옛 오차의 제곱에 비례하고, 유효자릿수가 대략 매 단계 두 배가 된다. 반대로 말하면 이 속도는 해 근처에서만 보장된다. 멀리서는 접선이 함수를 전혀 대표하지 못하므로 어떤 일이 일어나도 이상하지 않다.

고정점의 관점에서 보면 더 선명하다. 반복 사상을 `g(x) = x - f(x)/f'(x)` 로 두면, 단순근 `r` 에서 `g(r) = r` 이고 `g'(r) = 0` 이다. 고정점에서 도함수가 0이라는 것은 축약 계수를 원하는 만큼 작게 잡을 수 있다는 뜻이고, 그래서 수렴이 단순한 기하급수보다 빠르다.

```mermaid
graph TD
  A["초기값 x0"] --> B["함숫값과 도함숫값 계산"]
  B --> C{"도함수가 0에 가까운가?"}
  C -->|예| F["실패: 접선이 거의 수평"]
  C -->|아니오| D["접선의 영점으로 갱신"]
  D --> E{"잔차와 변화량이 충분히 작은가?"}
  E -->|아니오| B
  E -->|예| G["해 반환"]
  D -.->|초기값이 나쁠 때| H["발산 · 주기 궤도 · 다른 해로 이동"]
```
...
단순근 `r` (즉 `f(r) = 0`, `f'(r) ≠ 0`)에서는 `g'(r) = 0` 이다.
...
최적화 문제 `min f(x)` 에 적용할 때는 `F = grad f` 로 두므로 Jacobian이 Hessian이 된다.
...
`f` 가 두 번 연속미분가능하고 `r` 이 단순근이라 하자. `g'` 이 연속이고 `g'(r) = 0` 이므로, 임의의 양수 `q < 1` 에 대해 구간 `I = [r-d, r+d]` 를 충분히 작게 잡아 `|g'| ≤ q` 가 되게 할 수 있다. 평균값 정리로
...
- 중근. `f` 가 `r` 에서 중복도 `m` 의 근을 가지면 `g'(r) = 1 - 1/m` 이므로 수렴이 선형으로 떨어진다. 예컨대 이중근에서는 오차가 매 단계 절반씩만 준다. 반복식을 `x - m f/f'` 로 고치면 이차 수렴이 복구된다.
- 접선이 수평. `f'(x_k)` 가 0에 가까우면 한 단계에서 아주 멀리 튕겨 나간다.
- 주기 궤도. `f(x) = x^3 - 2x + 2` 에서 `x0 = 0` 으로 시작하면 `0 -> 1 -> 0 -> 1` 로 2주기 궤도에 갇힌다. 이 궤도는 끌개이므로 근처의 초기값들도 같은 운명을 맞는다.
- 발산. `f(x) = x^{1/3}` 은 원점에서만 0이지만 반복식이 `x_{k+1} = -2 x_k` 가 되어 어떤 초기값에서도 발산한다. 도함수가 해에서 유계가 아닌 경우다.
...
실무에서는 선탐색(감쇠 Newton), 신뢰영역, 이분법과의 결합(Brent 류)으로 이 문제들을 막는다. 감쇠 Newton은 단계 `x_k + t Δ_k` 에서 `t` 를 1부터 줄여 잔차가 실제로 감소하는 값을 고른다.
...
- Broyden 법. 다변수에서 `Δ` 와 `F` 의 변화만으로 Jacobian 근사를 계급 1 갱신한다.
...
나눗셈을 쓰지 않고 역수를 구하려면 `f(x) = 1/x - a` 를 쓴다. 반복식이 `x_{k+1} = x_k(2 - a x_k)` 가 되어 곱셈과 뺄셈만 남는다. 하드웨어의 부동소수점 나눗셈이 실제로 이런 식으로 구현된다.
...
for a in (2.0, 1e-6, 1e12):
    x = sqrt_newton(a)
    print(a, x, abs(x * x - a) / a)

# 오차가 매 단계 제곱으로 줄어드는 모습
x, a = 1.0, 2.0
for k in range(6):
    x = 0.5 * (x + a / x)
    print(k, repr(x), abs(x - 2 ** 0.5))
```

`a = 2` 에서 초기값 1로 시작하면 오차가 대략 0.08, 0.002, 2e-6, 1e-12 순으로 줄어든다. 유효자릿수가 매 단계 두 배가 되는 이차 수렴의 전형이다.[^1]
