...
값이 $-1$ 과 $0$ 과 $-2$ 로 소수점 여섯째 자리까지 정수다. 부호는 곡선의 방향 규약이 정한다.
...
이유는 이 적분이 사실 **차수 사상의 적분 표현**이기 때문이다. 사상 $T^2\to S^2$ 인 $(s,t)\mapsto(\mathbf r_1-\mathbf r_2)/|\mathbf r_1-\mathbf r_2|$ 가 구면의 부피형식을 당겨 온 것이 피적분함수이고, 그 적분은 사상의 차수 곧 정수다. 차수는 연속 변형에 불변이므로, 격자를 성기게 잡아 곡선을 조금 다르게 근사해도 **같은 정수**가 나온다. 수치적 안정성이 위상적 불변성의 직접적 결과다.
