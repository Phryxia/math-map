...
집합은 원소만으로 결정되는 대상이다. 순서와 중복은 구별하지 않는다. 함수, 관계, 수 체계, 확률 공간, 그래프, 위상 공간은 모두 집합 위에 구조를 얹어 정의한다. 어떤 모임이 집합인지는 [ZFC 공리계](zfc-axioms.md)(Zermelo–Fraenkel with choice)가 정한다.
