...
$\in$ 와 $\subseteq$ 는 다른 관계다. $1 \in \{1, 2\}$ 이지만 $1$ 은 부분집합이 아니고, $\{1\} \subseteq \{1, 2\}$ 이지만 $\{1\}$ 은 원소가 아니다. 집합이 다른 집합의 원소가 될 수 있기 때문에 이 구별이 필요하며, 공집합과 공집합만을 원소로 갖는 집합이 다른 것도 같은 이유다.
...
$A$ 를 $\{1,3\}$ 이라 하고 $B$ 를 $\{3,5\}$ 라 하면 교집합은 $\{3\}$ 이고 합집합은 $\{1,3,5\}$ 이며 차집합 $A\setminus B$ 는 $\{1\}$ 이다. 전체집합 $U$ 를 고정하면 여집합은 $U\setminus A$ 다.
...
무한집합에서는 $P(A)$ 가 $A$ 보다 반드시 크다는 Cantor 정리가 성립한다. 어떤 함수 $f : A \to P(A)$ 를 가져와도 $D = \{ x \in A : x \notin f(x) \}$ 는 상에 들어 있지 않기 때문이다. $D = f(a)$ 라면 $a \in D$ 인지 묻는 순간 모순이 나온다. 이 대각선 논법이 [가산성과 비가산성](cardinality.md)의 출발점이다.
