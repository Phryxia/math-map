...
공 $\Vert v\Vert\le r$ 에 적용하면 최단벡터의 길이 상계가 나온다. 존재만 말해 줄 뿐 어디 있는지는 알려 주지 않는 전형적인 비구성적 정리이며, 실제로 찾는 문제가 계산적으로 어렵다는 것이 이 분야의 출발점이다.
...
좋은 기저를 실제로 찾는 문제는 어렵지만, 어느 정도 좋은 기저는 다항시간에 찾을 수 있다. Gram–Schmidt 직교화를 $b_i^*$ 로, 계수를 $\mu_{ij}$ 라 할 때 다음 두 조건을 만족하면 LLL 축소 기저다.
